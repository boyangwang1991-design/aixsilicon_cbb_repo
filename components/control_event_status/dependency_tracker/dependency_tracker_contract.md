...
| 索引 ID | CTL-023 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/dependency_tracker |
| 功能家族 | Dependency Tracker |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：为操作维护未满足依赖的计数或位图，依赖满足后仅一次将操作标记为 ready。

边界焦点：重复完成事件不能重复减计数；回收后的 tag 不接受旧 generation 的依赖通知，不执行完整任务调度。
...
候选实现：counter/bitmap/DAG subset。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 为操作维护未满足依赖的计数或位图，依赖满足后仅一次将操作标记为 ready。 |
| REQ-002 | 关键边界 | 重复完成事件不能重复减计数；回收后的 tag 不接受旧 generation 的依赖通知，不执行完整任务调度。 |
