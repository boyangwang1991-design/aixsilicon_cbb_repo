...
| 索引 ID | CTL-001 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/up_down_counter |
| 功能家族 | Up/Down Counter |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：根据 up/down 输入更新计数值，支持配置的模回绕或饱和行为及可选 Gray 观察输出。

边界焦点：同拍 up 和 down 默认抵消，reset/clear/load 优先于计数；溢出和借位需独立反馈。
...
候选实现：binary/Gray/saturating。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 根据 up/down 输入更新计数值，支持配置的模回绕或饱和行为及可选 Gray 观察输出。 |
| REQ-002 | 关键边界 | 同拍 up 和 down 默认抵消，reset/clear/load 优先于计数；溢出和借位需独立反馈。 |
