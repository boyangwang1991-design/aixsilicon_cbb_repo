...
| 索引 ID | CTL-006 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/watchdog_core |
| 功能家族 | Watchdog |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
无总线/CSR的通用看门狗核，关注安全诊断覆盖

需求焦点：维护喂狗时间窗口，合法喂狗重装计数，过早或过晚喂狗产生不同局部故障标志。

边界焦点：配置改变需要显式重新启动；不直接复位 SoC，不提供 CSR、系统复位原因管理或中断路由。
...
候选实现：windowed/non-windowed。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 维护喂狗时间窗口，合法喂狗重装计数，过早或过晚喂狗产生不同局部故障标志。 |
| REQ-002 | 关键边界 | 配置改变需要显式重新启动；不直接复位 SoC，不提供 CSR、系统复位原因管理或中断路由。 |
