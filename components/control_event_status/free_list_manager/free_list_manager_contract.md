...
registry_id: CTL-021
name: free_list_manager
version: 0.1.0
status: draft
basis: planning_intent
---

# free_list_manager 需求合同
...
| 索引 ID | CTL-021 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/free_list_manager |
| 功能家族 | Free-list Manager |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：维护空闲索引的 FIFO 或栈，支持成组分配与回收，并保持索引唯一性。

边界焦点：多路回收去重或报错，不得把同一索引入表两次；空列表不返回有效索引。
...
候选实现：FIFO/stack/bitmap。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 维护空闲索引的 FIFO 或栈，支持成组分配与回收，并保持索引唯一性。 |
| REQ-002 | 关键边界 | 多路回收去重或报错，不得把同一索引入表两次；空列表不返回有效索引。 |
