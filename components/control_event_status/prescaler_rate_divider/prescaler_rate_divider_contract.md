...
registry_id: CTL-007
name: prescaler_rate_divider
version: 0.1.0
status: draft
basis: planning_intent
---

# prescaler_rate_divider 需求合同
...
| 索引 ID | CTL-007 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/prescaler_rate_divider |
| 功能家族 | Prescaler/Rate Divider |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：将输入 tick 按整数除数或相位累加比例转换为输出 tick，长期计数符合所选比率。

边界焦点：输出最多每输入周期一次；分数模式保留余相位，重载模式明确清除相位，除数零拒绝。

拟复用场景：外围 IP 局部计时与状态管理；加速器内部依赖/事件控制。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：局部事件/控制、配置值与可选资源索引输入；计数、脉冲、状态或资源结果输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：计数位宽≥1并覆盖门限；容量/模数≥1；窗口和超时单位为显式 tick；索引范围受资源数约束。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：integer/fraction accumulator。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 将输入 tick 按整数除数或相位累加比例转换为输出 tick，长期计数符合所选比率。 |
| REQ-002 | 关键边界 | 输出最多每输入周期一次；分数模式保留余相位，重载模式明确清除相位，除数零拒绝。 |
| REQ-003 | 时序/接受 | 状态只在对应有效事件推进；单周期事件不得重复触发，电平请求与脉冲事件不能混用。 |
| REQ-004 | 复位/重启 | reset 优先于常规操作；clear/load 与新事件同拍优先级采用逐项定义，其他情况下不静默丢掉事件。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：零活动、连续活动、同拍相反事件、门限/回绕、容量满空、暂停/恢复与非法索引。 |
