...
| 索引 ID | CTL-022 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/scoreboard |
| 功能家族 | Scoreboard |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：对有限资源/tag 记录 busy 或完成状态，查询给出当前依赖是否满足。

边界焦点：分配、完成和清除同拍采用明确的旧状态查询/新状态更新规则；未知完成必须检测。
...
候选实现：bit/vector/tagged。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对有限资源/tag 记录 busy 或完成状态，查询给出当前依赖是否满足。 |
| REQ-002 | 关键边界 | 分配、完成和清除同拍采用明确的旧状态查询/新状态更新规则；未知完成必须检测。 |
