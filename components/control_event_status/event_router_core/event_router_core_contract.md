...
registry_id: CTL-016
name: event_router_core
version: 0.1.0
status: draft
basis: planning_intent
---

# event_router_core 需求合同
...
| 索引 ID | CTL-016 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/event_router_core |
| 功能家族 | Event Router |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
无系统地址图/CSR的路由核，关注Mux、扇出和配置

需求焦点：按静态或显式配置的源到目的映射将局部事件路由到一个或多个目的输出。

边界焦点：未映射源给出丢弃指示；配置切换在安全边界提交，不定义芯片中断号或软件地址图。
...
候选实现：programmable/static map。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按静态或显式配置的源到目的映射将局部事件路由到一个或多个目的输出。 |
| REQ-002 | 关键边界 | 未映射源给出丢弃指示；配置切换在安全边界提交，不定义芯片中断号或软件地址图。 |
