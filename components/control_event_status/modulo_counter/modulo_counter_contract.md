...
| 索引 ID | CTL-002 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/modulo_counter |
| 功能家族 | Modulo Counter |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：按使能在 0 到 M-1 循环计数，并在 M-1 被推进到零时输出一次回绕事件。

边界焦点：M=1 每次有效推进均产生回绕，停顿不产生回绕；动态修改 M 需通过重载重新建立合法状态。
...
候选实现：arbitrary/power-of-two。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按使能在 0 到 M-1 循环计数，并在 M-1 被推进到零时输出一次回绕事件。 |
| REQ-002 | 关键边界 | M=1 每次有效推进均产生回绕，停顿不产生回绕；动态修改 M 需通过重载重新建立合法状态。 |
