...
registry_id: CTL-003
name: timestamp_counter
version: 0.1.0
status: draft
basis: planning_intent
---

# timestamp_counter 需求合同
...
| 索引 ID | CTL-003 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/timestamp_counter |
| 功能家族 | Timestamp Counter |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：按基准时钟和可选预分频生成单调模计数时间戳，并支持原子快照。

边界焦点：回绕是明确的模语义；跨域读取必须经快照或 Gray 同步，不能直接采样二进制计数总线。
...
候选实现：free-running/prescaled。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按基准时钟和可选预分频生成单调模计数时间戳，并支持原子快照。 |
| REQ-002 | 关键边界 | 回绕是明确的模语义；跨域读取必须经快照或 Gray 同步，不能直接采样二进制计数总线。 |
