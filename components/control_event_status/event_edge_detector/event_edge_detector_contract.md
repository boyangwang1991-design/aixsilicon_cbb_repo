...
registry_id: CTL-013
name: event_edge_detector
version: 0.1.0
status: draft
basis: planning_intent
---

# event_edge_detector 需求合同
...
| 索引 ID | CTL-013 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/event_edge_detector |
| 功能家族 | Event Edge Detector |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：在同一时钟域比较当前和前一次采样值，分别输出上升、下降或双沿单周期事件。

边界焦点：复位后首次采样只建立基线而不伪造边沿；异步输入先由外部同步器处理。
...
候选实现：rise/fall/both。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在同一时钟域比较当前和前一次采样值，分别输出上升、下降或双沿单周期事件。 |
| REQ-002 | 关键边界 | 复位后首次采样只建立基线而不伪造边沿；异步输入先由外部同步器处理。 |
