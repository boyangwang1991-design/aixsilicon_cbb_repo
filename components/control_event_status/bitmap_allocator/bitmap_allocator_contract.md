...
| 索引 ID | CTL-020 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/bitmap_allocator |
| 功能家族 | Bitmap Allocator |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：从空闲位图选一个或多个置位位置输出索引，分配成功时清除对应空闲位。

边界焦点：回收只作用于已分配索引；位图尾部非资源位永久屏蔽，耗尽时 valid 为零。
...
候选实现：linear/tree/hierarchical。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 从空闲位图选一个或多个置位位置输出索引，分配成功时清除对应空闲位。 |
| REQ-002 | 关键边界 | 回收只作用于已分配索引；位图尾部非资源位永久屏蔽，耗尽时 valid 为零。 |
