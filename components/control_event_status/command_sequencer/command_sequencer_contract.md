...
registry_id: CTL-011
name: command_sequencer
version: 0.1.0
status: draft
basis: planning_intent
---

# command_sequencer 需求合同
...
| 索引 ID | CTL-011 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/command_sequencer |
| 功能家族 | Command Sequencer |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：按入队顺序把有限命令转换为本地操作握手，并在完成后返回原命令标识。

边界焦点：等待期间命令参数保持稳定；错误只终止当前命令或按显式 flush 清队，不能静默跳过命令。
...
候选实现：queue/FSM/table。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按入队顺序把有限命令转换为本地操作握手，并在完成后返回原命令标识。 |
| REQ-002 | 关键边界 | 等待期间命令参数保持稳定；错误只终止当前命令或按显式 flush 清队，不能静默跳过命令。 |
