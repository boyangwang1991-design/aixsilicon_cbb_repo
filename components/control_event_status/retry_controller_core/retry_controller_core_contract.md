...
registry_id: CTL-012
name: retry_controller_core
version: 0.1.0
status: draft
basis: planning_intent
---

# retry_controller_core 需求合同
...
| 索引 ID | CTL-012 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/retry_controller_core |
| 功能家族 | Retry Controller |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：对失败操作按有限次数和配置退避间隔发出重试请求，成功或耗尽后给出终态。

边界焦点：重试次数只在一次重试实际派发后增加；不保存协议报文、不生成总线重放，也不无限忙等。
...
候选实现：bounded/backoff/selective。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对失败操作按有限次数和配置退避间隔发出重试请求，成功或耗尽后给出终态。 |
| REQ-002 | 关键边界 | 重试次数只在一次重试实际派发后增加；不保存协议报文、不生成总线重放，也不无限忙等。 |
