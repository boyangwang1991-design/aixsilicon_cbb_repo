...
registry_id: CTL-014
name: pulse_stretcher_compressor
version: 0.1.0
status: draft
basis: planning_intent
---

# pulse_stretcher_compressor 需求合同
...
| 索引 ID | CTL-014 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/pulse_stretcher_compressor |
| 功能家族 | Pulse Stretcher/Compressor |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：把接受的事件转换为配置长度的电平脉冲，或把持续电平压缩为一次单周期事件。

边界焦点：重触发策略显式选择延长或忽略；压缩模式必须等输入撤销后重新武装，不能每拍重复触发。
...
候选实现：fixed/programmable。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把接受的事件转换为配置长度的电平脉冲，或把持续电平压缩为一次单周期事件。 |
| REQ-002 | 关键边界 | 重触发策略显式选择延长或忽略；压缩模式必须等输入撤销后重新武装，不能每拍重复触发。 |
