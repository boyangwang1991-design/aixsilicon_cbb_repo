...
registry_id: CTL-024
name: quiesce_drain_ctrl
version: 0.1.0
status: draft
basis: planning_intent
---

# quiesce_drain_ctrl 需求合同
...
| 索引 ID | CTL-024 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/quiesce_drain_ctrl |
| 功能家族 | Quiesce/Drain Controller |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在停止请求后禁止接受新工作，并等待现有未完成工作归零后确认 quiescent。

边界焦点：停止边界上已握手工作计入待排空集合；恢复请求解除禁止，超时仅报告本地排空失败。
...
候选实现：stop-accept/drain/ack。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在停止请求后禁止接受新工作，并等待现有未完成工作归零后确认 quiescent。 |
| REQ-002 | 关键边界 | 停止边界上已握手工作计入待排空集合；恢复请求解除禁止，超时仅报告本地排空失败。 |
