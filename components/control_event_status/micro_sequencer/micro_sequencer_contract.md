...
registry_id: CTL-010
name: micro_sequencer
version: 0.1.0
status: draft
basis: planning_intent
---

# micro_sequencer 需求合同
...
| 索引 ID | CTL-010 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/micro_sequencer |
| 功能家族 | Micro-sequencer |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：按控制 ROM/表中的指令产生局部控制输出，支持有限分支、等待和结束。

边界焦点：程序计数器不得越界执行；非法操作码停止并报错，不实现处理器 ISA 或可软件管理的完整微控制器。
...
候选实现：ROM/table-driven。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按控制 ROM/表中的指令产生局部控制输出，支持有限分支、等待和结束。 |
| REQ-002 | 关键边界 | 程序计数器不得越界执行；非法操作码停止并报错，不实现处理器 ISA 或可软件管理的完整微控制器。 |
