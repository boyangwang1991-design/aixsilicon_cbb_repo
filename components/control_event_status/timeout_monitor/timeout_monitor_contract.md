...
| 索引 ID | CTL-005 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/timeout_monitor |
| 功能家族 | Timeout Monitor |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：从 start 到完成或 cancel 监控有效计时 tick，超过门限时产生一次超时事件。

边界焦点：同拍完成与门限到达默认完成优先；暂停是否计时由静态模式决定，不能把背压一律判为故障。
...
候选实现：cycle/event/progress based。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 从 start 到完成或 cancel 监控有效计时 tick，超过门限时产生一次超时事件。 |
| REQ-002 | 关键边界 | 同拍完成与门限到达默认完成优先；暂停是否计时由静态模式决定，不能把背压一律判为故障。 |
