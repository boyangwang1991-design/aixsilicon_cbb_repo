...
| 索引 ID | CTL-015 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/event_collector |
| 功能家族 | Event Collector |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：将多路同域事件收集为 OR、位图或计数输出，保证所选聚合模式的事件语义明确。

边界焦点：位图模式保留同时多源，计数模式按同拍置位数增加；OR 模式只承诺有无事件，不承诺次数。
...
候选实现：OR/tree/bitmap/count。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 将多路同域事件收集为 OR、位图或计数输出，保证所选聚合模式的事件语义明确。 |
| REQ-002 | 关键边界 | 位图模式保留同时多源，计数模式按同拍置位数增加；OR 模式只承诺有无事件，不承诺次数。 |
