...
registry_id: CTL-019
name: sequence_number_manager
version: 0.1.0
status: draft
basis: planning_intent
---

# sequence_number_manager 需求合同
...
| 索引 ID | CTL-019 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/sequence_number_manager |
| 功能家族 | Sequence Number Manager |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：在发送接受时分配下一个序号，并按模数处理回绕及有限窗口中的新旧比较。

边界焦点：窗口跨度必须小于模数的一半以消除歧义；重置会开启新序列周期，不能与旧未完成项混用。
...
候选实现：wrap/window/compare。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在发送接受时分配下一个序号，并按模数处理回绕及有限窗口中的新旧比较。 |
| REQ-002 | 关键边界 | 窗口跨度必须小于模数的一半以消除歧义；重置会开启新序列周期，不能与旧未完成项混用。 |
