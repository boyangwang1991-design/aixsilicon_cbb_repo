...
| 索引 ID | CTL-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/control_event_status / components/control_event_status/timer_core |
| 功能家族 | Timer |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
无总线/CSR的通用计时核，关注Prescaler共享

需求焦点：按加载值执行单次或周期倒计时，到期时产生局部事件而不实现总线或中断控制器。

边界焦点：load 优先于 tick，单次到期后停止；周期零值非法，停止与重新启动不得复用不明确的残余计数。
...
候选实现：one-shot/periodic/cascade。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按加载值执行单次或周期倒计时，到期时产生局部事件而不实现总线或中断控制器。 |
| REQ-002 | 关键边界 | load 优先于 tick，单次到期后停止；周期零值非法，停止与重新启动不得复用不明确的残余计数。 |
