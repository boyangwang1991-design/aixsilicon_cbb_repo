...
registry_id: DFT-005
name: mbist_port_arbiter
version: 0.1.0
status: draft
basis: planning_intent
---

# mbist_port_arbiter 需求合同
...
| 索引 ID | DFT-005 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dft_test / components/dft_test/mbist_port_arbiter |
| 功能家族 | MBIST Port Arbiter |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：在功能端与 MBIST 端之间切换存储端口所有权，任一时刻最多一个端口可发有效访问。

边界焦点：切到测试前排空已接受功能访问；所有权确认前禁止两端同时驱动，不实现 March 算法控制器。
...
候选实现：functional/test ownership。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在功能端与 MBIST 端之间切换存储端口所有权，任一时刻最多一个端口可发有效访问。 |
| REQ-002 | 关键边界 | 切到测试前排空已接受功能访问；所有权确认前禁止两端同时驱动，不实现 March 算法控制器。 |
