...
registry_id: DFT-001
name: test_mode_synchronizer
version: 0.1.0
status: draft
basis: planning_intent
---

# test_mode_synchronizer 需求合同
...
| 索引 ID | DFT-001 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dft_test / components/dft_test/test_mode_synchronizer |
| 功能家族 | Test-mode Synchronizer |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：把静态测试模式选择同步到功能域，并提供稳定的功能/测试模式状态。

边界焦点：测试模式只能在规定的静止或复位窗口改变；切换期间不得形成混合模式控制。
...
候选实现：static test control sync。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把静态测试模式选择同步到功能域，并提供稳定的功能/测试模式状态。 |
| REQ-002 | 关键边界 | 测试模式只能在规定的静止或复位窗口改变；切换期间不得形成混合模式控制。 |
