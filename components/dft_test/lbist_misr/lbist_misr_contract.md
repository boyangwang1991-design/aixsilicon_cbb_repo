...
| 索引 ID | DFT-006 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dft_test / components/dft_test/lbist_misr |
| 功能家族 | LBIST/MISR |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：每个有效测试采样将输入响应压缩到 MISR 状态，反馈由配置多项式和种子决定。

边界焦点：被屏蔽响应不污染签名；移位步数与有效采样次数一致，不宣称签名相等代表绝对无故障。
...
候选实现：configurable polynomial/width。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 每个有效测试采样将输入响应压缩到 MISR 状态，反馈由配置多项式和种子决定。 |
| REQ-002 | 关键边界 | 被屏蔽响应不污染签名；移位步数与有效采样次数一致，不宣称签名相等代表绝对无故障。 |
