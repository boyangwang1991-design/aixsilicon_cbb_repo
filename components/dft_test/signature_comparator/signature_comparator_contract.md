...
registry_id: DFT-008
name: signature_comparator
version: 0.1.0
status: draft
basis: planning_intent
---

# signature_comparator 需求合同
...
| 索引 ID | DFT-008 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dft_test / components/dft_test/signature_comparator |
| 功能家族 | Signature Comparator |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：在比较使能有效时对实际签名和期望签名按掩码比较，输出匹配及不匹配位图。

边界焦点：掩码为零时输出匹配并标示无有效比较位；未知输入处理需由验证检查捕获，不当作确定通过。
...
候选实现：expected/masked compare。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在比较使能有效时对实际签名和期望签名按掩码比较，输出匹配及不匹配位图。 |
| REQ-002 | 关键边界 | 掩码为零时输出匹配并标示无有效比较位；未知输入处理需由验证检查捕获，不当作确定通过。 |
