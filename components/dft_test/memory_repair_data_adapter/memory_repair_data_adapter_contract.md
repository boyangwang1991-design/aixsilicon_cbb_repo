...
registry_id: DFT-010
name: memory_repair_data_adapter
version: 0.1.0
status: draft
basis: planning_intent
---

# memory_repair_data_adapter 需求合同
...
| 索引 ID | DFT-010 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dft_test / components/dft_test/memory_repair_data_adapter |
| 功能家族 | Memory Repair Data Adapter |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：把显式输入的修复条目转换成目标宏的行/列冗余选择编码和有效位。

边界焦点：越界地址、重复条目及冗余资源不足须报告；不进行 eFuse 烧写、不选择修复算法或隐藏工艺约束。
...
候选实现：fuse/OTP/BISR mapping。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把显式输入的修复条目转换成目标宏的行/列冗余选择编码和有效位。 |
| REQ-002 | 关键边界 | 越界地址、重复条目及冗余资源不足须报告；不进行 eFuse 烧写、不选择修复算法或隐藏工艺约束。 |
