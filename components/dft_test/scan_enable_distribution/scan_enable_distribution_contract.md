...
registry_id: DFT-002
name: scan_enable_distribution
version: 0.1.0
status: draft
basis: planning_intent
---

# scan_enable_distribution 需求合同
...
| 索引 ID | DFT-002 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dft_test / components/dft_test/scan_enable_distribution |
| 功能家族 | Scan-enable Distribution Helper |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：把 scan enable 分发到本地扫描控制节点，并满足扫描时钟相位对使能稳定性的要求。

边界焦点：功能模式输出不得误置 scan enable；不同分支的锁存/寄存延迟必须匹配扫描时序约束。

拟复用场景：存储测试端口适配；局部扫描/LBIST 数据控制。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：显式测试模式、扫描/测试控制和待测数据输入；受控测试路径、签名或局部状态输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：测试数据宽度≥1；模式编码/极性显式；多项式/种子、宏端口能力或扫描相位按实际核配置。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：local latch/replication。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把 scan enable 分发到本地扫描控制节点，并满足扫描时钟相位对使能稳定性的要求。 |
| REQ-002 | 关键边界 | 功能模式输出不得误置 scan enable；不同分支的锁存/寄存延迟必须匹配扫描时序约束。 |
| REQ-003 | 时序/接受 | 测试路径只有在测试使能与安全切换条件满足时生效；功能模式不得形成非预期写入或时钟/复位覆盖。 |
| REQ-004 | 复位/重启 | reset 后测试控制默认禁止；状态和种子初始化可重放，功能/测试交接需要空闲或复位窗口。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：功能直通、进入/退出测试、非法模式、测试暂停、种子重载、扫描相位/端口冲突及签名比较。 |
