...
registry_id: DFT-009
name: test_access_mux
version: 0.1.0
status: draft
basis: planning_intent
---

# test_access_mux 需求合同
...
| 索引 ID | DFT-009 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dft_test / components/dft_test/test_access_mux |
| 功能家族 | Test Access Mux |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：在受控测试模式下选择测试访问路径，功能模式保持功能路径语义。

边界焦点：切换仅在两侧无未完成局部访问时提交；未选通端不得产生写副作用，不承担 JTAG 控制器功能。
...
候选实现：scan/JTAG/internal bus。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在受控测试模式下选择测试访问路径，功能模式保持功能路径语义。 |
| REQ-002 | 关键边界 | 切换仅在两侧无未完成局部访问时提交；未选通端不得产生写副作用，不承担 JTAG 控制器功能。 |
