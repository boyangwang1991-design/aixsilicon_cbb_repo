...
registry_id: DFT-004
name: reset_ctrl_test_override
version: 0.1.0
status: draft
basis: planning_intent
---

# reset_ctrl_test_override 需求合同
...
| 索引 ID | DFT-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dft_test / components/dft_test/reset_ctrl_test_override |
| 功能家族 | Reset-control Test Override |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在测试窗口内按显式控制旁路或强制局部复位，功能模式恢复原复位语义。

边界焦点：未经测试使能不得覆盖功能复位；测试控制撤销与复位释放顺序需要 RDC 约束，不能异步毛刺释放。
...
候选实现：controlled bypass/force。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在测试窗口内按显式控制旁路或强制局部复位，功能模式恢复原复位语义。 |
| REQ-002 | 关键边界 | 未经测试使能不得覆盖功能复位；测试控制撤销与复位释放顺序需要 RDC 约束，不能异步毛刺释放。 |
