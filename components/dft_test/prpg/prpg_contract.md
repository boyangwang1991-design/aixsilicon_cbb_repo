...
| 索引 ID | DFT-007 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dft_test / components/dft_test/prpg |
| 功能家族 | PRPG |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：从非锁死种子生成确定性伪随机测试序列，可经固定相位变换形成多路输出。

边界焦点：暂停必须保持状态；重装种子与推进同拍时重装优先，记录序列长度和可复现的初始条件。
...
候选实现：LFSR/phase shifter。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 从非锁死种子生成确定性伪随机测试序列，可经固定相位变换形成多路输出。 |
| REQ-002 | 关键边界 | 暂停必须保持状态；重装种子与推进同拍时重装优先，记录序列长度和可复现的初始条件。 |
