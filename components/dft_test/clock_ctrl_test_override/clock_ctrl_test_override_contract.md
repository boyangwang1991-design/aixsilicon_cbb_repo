...
registry_id: DFT-003
name: clock_ctrl_test_override
version: 0.1.0
status: draft
basis: planning_intent
---

# clock_ctrl_test_override 需求合同
...
| 索引 ID | DFT-003 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dft_test / components/dft_test/clock_ctrl_test_override |
| 功能家族 | Clock-control Test Override |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在授权测试模式下覆盖本地 ICG/时钟控制以保证测试时钟可达。

边界焦点：测试覆盖必须经过安全控制窗口；离开测试模式时恢复功能控制，不允许组合切换产生窄脉冲。
...
候选实现：ICG/mux bypass。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在授权测试模式下覆盖本地 ICG/时钟控制以保证测试时钟可达。 |
| REQ-002 | 关键边界 | 测试覆盖必须经过安全控制窗口；离开测试模式时恢复功能控制，不允许组合切换产生窄脉冲。 |
