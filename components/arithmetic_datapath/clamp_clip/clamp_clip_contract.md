...
| 索引 ID | ARI-010 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/clamp_clip |
| 功能家族 | Clamp/Clip |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：按上下限夹取输入，输出 max(lower,min(input,upper))，并指出是否发生裁剪。

边界焦点：lower 大于 upper 为非法配置；比较与输出采用一致符号域，等于边界不算裁剪。
...
候选实现：symmetric/asymmetric limits。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按上下限夹取输入，输出 max(lower,min(input,upper))，并指出是否发生裁剪。 |
| REQ-002 | 关键边界 | lower 大于 upper 为非法配置；比较与输出采用一致符号域，等于边界不算裁剪。 |
