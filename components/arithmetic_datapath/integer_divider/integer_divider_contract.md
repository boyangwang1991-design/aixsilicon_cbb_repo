...
registry_id: ARI-021
name: integer_divider
version: 0.1.0
status: draft
basis: planning_intent
---

# integer_divider 需求合同
...
| 索引 ID | ARI-021 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/integer_divider |
| 功能家族 | Integer Divider |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：输出商与余数，非零除数下满足 A=Q×B+R，余数绝对值小于除数绝对值。

边界焦点：有符号商向零截断、余数跟随被除数符号；除零和最小负数除负一必须输出错误标志。
...
候选实现：restoring、non-restoring、SRT。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 输出商与余数，非零除数下满足 A=Q×B+R，余数绝对值小于除数绝对值。 |
| REQ-002 | 关键边界 | 有符号商向零截断、余数跟随被除数符号；除零和最小负数除负一必须输出错误标志。 |
