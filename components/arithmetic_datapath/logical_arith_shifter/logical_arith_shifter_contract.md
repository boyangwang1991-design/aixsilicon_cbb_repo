...
registry_id: ARI-015
name: logical_arith_shifter
version: 0.1.0
status: draft
basis: planning_intent
---

# logical_arith_shifter 需求合同
...
| 索引 ID | ARI-015 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/logical_arith_shifter |
| 功能家族 | Logical/Arithmetic Shifter |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：支持逻辑左移、逻辑右移和算术右移，算术右移复制原符号位。

边界焦点：移位量为零保持输入；移位量大于等于位宽时逻辑结果为零、算术右移为符号填充。
...
候选实现：staged、barrel、iterative。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 支持逻辑左移、逻辑右移和算术右移，算术右移复制原符号位。 |
| REQ-002 | 关键边界 | 移位量为零保持输入；移位量大于等于位宽时逻辑结果为零、算术右移为符号填充。 |
