...
| 索引 ID | ARI-002 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/adder_subtractor |
| 功能家族 | Adder/Subtractor |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：按操作选择计算 A+B 或 A-B，支持显式有符号/无符号解释并输出进位或借位。

边界焦点：输出默认按目标位宽截断；有符号溢出与无符号进位独立报告，不能用最高位替代溢出判断。
...
候选实现：ripple、CLA、prefix、segmented。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按操作选择计算 A+B 或 A-B，支持显式有符号/无符号解释并输出进位或借位。 |
| REQ-002 | 关键边界 | 输出默认按目标位宽截断；有符号溢出与无符号进位独立报告，不能用最高位替代溢出判断。 |
