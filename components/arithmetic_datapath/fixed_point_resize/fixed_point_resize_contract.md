...
registry_id: ARI-013
name: fixed_point_resize
version: 0.1.0
status: draft
basis: planning_intent
---

# fixed_point_resize 需求合同
...
| 索引 ID | ARI-013 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/fixed_point_resize |
| 功能家族 | Fixed-point Resize |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：将输入定点格式转换为另一整数位/小数位组合，按配置执行扩展、舍入及饱和。

边界焦点：先完成数值缩放和舍入，再判断输出域溢出；符号扩展不得改变原数值。
...
候选实现：extend、round、saturate。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 将输入定点格式转换为另一整数位/小数位组合，按配置执行扩展、舍入及饱和。 |
| REQ-002 | 关键边界 | 先完成数值缩放和舍入，再判断输出域溢出；符号扩展不得改变原数值。 |
