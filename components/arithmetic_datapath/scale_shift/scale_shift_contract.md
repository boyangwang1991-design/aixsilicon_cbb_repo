...
| 索引 ID | ARI-014 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/scale_shift |
| 功能家族 | Scale/Shift |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：按配置二进制指数对输入左移或右移以实现 2 的整数次幂缩放。

边界焦点：移位量超出数据宽度时使用定义的零填充或符号填充；丢位/溢出必须可观察，不能取模移位。
...
候选实现：power-of-two、programmable。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按配置二进制指数对输入左移或右移以实现 2 的整数次幂缩放。 |
| REQ-002 | 关键边界 | 移位量超出数据宽度时使用定义的零填充或符号填充；丢位/溢出必须可观察，不能取模移位。 |
