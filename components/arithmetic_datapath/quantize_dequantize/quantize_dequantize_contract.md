...
| 索引 ID | ARI-034 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/quantize_dequantize |
| 功能家族 | Quantize/Dequantize |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：按 scale 和 zero-point 执行仿射量化或反量化，量化结果在目标整数域内饱和。

边界焦点：scale 必须为正；逐通道系数与通道索引对齐，舍入在加 zero-point 前完成。
...
候选实现：affine、symmetric、per-channel。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按 scale 和 zero-point 执行仿射量化或反量化，量化结果在目标整数域内饱和。 |
| REQ-002 | 关键边界 | scale 必须为正；逐通道系数与通道索引对齐，舍入在加 zero-point 前完成。 |
