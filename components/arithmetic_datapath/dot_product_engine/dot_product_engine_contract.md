...
registry_id: ARI-020
name: dot_product_engine
version: 0.1.0
status: draft
basis: planning_intent
---

# dot_product_engine 需求合同
...
| 索引 ID | ARI-020 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/dot_product_engine |
| 功能家族 | Dot-product Engine |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：接收指定长度的向量对，计算各元素乘积之和并在向量完成后输出一个结果。

边界焦点：折叠实现需锁定输入长度与符号格式，最后一个元素只能累计一次；取消当前向量时不得输出残值。
...
候选实现：parallel、time-mux、tree。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 接收指定长度的向量对，计算各元素乘积之和并在向量完成后输出一个结果。 |
| REQ-002 | 关键边界 | 折叠实现需锁定输入长度与符号格式，最后一个元素只能累计一次；取消当前向量时不得输出残值。 |
