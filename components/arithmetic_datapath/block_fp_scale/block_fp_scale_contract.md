...
| 索引 ID | ARI-033 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/block_fp_scale |
| 功能家族 | Block Floating-point Scale |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：为一个数据块选取公共指数，对各元素同步缩放并保留块边界与有效掩码。

边界焦点：指数根据有效元素计算，全零块使用约定零指数；指数及尾数必须一起输出，不能跨块串用。
...
候选实现：shared exponent、normalize。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 为一个数据块选取公共指数，对各元素同步缩放并保留块边界与有效掩码。 |
| REQ-002 | 关键边界 | 指数根据有效元素计算，全零块使用约定零指数；指数及尾数必须一起输出，不能跨块串用。 |
