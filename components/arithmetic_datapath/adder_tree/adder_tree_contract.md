...
| 索引 ID | ARI-005 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/adder_tree |
| 功能家族 | Adder Tree |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：用平衡归约树计算输入向量之和，流水各级保持同一输入样本的对齐。

边界焦点：非二次幂路数以零补齐；可选饱和发生于显式定义的最终输出，不能在中间节点隐式截断。
...
候选实现：balanced、registered、saturating。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 用平衡归约树计算输入向量之和，流水各级保持同一输入样本的对齐。 |
| REQ-002 | 关键边界 | 非二次幂路数以零补齐；可选饱和发生于显式定义的最终输出，不能在中间节点隐式截断。 |
