...
registry_id: ARI-026
name: reciprocal_rsqrt_approx
version: 0.1.0
status: draft
basis: planning_intent
---

# reciprocal_rsqrt_approx 需求合同
...
| 索引 ID | ARI-026 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/reciprocal_rsqrt_approx |
| 功能家族 | Reciprocal/RSqrt Approximation |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：分别近似计算正定点输入的倒数或平方根倒数，返回结果及输入域异常标志。

边界焦点：零输入及负数输入分别拒绝；每个支持格式必须给出误差上界，验收覆盖分段边界和最小正输入。
...
候选实现：LUT+iteration、piecewise。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 分别近似计算正定点输入的倒数或平方根倒数，返回结果及输入域异常标志。 |
| REQ-002 | 关键边界 | 零输入及负数输入分别拒绝；每个支持格式必须给出误差上界，验收覆盖分段边界和最小正输入。 |
