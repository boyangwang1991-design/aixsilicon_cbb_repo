...
registry_id: ARI-011
name: saturating_add_sub
version: 0.1.0
status: draft
basis: planning_intent
---

# saturating_add_sub 需求合同
...
| 索引 ID | ARI-011 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/saturating_add_sub |
| 功能家族 | Saturating Add/Sub |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：执行加减并将精确结果限制在目标数值域，输出饱和结果和饱和标志。

边界焦点：有符号正负溢出分别夹到最大/最小值；无符号减法下溢夹零，不能回绕到大正数。
...
候选实现：signed/unsigned。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 执行加减并将精确结果限制在目标数值域，输出饱和结果和饱和标志。 |
| REQ-002 | 关键边界 | 有符号正负溢出分别夹到最大/最小值；无符号减法下溢夹零，不能回绕到大正数。 |
