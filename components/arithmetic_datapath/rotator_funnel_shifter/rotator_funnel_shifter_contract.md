...
registry_id: ARI-016
name: rotator_funnel_shifter
version: 0.1.0
status: draft
basis: planning_intent
---

# rotator_funnel_shifter 需求合同
...
| 索引 ID | ARI-016 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/rotator_funnel_shifter |
| 功能家族 | Rotator/Funnel Shifter |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：循环移位按数据位宽取模；漏斗移位从两个输入拼接形成的位向量中截取连续窗口。

边界焦点：旋转与漏斗模式分别定义方向；零位移、整字位移及跨输入边界必须与位级参考一致。
...
候选实现：barrel、staged。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 循环移位按数据位宽取模；漏斗移位从两个输入拼接形成的位向量中截取连续窗口。 |
| REQ-002 | 关键边界 | 旋转与漏斗模式分别定义方向；零位移、整字位移及跨输入边界必须与位级参考一致。 |
