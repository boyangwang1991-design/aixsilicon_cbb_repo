...
| 索引 ID | ARI-023 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/modulo_reducer |
| 功能家族 | Modulo/Reducer |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：对非负输入计算相对于正模数 M 的余数，结果必须满足 0≤R<M。

边界焦点：M=1 输出零；非二次幂模数不能简单截位，Barrett 等实现需校正到规范余数域。
...
候选实现：arbitrary、power-of-two、Barrett。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对非负输入计算相对于正模数 M 的余数，结果必须满足 0≤R<M。 |
| REQ-002 | 关键边界 | M=1 输出零；非二次幂模数不能简单截位，Barrett 等实现需校正到规范余数域。 |
