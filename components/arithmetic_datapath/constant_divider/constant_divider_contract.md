...
registry_id: ARI-022
name: constant_divider
version: 0.1.0
status: draft
basis: planning_intent
---

# constant_divider 需求合同
...
| 索引 ID | ARI-022 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/constant_divider |
| 功能家族 | Constant Divider |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：对固定非零整数除数输出精确商与余数，优化实现须与整数除法逐值等价。

边界焦点：倒数乘法近似必须包含必要修正步骤；除数零在配置检查时拒绝，不以误差容忍代替精确整数语义。
...
候选实现：reciprocal multiply、shift-add。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对固定非零整数除数输出精确商与余数，优化实现须与整数除法逐值等价。 |
| REQ-002 | 关键边界 | 倒数乘法近似必须包含必要修正步骤；除数零在配置检查时拒绝，不以误差容忍代替精确整数语义。 |
