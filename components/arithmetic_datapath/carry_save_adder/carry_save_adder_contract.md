...
| 索引 ID | ARI-003 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/carry_save_adder |
| 功能家族 | Carry-save Adder |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：把三个同权输入压缩成 sum 与 carry，使 sum 加左移一位的 carry 等于完整输入和。

边界焦点：必须保留最高进位，接口明确 carry 是否已移位；4:2 扩展模式不得丢弃输入/输出进位链。
...
候选实现：3:2、4:2 compressor。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把三个同权输入压缩成 sum 与 carry，使 sum 加左移一位的 carry 等于完整输入和。 |
| REQ-002 | 关键边界 | 必须保留最高进位，接口明确 carry 是否已移位；4:2 扩展模式不得丢弃输入/输出进位链。 |
