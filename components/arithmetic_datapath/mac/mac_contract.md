...
| 索引 ID | ARI-019 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/mac |
| 功能家族 | Multiply-Accumulate |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：计算 A×B+C，默认使用完整乘积和足够宽的累加结果，仅在输出端执行配置的截断。

边界焦点：输入操作数和累加项必须属于同一事务；融合与分离实现不能隐式改变舍入位置。
...
候选实现：fused、separate、pipelined。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 计算 A×B+C，默认使用完整乘积和足够宽的累加结果，仅在输出端执行配置的截断。 |
| REQ-002 | 关键边界 | 输入操作数和累加项必须属于同一事务；融合与分离实现不能隐式改变舍入位置。 |
