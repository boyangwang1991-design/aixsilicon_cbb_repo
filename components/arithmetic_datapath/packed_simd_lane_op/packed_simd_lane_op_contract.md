...
registry_id: ARI-035
name: packed_simd_lane_op
version: 0.1.0
status: draft
basis: planning_intent
---

# packed_simd_lane_op 需求合同
...
| 索引 ID | ARI-035 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/packed_simd_lane_op |
| 功能家族 | Packed SIMD Lane Operator |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：将数据分成独立 lane 执行配置的算术/比较操作，lane 掩码控制哪些结果有效。

边界焦点：进位不能穿越 lane 边界；被屏蔽 lane 按定义保持或置零，不能参与跨 lane 状态更新。
...
候选实现：add/mul/min/max、lane mask。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 将数据分成独立 lane 执行配置的算术/比较操作，lane 掩码控制哪些结果有效。 |
| REQ-002 | 关键边界 | 进位不能穿越 lane 边界；被屏蔽 lane 按定义保持或置零，不能参与跨 lane 状态更新。 |
