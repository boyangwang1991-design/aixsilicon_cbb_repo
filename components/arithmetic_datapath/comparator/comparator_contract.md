...
| 索引 ID | ARI-008 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/comparator |
| 功能家族 | Comparator |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：对相同位宽的两个输入输出小于、等于和大于结果，有符号性由静态配置决定。

边界焦点：三个关系在已知输入上互斥且完备；分段实现必须先处理高有效位，不能跨段错误合并。
...
候选实现：signed/unsigned、segmented。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对相同位宽的两个输入输出小于、等于和大于结果，有符号性由静态配置决定。 |
| REQ-002 | 关键边界 | 三个关系在已知输入上互斥且完备；分段实现必须先处理高有效位，不能跨段错误合并。 |
