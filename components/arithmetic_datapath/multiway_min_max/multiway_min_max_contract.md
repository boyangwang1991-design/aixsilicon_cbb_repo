...
registry_id: ARI-009
name: multiway_min_max
version: 0.1.0
status: draft
basis: planning_intent
---

# multiway_min_max 需求合同
...
| 索引 ID | ARI-009 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/multiway_min_max |
| 功能家族 | Multi-way Min/Max |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在有效输入集合中选择最小或最大数，同时返回原输入索引。

边界焦点：相等值选最小索引；无有效输入时 valid 为零，索引和数据不作为有效结果消费。
...
候选实现：linear、balanced、pipelined。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在有效输入集合中选择最小或最大数，同时返回原输入索引。 |
| REQ-002 | 关键边界 | 相等值选最小索引；无有效输入时 valid 为零，索引和数据不作为有效结果消费。 |
