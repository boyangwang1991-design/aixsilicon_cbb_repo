...
| 索引 ID | ARI-027 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/cordic |
| 功能家族 | CORDIC |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：按配置执行旋转或向量模式迭代，输出坐标和角度，并显式处理迭代增益。

边界焦点：输入角度先按支持收敛域约束或进行象限预处理；迭代次数、定点格式与误差界共同构成配置。
...
候选实现：iterative、unrolled、pipelined。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按配置执行旋转或向量模式迭代，输出坐标和角度，并显式处理迭代增益。 |
| REQ-002 | 关键边界 | 输入角度先按支持收敛域约束或进行象限预处理；迭代次数、定点格式与误差界共同构成配置。 |
