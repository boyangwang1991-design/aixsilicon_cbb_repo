...
registry_id: ARI-007
name: absolute_value_negate
version: 0.1.0
status: draft
basis: planning_intent
---

# absolute_value_negate 需求合同
...
| 索引 ID | ARI-007 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/absolute_value_negate |
| 功能家族 | Absolute Value/Negate |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：按模式输出二补码取负或绝对值，并报告无法以原有符号位宽表示的结果。

边界焦点：最小负数的绝对值可选扩位精确输出或饱和为最大正数；禁止无指示地仍输出负值。
...
候选实现：signed、saturating。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按模式输出二补码取负或绝对值，并报告无法以原有符号位宽表示的结果。 |
| REQ-002 | 关键边界 | 最小负数的绝对值可选扩位精确输出或饱和为最大正数；禁止无指示地仍输出负值。 |
