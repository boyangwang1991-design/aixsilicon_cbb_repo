...
registry_id: ARI-025
name: average_weighted_sum
version: 0.1.0
status: draft
basis: planning_intent
---

# average_weighted_sum 需求合同
...
| 索引 ID | ARI-025 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/average_weighted_sum |
| 功能家族 | Average/Weighted Sum |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：计算输入样本的加权和，并可按显式非零归一化系数生成平均值。

边界焦点：中间累加使用足够位宽；分母为零拒绝，输出舍入模式明确，禁止隐式假设权重和为二次幂。
...
候选实现：shift、reciprocal、MAC。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 计算输入样本的加权和，并可按显式非零归一化系数生成平均值。 |
| REQ-002 | 关键边界 | 中间累加使用足够位宽；分母为零拒绝，输出舍入模式明确，禁止隐式假设权重和为二次幂。 |
