...
| 索引 ID | ARI-018 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/constant_multiplier |
| 功能家族 | Constant Multiplier |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：对输入乘以编译期常量，移位加减实现必须与完整整数乘法等价。

边界焦点：常量零、一、负一和非二次幂均要覆盖；负常量的符号扩展及结果增长不得被优化掉。
...
候选实现：shift-add、CSD、MCM。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对输入乘以编译期常量，移位加减实现必须与完整整数乘法等价。 |
| REQ-002 | 关键边界 | 常量零、一、负一和非二次幂均要覆盖；负常量的符号扩展及结果增长不得被优化掉。 |
