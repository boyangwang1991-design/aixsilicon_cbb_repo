...
registry_id: ARI-004
name: multi_operand_adder
version: 0.1.0
status: draft
basis: planning_intent
---

# multi_operand_adder 需求合同
...
| 索引 ID | ARI-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/multi_operand_adder |
| 功能家族 | Multi-operand Adder |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：对 N 个输入求和，默认输出宽度至少为输入宽度加 ceil(log2(N))，避免静默溢出。

边界焦点：有符号操作数逐个符号扩展；N=1 为等价直通，多余树叶填算术零。
...
候选实现：linear、balanced、CSA tree。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对 N 个输入求和，默认输出宽度至少为输入宽度加 ceil(log2(N))，避免静默溢出。 |
| REQ-002 | 关键边界 | 有符号操作数逐个符号扩展；N=1 为等价直通，多余树叶填算术零。 |
