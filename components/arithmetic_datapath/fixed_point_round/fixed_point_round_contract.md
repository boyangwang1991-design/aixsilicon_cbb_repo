...
registry_id: ARI-012
name: fixed_point_round
version: 0.1.0
status: draft
basis: planning_intent
---

# fixed_point_round 需求合同
...
| 索引 ID | ARI-012 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/fixed_point_round |
| 功能家族 | Fixed-point Round |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：丢弃指定低位时按选择的舍入模式修正保留部分，并输出舍入进位。

边界焦点：RNE 的恰好半值按保留最低位奇偶处理；随机舍入必须使用显式外部随机输入，不能隐含随机源。
...
候选实现：truncate、RNE、RNA、stochastic。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 丢弃指定低位时按选择的舍入模式修正保留部分，并输出舍入进位。 |
| REQ-002 | 关键边界 | RNE 的恰好半值按保留最低位奇偶处理；随机舍入必须使用显式外部随机输入，不能隐含随机源。 |
