...
| 索引 ID | ARI-017 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arithmetic_datapath / components/arithmetic_datapath/integer_multiplier |
| 功能家族 | Integer Multiplier |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：计算两个整数的乘积，默认保留两输入位宽之和的完整乘积。

边界焦点：有符号乘法先按符号域解释，最小负数乘最小负数也必须精确；截短输出只能由显式配置启用。
...
候选实现：array、Booth、Wallace/Dadda。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 计算两个整数的乘积，默认保留两输入位宽之和的完整乘积。 |
| REQ-002 | 关键边界 | 有符号乘法先按符号域解释，最小负数乘最小负数也必须精确；截短输出只能由显式配置启用。 |
