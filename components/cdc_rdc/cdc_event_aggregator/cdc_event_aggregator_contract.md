...
registry_id: CDC-012
name: cdc_event_aggregator
version: 0.1.0
status: draft
basis: planning_intent
---

# cdc_event_aggregator 需求合同
...
| 索引 ID | CDC-012 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/cdc_rdc / components/cdc_rdc/cdc_event_aggregator |
| 功能家族 | Clock-domain Event Aggregator |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：每个源域事件独立经过适用的同步机制，再在汇聚域形成事件位图或计数增量。

边界焦点：同拍多个源事件不可因简单优先选择而静默丢失；各源最小间隔或握手约束必须分别声明。
...
候选实现：per-source sync + collect。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 每个源域事件独立经过适用的同步机制，再在汇聚域形成事件位图或计数增量。 |
| REQ-002 | 关键边界 | 同拍多个源事件不可因简单优先选择而静默丢失；各源最小间隔或握手约束必须分别声明。 |
