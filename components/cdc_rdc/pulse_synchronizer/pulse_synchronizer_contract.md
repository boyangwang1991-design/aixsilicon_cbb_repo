...
registry_id: CDC-003
name: pulse_synchronizer
version: 0.1.0
status: draft
basis: planning_intent
---

# pulse_synchronizer 需求合同
...
| 索引 ID | CDC-003 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/cdc_rdc / components/cdc_rdc/pulse_synchronizer |
| 功能家族 | Pulse Synchronizer |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：将源域被接受的单次脉冲转换为目的域一个周期的事件脉冲。

边界焦点：无握手模式必须满足最小事件间隔；带 ack 模式 busy 时拒绝新事件并显式反馈，不得静默合并。
...
候选实现：toggle、stretch、acknowledged。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 将源域被接受的单次脉冲转换为目的域一个周期的事件脉冲。 |
| REQ-002 | 关键边界 | 无握手模式必须满足最小事件间隔；带 ack 模式 busy 时拒绝新事件并显式反馈，不得静默合并。 |
