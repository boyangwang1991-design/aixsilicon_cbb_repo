...
registry_id: CDC-001
name: single_bit_synchronizer
version: 0.1.0
status: draft
basis: planning_intent
---

# single_bit_synchronizer 需求合同
...
| 索引 ID | CDC-001 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/cdc_rdc / components/cdc_rdc/single_bit_synchronizer |
| 功能家族 | Single-bit Synchronizer |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：把单比特电平经过至少两级目的时钟采样同步到目的域，功能输出只取同步链末级。

边界焦点：短于目的采样窗口的脉冲不保证捕获；首级输出不得被功能逻辑使用，同步属性与布局约束必须随交付提供。
...
候选实现：2/3-stage、hardened cell。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把单比特电平经过至少两级目的时钟采样同步到目的域，功能输出只取同步链末级。 |
| REQ-002 | 关键边界 | 短于目的采样窗口的脉冲不保证捕获；首级输出不得被功能逻辑使用，同步属性与布局约束必须随交付提供。 |
