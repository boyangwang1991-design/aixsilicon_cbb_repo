...
registry_id: RDC-006
name: warm_cold_reset_ctrl
version: 0.1.0
status: draft
basis: planning_intent
---

# warm_cold_reset_ctrl 需求合同
...
| 索引 ID | RDC-006 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/cdc_rdc / components/cdc_rdc/warm_cold_reset_ctrl |
| 功能家族 | Warm/Cold Reset Controller |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：根据冷/暖复位请求对配置的局部复位域输出控制，暖复位保留指定状态域。

边界焦点：冷复位优先于暖复位；保持域与清除域的跨域接口需隔离，本核不拥有产品级复位原因策略。
...
候选实现：cause/filter/distribution。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 根据冷/暖复位请求对配置的局部复位域输出控制，暖复位保留指定状态域。 |
| REQ-002 | 关键边界 | 冷复位优先于暖复位；保持域与清除域的跨域接口需隔离，本核不拥有产品级复位原因策略。 |
