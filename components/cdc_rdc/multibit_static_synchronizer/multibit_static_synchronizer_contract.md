...
registry_id: CDC-002
name: multibit_static_synchronizer
version: 0.1.0
status: draft
basis: planning_intent
---

# multibit_static_synchronizer 需求合同
...
| 索引 ID | CDC-002 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/cdc_rdc / components/cdc_rdc/multibit_static_synchronizer |
| 功能家族 | Multi-bit Static Synchronizer |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：逐位同步低变化率配置总线，仅在源数据满足稳定窗口时允许目的端使用同步结果。

边界焦点：不承诺动态多比特数据原子性；源侧必须在消费期间保持整组配置不变，动态更新需改用握手 CDC。

拟复用场景：异步配置/事件交互；局部数据通路和复位域衔接。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：源域控制/数据、源/目的时钟复位输入；同步数据/事件、busy/ack 或域就绪输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：同步级数≥2（需同步链时）；数据宽度≥1；缓存深度和源事件间隔满足所选跨域机制；时钟关系为显式假设。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：per-bit + stability contract。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 逐位同步低变化率配置总线，仅在源数据满足稳定窗口时允许目的端使用同步结果。 |
| REQ-002 | 关键边界 | 不承诺动态多比特数据原子性；源侧必须在消费期间保持整组配置不变，动态更新需改用握手 CDC。 |
| REQ-003 | 时序/接受 | 连续同步核按目的域可见值定义采样语义；握手核另行定义源接受和确认。延迟随相位/同步级数变化，不以固定周期描述异步延迟。 |
| REQ-004 | 复位/重启 | 分别覆盖源先复位、目的先复位、同时复位和单域恢复；不允许释放时产生伪事件或将旧数据当新事务。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：快慢时钟互换、相位扫描、长停钟、事件间隔边界、单域复位、中途复位和 CDC/RDC 结构检查。 |
