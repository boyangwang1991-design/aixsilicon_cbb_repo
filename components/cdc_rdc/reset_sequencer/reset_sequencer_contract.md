...
| 索引 ID | RDC-005 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/cdc_rdc / components/cdc_rdc/reset_sequencer |
| 功能家族 | Reset Sequencer |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：按显式无环依赖关系逐个释放局部复位，并等待依赖就绪或超时反馈。

边界焦点：配置中的依赖环拒绝；超时保持后继复位且报告，不实现软件复位管理或整个 SoC 启动策略。
...
候选实现：dependency DAG、timeout。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按显式无环依赖关系逐个释放局部复位，并等待依赖就绪或超时反馈。 |
| REQ-002 | 关键边界 | 配置中的依赖环拒绝；超时保持后继复位且报告，不实现软件复位管理或整个 SoC 启动策略。 |
