...
registry_id: CDC-005
name: handshake_synchronizer
version: 0.1.0
status: draft
basis: planning_intent
---

# handshake_synchronizer 需求合同
...
| 索引 ID | CDC-005 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/cdc_rdc / components/cdc_rdc/handshake_synchronizer |
| 功能家族 | Handshake Synchronizer |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：通过两相或四相 req/ack 协议将事件从源域可靠送达目的域并返回完成。

边界焦点：前一握手完成前不接受下一事件；任一域复位时按明确的中止策略清除半完成握手，不能凭空完成。
...
候选实现：2-phase、4-phase。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 通过两相或四相 req/ack 协议将事件从源域可靠送达目的域并返回完成。 |
| REQ-002 | 关键边界 | 前一握手完成前不接受下一事件；任一域复位时按明确的中止策略清除半完成握手，不能凭空完成。 |
