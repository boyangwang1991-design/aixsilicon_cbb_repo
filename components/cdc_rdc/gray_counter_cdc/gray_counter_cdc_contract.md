...
registry_id: CDC-008
name: gray_counter_cdc
version: 0.1.0
status: draft
basis: planning_intent
---

# gray_counter_cdc 需求合同
...
| 索引 ID | CDC-008 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/cdc_rdc / components/cdc_rdc/gray_counter_cdc |
| 功能家族 | Gray Counter CDC |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：源域计数值先编码为 Gray，再同步到目的域并可解码为二进制观测值。

边界焦点：源计数每次仅允许增加一；目的观察可跳过中间值但不能承诺精确事件计数，需提供 Gray 总线时序约束。
...
候选实现：binary-gray-sync-decode。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 源域计数值先编码为 Gray，再同步到目的域并可解码为二进制观测值。 |
| REQ-002 | 关键边界 | 源计数每次仅允许增加一；目的观察可跳过中间值但不能承诺精确事件计数，需提供 Gray 总线时序约束。 |
