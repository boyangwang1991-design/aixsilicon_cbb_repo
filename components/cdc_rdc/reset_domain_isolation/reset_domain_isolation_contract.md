...
registry_id: RDC-004
name: reset_domain_isolation
version: 0.1.0
status: draft
basis: planning_intent
---

# reset_domain_isolation 需求合同
...
| 索引 ID | RDC-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/cdc_rdc / components/cdc_rdc/reset_domain_isolation |
| 功能家族 | Reset Domain Isolation |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在源域复位或不可用期间对跨边界数据/valid 夹持，并在恢复确认后解除。

边界焦点：夹持 valid=0 防止产生伪事务；解除须等待同步与状态重建，不能只看异步复位引脚变高。
...
候选实现：clamp/handshake。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在源域复位或不可用期间对跨边界数据/valid 夹持，并在恢复确认后解除。 |
| REQ-002 | 关键边界 | 夹持 valid=0 防止产生伪事务；解除须等待同步与状态重建，不能只看异步复位引脚变高。 |
