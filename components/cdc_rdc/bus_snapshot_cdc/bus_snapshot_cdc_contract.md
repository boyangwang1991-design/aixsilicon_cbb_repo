...
registry_id: CDC-007
name: bus_snapshot_cdc
version: 0.1.0
status: draft
basis: planning_intent
---

# bus_snapshot_cdc 需求合同
...
| 索引 ID | CDC-007 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/cdc_rdc / components/cdc_rdc/bus_snapshot_cdc |
| 功能家族 | Bus Snapshot CDC |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：收到快照请求后在源域同一采样沿锁存整组信号，再把该快照作为一个原子结果送往目的域。

边界焦点：快照期间的新变化不影响已锁存结果；重复请求按 busy 反压，不拼接来自不同采样时刻的位。
...
候选实现：shadow/latch/snapshot。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 收到快照请求后在源域同一采样沿锁存整组信号，再把该快照作为一个原子结果送往目的域。 |
| REQ-002 | 关键边界 | 快照期间的新变化不影响已锁存结果；重复请求按 busy 反压，不拼接来自不同采样时刻的位。 |
