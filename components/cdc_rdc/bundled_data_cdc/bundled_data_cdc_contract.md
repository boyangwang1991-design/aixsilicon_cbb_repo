...
registry_id: CDC-006
name: bundled_data_cdc
version: 0.1.0
status: draft
basis: planning_intent
---

# bundled_data_cdc 需求合同
...
| 索引 ID | CDC-006 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/cdc_rdc / components/cdc_rdc/bundled_data_cdc |
| 功能家族 | Bundled-data CDC |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在源域锁存数据并保持到 ack 返回，目的域仅在同步后的请求有效时采样整组数据。

边界焦点：数据路径稳定窗口必须覆盖控制同步和目的采样；接口不允许发送者在 busy 期间改写待传数据。
...
候选实现：req/ack + stable data。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在源域锁存数据并保持到 ack 返回，目的域仅在同步后的请求有效时采样整组数据。 |
| REQ-002 | 关键边界 | 数据路径稳定窗口必须覆盖控制同步和目的采样；接口不允许发送者在 busy 期间改写待传数据。 |
