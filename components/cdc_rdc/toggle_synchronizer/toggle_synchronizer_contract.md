...
registry_id: CDC-004
name: toggle_synchronizer
version: 0.1.0
status: draft
basis: planning_intent
---

# toggle_synchronizer 需求合同
...
| 索引 ID | CDC-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/cdc_rdc / components/cdc_rdc/toggle_synchronizer |
| 功能家族 | Toggle Synchronizer |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：源域每次接受事件翻转 toggle，目的域检测同步 toggle 的变化并输出单周期事件。

边界焦点：两次源翻转不能落在同一次目的采样间隔内；双域复位需对齐初值，禁止复位释放形成伪事件。
...
候选实现：event toggle、counter extension。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 源域每次接受事件翻转 toggle，目的域检测同步 toggle 的变化并输出单周期事件。 |
| REQ-002 | 关键边界 | 两次源翻转不能落在同一次目的采样间隔内；双域复位需对齐初值，禁止复位释放形成伪事件。 |
