...
| 索引 ID | ARB-011 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arbitration_scheduling / components/arbitration_scheduling/credit_manager |
| 功能家族 | Credit Manager |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：按通道维护可用 credit，发送扣减、归还增加，可用量始终位于零与配置容量之间。

边界焦点：同拍发送与归还按净变化计算；不足 credit 时禁止发送授权，重复或超额归还产生局部错误指示。
...
候选实现：per-VC/shared pool。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按通道维护可用 credit，发送扣减、归还增加，可用量始终位于零与配置容量之间。 |
| REQ-002 | 关键边界 | 同拍发送与归还按净变化计算；不足 credit 时禁止发送授权，重复或超额归还产生局部错误指示。 |
