...
registry_id: ARB-020
name: barrier_join_controller
version: 0.1.0
status: draft
basis: planning_intent
---

# barrier_join_controller 需求合同
...
| 索引 ID | ARB-020 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arbitration_scheduling / components/arbitration_scheduling/barrier_join_controller |
| 功能家族 | Barrier/Join Controller |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：在一个 generation 内收集所有已启用参与者的到达事件，全部到齐后只发出一次完成。

边界焦点：重复到达不增加计数；迟到的旧 generation 事件被拒绝，开始新轮前明确清除旧到达集合。
...
候选实现：count/bitmap/generation。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在一个 generation 内收集所有已启用参与者的到达事件，全部到齐后只发出一次完成。 |
| REQ-002 | 关键边界 | 重复到达不增加计数；迟到的旧 generation 事件被拒绝，开始新轮前明确清除旧到达集合。 |
