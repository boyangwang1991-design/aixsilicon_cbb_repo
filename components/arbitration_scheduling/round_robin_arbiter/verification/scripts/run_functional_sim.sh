...
TB="$P/verification/simulation/rra_tb.sv"

# ---- 单仿真多场景（穷举 + 轮转序 + 边界 + 随机互斥/等价 + 锁存 + 寄存 + ack 锁定）----
( cd "$WORK" && vcs -full64 -timescale=1ns/1ps -sverilog -assert enable_diag $RTL $TB \
    -o /tmp/rra_g4 > "$EV/tmp_compile.log" 2>&1 )
# -assert disable_cover 关闭覆盖 info；-suppress 抑制 VCS 提示类信息，保持证据干净
/tmp/rra_g4 -assert disable_cover -suppress=all > "$EV/functional_sim.txt" 2>&1
grep -q "RRA_TB PASS" "$EV/functional_sim.txt" || {
    echo "functional sim FAILED"; cat "$EV/functional_sim.txt"; exit 1; }
echo "[sim] RRA_TB PASS (exhaust_w4 + rr_order + edge + random2000×N64 + equiv + latched + registered + ack_lock + mutation)"
