...
registry_id: ARB-015
name: request_distributor
version: 0.1.0
status: draft
basis: planning_intent
---

# request_distributor 需求合同
...
| 索引 ID | ARB-015 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arbitration_scheduling / components/arbitration_scheduling/request_distributor |
| 功能家族 | Request Distributor |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：把每个已接收请求分配给一个可服务目的端，并携带原请求身份。

边界焦点：轮转、哈希或负载感知模式分别配置；被选端不 ready 时不得丢弃请求或重复分发到另一端。
...
候选实现：RR/hash/load-aware。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把每个已接收请求分配给一个可服务目的端，并携带原请求身份。 |
| REQ-002 | 关键边界 | 轮转、哈希或负载感知模式分别配置；被选端不 ready 时不得丢弃请求或重复分发到另一端。 |
