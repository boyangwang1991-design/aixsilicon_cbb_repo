...
| 索引 ID | ARB-012 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arbitration_scheduling / components/arbitration_scheduling/token_allocator |
| 功能家族 | Token Allocator |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：从空闲 token 集合分配唯一 token，并在合法回收后重新置为空闲。

边界焦点：满池回收重复 token、越界 token 均须拒绝并告警；同拍分配回收默认不复用刚回收 token。
...
候选实现：bitmap/free-list/tree。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 从空闲 token 集合分配唯一 token，并在合法回收后重新置为空闲。 |
| REQ-002 | 关键边界 | 满池回收重复 token、越界 token 均须拒绝并告警；同拍分配回收默认不复用刚回收 token。 |
