...
registry_id: ARB-009
name: pipelined_arbiter
version: 0.1.0
status: draft
basis: planning_intent
---

# pipelined_arbiter 需求合同
...
| 索引 ID | ARB-009 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arbitration_scheduling / components/arbitration_scheduling/pipelined_arbiter |
| 功能家族 | Pipelined Arbiter |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：将仲裁过程分为配置的流水级，并将请求者身份与授权结果在同一流水事务中传递。

边界焦点：流水期间撤销请求不允许把旧授权错误归给新请求；采用握手接收请求快照，输出停顿必须保留结果。
...
候选实现：registered grant、lookahead。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 将仲裁过程分为配置的流水级，并将请求者身份与授权结果在同一流水事务中传递。 |
| REQ-002 | 关键边界 | 流水期间撤销请求不允许把旧授权错误归给新请求；采用握手接收请求快照，输出停顿必须保留结果。 |
