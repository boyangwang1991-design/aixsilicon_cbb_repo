...
registry_id: ARB-013
name: resource_pool_manager
version: 0.1.0
status: draft
basis: planning_intent
---

# resource_pool_manager 需求合同
...
| 索引 ID | ARB-013 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arbitration_scheduling / components/arbitration_scheduling/resource_pool_manager |
| 功能家族 | Resource Pool Manager |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：对可分配资源槽维护所有权，提供分配成功标志、槽号以及对应回收接口。

边界焦点：多路分配不得返回相同槽；资源耗尽必须反压，复位将池恢复到显式定义的全空闲集合。
...
候选实现：free list、stack、bitmap。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对可分配资源槽维护所有权，提供分配成功标志、槽号以及对应回收接口。 |
| REQ-002 | 关键边界 | 多路分配不得返回相同槽；资源耗尽必须反压，复位将池恢复到显式定义的全空闲集合。 |
