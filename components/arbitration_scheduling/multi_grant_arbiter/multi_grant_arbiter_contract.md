...
registry_id: ARB-007
name: multi_grant_arbiter
version: 0.1.0
status: draft
basis: planning_intent
---

# multi_grant_arbiter 需求合同
...
| 索引 ID | ARB-007 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arbitration_scheduling / components/arbitration_scheduling/multi_grant_arbiter |
| 功能家族 | Multi-grant Arbiter |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：每拍从有效请求中输出不超过 K 个不同授权，且每个授权均满足当前资源可用条件。

边界焦点：K 不得大于请求数；有 Bank 约束时同一互斥 Bank 只授权一个请求，未获授权请求保持待服务。
...
候选实现：top-K、prefix、bank-aware。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 每拍从有效请求中输出不超过 K 个不同授权，且每个授权均满足当前资源可用条件。 |
| REQ-002 | 关键边界 | K 不得大于请求数；有 Bank 约束时同一互斥 Bank 只授权一个请求，未获授权请求保持待服务。 |
