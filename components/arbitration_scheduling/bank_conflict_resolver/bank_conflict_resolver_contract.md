...
registry_id: ARB-017
name: bank_conflict_resolver
version: 0.1.0
status: draft
basis: planning_intent
---

# bank_conflict_resolver 需求合同
...
| 索引 ID | ARB-017 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arbitration_scheduling / components/arbitration_scheduling/bank_conflict_resolver |
| 功能家族 | Bank Conflict Resolver |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：检测同拍访问中的 Bank 冲突，对每个 Bank 保留一个服务请求，并反馈其余请求的重试状态。

边界焦点：地址映射在请求等待期间保持不变；重试请求不得丢失、重复完成或被错误当作已接受。
...
候选实现：replay/stall/remap。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 检测同拍访问中的 Bank 冲突，对每个 Bank 保留一个服务请求，并反馈其余请求的重试状态。 |
| REQ-002 | 关键边界 | 地址映射在请求等待期间保持不变；重试请求不得丢失、重复完成或被错误当作已接受。 |
