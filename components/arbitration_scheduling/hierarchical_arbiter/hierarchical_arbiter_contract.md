...
registry_id: ARB-008
name: hierarchical_arbiter
version: 0.1.0
status: draft
basis: planning_intent
---

# hierarchical_arbiter 需求合同
...
| 索引 ID | ARB-008 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arbitration_scheduling / components/arbitration_scheduling/hierarchical_arbiter |
| 功能家族 | Hierarchical Arbiter |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在分组请求上先执行组内候选选择、再执行组间选择，最终授权必须反馈到唯一叶子请求。

边界焦点：空组不可获服务；组间公平与组内公平分别评估，不宣称分层轮转等价于全局等权轮转。
...
候选实现：local+global、cluster。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在分组请求上先执行组内候选选择、再执行组间选择，最终授权必须反馈到唯一叶子请求。 |
| REQ-002 | 关键边界 | 空组不可获服务；组间公平与组内公平分别评估，不宣称分层轮转等价于全局等权轮转。 |
