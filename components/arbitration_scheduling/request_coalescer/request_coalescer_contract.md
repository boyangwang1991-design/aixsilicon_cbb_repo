...
| 索引 ID | ARB-014 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arbitration_scheduling / components/arbitration_scheduling/request_coalescer |
| 功能家族 | Request Coalescer |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：将目标、地址和兼容属性完全相同的未完成请求合并为一个下游请求，并记录全部订阅者。

边界焦点：有副作用或顺序语义不允许合并；一次完成必须扇出给全部且仅原订阅者，缓冲不足时反压。
...
候选实现：same target/address merge。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 将目标、地址和兼容属性完全相同的未完成请求合并为一个下游请求，并记录全部订阅者。 |
| REQ-002 | 关键边界 | 有副作用或顺序语义不允许合并；一次完成必须扇出给全部且仅原订阅者，缓冲不足时反压。 |
