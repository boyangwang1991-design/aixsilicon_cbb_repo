...
if command -v vcs >/dev/null 2>&1; then
    ...
    for n in 2 4 8 16 32 64; do
      for wm in 0 1; do
        if ( cd "$WORK" && vcs -full64 -timescale=1ns/1ps -sverilog \
            -pvalue+weighted_rr_arbiter.NUM_REQ=$n \
            -pvalue+weighted_rr_arbiter.PC_IMPL=$impl \
            -pvalue+weighted_rr_arbiter.WMODE=$wm \
            $RTL -o /tmp/wrra_g3_${impl}_${n}_${wm} > "$EV/tmp.log" 2>&1 ); then
          echo "PASS impl=$impl N=$n WMODE=$wm" | tee -a "$EV/param_matrix.txt"
        else
          echo "FAIL impl=$impl N=$n WMODE=$wm" | tee -a "$EV/param_matrix.txt"; cat "$EV/tmp.log"; exit 1
        fi
      done
    done
  done
  ...
  ( cd "$WORK" && vcs -full64 -timescale=1ns/1ps -sverilog $RTL $NEG_TB -top negative_elab_tb \
      -o /tmp/wrra_neg > "$EV/negative_elab.txt" 2>&1 )
  neg_rc=$?
  set -e
  if [ "$neg_rc" -eq 0 ]; then
    echo "[NEGATIVE] elaboration 未拦截非法参数，负向测试失败"; exit 1
  fi
  grep -qi "PC-001\|PC-002\|PC-003\|PC-004\|PC-005\|PC-006\|PC-007\|PC-008" "$EV/negative_elab.txt" || {
...
# ---- 2. Lint（SpyGlass lint/lint_rtl，project 调用）----
if command -v spyglass >/dev/null 2>&1; then
  SGW="$WORK/sg"
  rm -rf "$SGW" && mkdir -p "$SGW"
  cat > "$SGW/wrra_lint.prj" <<EOF
set_option enableSV yes
set_option enableSV09 yes
set_option top weighted_rr_arbiter
read_file -type hdl $RTL
EOF
  ( cd "$SGW" && spyglass -project wrra_lint.prj -goal lint/lint_rtl -batch \
