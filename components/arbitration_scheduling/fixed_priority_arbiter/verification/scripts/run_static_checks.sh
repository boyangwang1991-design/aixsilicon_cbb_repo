...
RTL="$P/rtl/fixed_priority_arbiter.sv"
...
if command -v vcs >/dev/null 2>&1; then
  ...
  for impl in 0 1 2; do
    for n in 2 4 8 16 32 64; do
      if ( cd "$WORK" && vcs -full64 -timescale=1ns/1ps -sverilog \
          -pvalue+fixed_priority_arbiter.NUM_REQ=$n \
          -pvalue+fixed_priority_arbiter.PC_IMPL=$impl \
          $RTL -o /tmp/fpa_g3_${impl}_${n} > "$EV/tmp.log" 2>&1 ); then
  ...
  ( cd "$WORK" && vcs -full64 -timescale=1ns/1ps -sverilog $RTL $NEG_TB -top negative_elab_tb \
      -o /tmp/fpa_neg > "$EV/negative_elab.txt" 2>&1 )
  neg_rc=$?
  set -e
  if [ "$neg_rc" -eq 0 ]; then
    echo "[NEGATIVE] elaboration 未拦截非法参数，负向测试失败"; exit 1
  fi
  grep -qi "PC-001\|PC-002\|PRIORITY\|REQ_TYPE\|FAST_GRANT\|PC_IMPL" "$EV/negative_elab.txt" || {
...
if command -v spyglass >/dev/null 2>&1; then
  ...
  cat > "$SGW/fpa_lint.prj" <<EOF
set_option enableSV yes
set_option enableSV09 yes
set_option top fixed_priority_arbiter
read_file -type hdl $RTL
EOF
  ( cd "$SGW" && spyglass -project fpa_lint.prj -goal lint/lint_rtl -batch \
