...
# 固定 seed: TB 内 SEED=32'hFPA_2026_0828（tc_random/equiv 可重放）
...
TB="$P/verification/simulation/fpa_tb.sv"

# ---- 单仿真多场景（穷举 + 优先级 + 边界 + 随机互斥/等价 + 锁存 + 寄存）----
( cd "$WORK" && vcs -full64 -timescale=1ns/1ps -sverilog -assert enable_diag $RTL $TB \
    -o /tmp/fpa_g4 > "$EV/tmp_compile.log" 2>&1 )
# -assert disable_cover 关闭覆盖 info；-suppress 抑制 VCS 提示类信息，保持证据干净
/tmp/fpa_g4 -assert disable_cover -suppress=all > "$EV/functional_sim.txt" 2>&1
grep -q "FPA_TB PASS" "$EV/functional_sim.txt" || {
    echo "functional sim FAILED"; cat "$EV/functional_sim.txt"; exit 1; }
echo "[sim] FPA_TB PASS (exhaust_w4 + priority + edge + random2000×N64 + equiv + latched + registered)"
