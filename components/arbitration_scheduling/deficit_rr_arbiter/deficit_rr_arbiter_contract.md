...
registry_id: ARB-004
name: deficit_rr_arbiter
version: 0.1.0
status: draft
basis: planning_intent
---

# deficit_rr_arbiter 需求合同
...
| 索引 ID | ARB-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arbitration_scheduling / components/arbitration_scheduling/deficit_rr_arbiter |
| 功能家族 | Deficit RR Arbiter |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：为每个请求者维护 deficit，轮到该请求者时增加 quantum，仅在 deficit 不小于条目代价时允许服务。

边界焦点：服务后扣除实际代价；空队列默认清零 deficit，连续大包可跨轮积累，禁止计数溢出导致虚假服务资格。

拟复用场景：DMA 多请求共享端口；多 Bank 数据通路资源分配。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：请求有效向量、候选身份/代价与资源可用量输入；授权/接受向量、选择索引和局部错误输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：N 请求路数≥1；资源容量≥1；计数位宽须表示容量；K 授权数满足 1≤K≤N；权重/代价按本核所需配置。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：byte/packet quantum。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 为每个请求者维护 deficit，轮到该请求者时增加 quantum，仅在 deficit 不小于条目代价时允许服务。 |
| REQ-002 | 关键边界 | 服务后扣除实际代价；空队列默认清零 deficit，连续大包可跨轮积累，禁止计数溢出导致虚假服务资格。 |
| REQ-003 | 时序/接受 | 授权出现不等于完成；仲裁状态只在明确的服务确认时推进，等待期间保持事务身份。 |
| REQ-004 | 复位/重启 | reset 清空未完成分配并恢复已声明的初始资源集合；消费者必须同步废弃旧授权。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：全空请求、全竞争、单请求持续、停顿、同拍分配/回收、长期公平与资源耗尽。 |
