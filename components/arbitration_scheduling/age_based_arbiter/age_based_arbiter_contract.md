...
registry_id: ARB-005
name: age_based_arbiter
version: 0.1.0
status: draft
basis: planning_intent
---

# age_based_arbiter 需求合同
...
| 索引 ID | ARB-005 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arbitration_scheduling / components/arbitration_scheduling/age_based_arbiter |
| 功能家族 | Age-based Arbiter |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：优先服务等待时间最大的有效请求，并为等待年龄相同者使用固定索引顺序消歧。

边界焦点：等待年龄仅在等待周期递增，获服务后清除；计数饱和而不回绕，防止最老请求变成最年轻。
...
候选实现：timestamp/counter。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 优先服务等待时间最大的有效请求，并为等待年龄相同者使用固定索引顺序消歧。 |
| REQ-002 | 关键边界 | 等待年龄仅在等待周期递增，获服务后清除；计数饱和而不回绕，防止最老请求变成最年轻。 |
