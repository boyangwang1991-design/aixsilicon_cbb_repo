...
registry_id: ARB-018
name: outstanding_tracker
version: 0.1.0
status: draft
basis: planning_intent
---

# outstanding_tracker 需求合同
...
| 索引 ID | ARB-018 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/arbitration_scheduling / components/arbitration_scheduling/outstanding_tracker |
| 功能家族 | Outstanding Tracker |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：按 tag 记录已接受而未完成的操作，匹配完成事件后释放对应表项。

边界焦点：重复 tag 分配、未知 tag 完成及容量耗尽均须检测；同拍分配完成按旧表匹配后更新。
...
候选实现：counter/tag table/bitmap。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按 tag 记录已接受而未完成的操作，匹配完成事件后释放对应表项。 |
| REQ-002 | 关键边界 | 重复 tag 分配、未知 tag 完成及容量耗尽均须检测；同拍分配完成按旧表匹配后更新。 |
