...
| 索引 ID | COD-002 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/coding_integrity / components/coding_integrity/crc_gen_check |
| 功能家族 | CRC Generator/Checker |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：按显式多项式、初值、输入位序、反射和最终异或配置更新 CRC 状态，并可检查接收余数。

边界焦点：帧开始/结束及尾拍有效字节数量参与计算；空帧和部分尾拍不能多计算填充字节。
...
候选实现：serial、parallel、sliced。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按显式多项式、初值、输入位序、反射和最终异或配置更新 CRC 状态，并可检查接收余数。 |
| REQ-002 | 关键边界 | 帧开始/结束及尾拍有效字节数量参与计算；空帧和部分尾拍不能多计算填充字节。 |
