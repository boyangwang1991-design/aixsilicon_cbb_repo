...
| 索引 ID | COD-008 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/coding_integrity / components/coding_integrity/lfsr_prbs |
| 功能家族 | LFSR/PRBS |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：按指定反馈多项式、状态位序和初始种子生成 LFSR 状态及每拍 PRBS 数据。

边界焦点：非法锁死种子拒绝或显式重置；输出步进按接受事件推进，不能在接收端停顿时跳过序列。
...
候选实现：Fibonacci、Galois、parallel。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按指定反馈多项式、状态位序和初始种子生成 LFSR 状态及每拍 PRBS 数据。 |
| REQ-002 | 关键边界 | 非法锁死种子拒绝或显式重置；输出步进按接受事件推进，不能在接收端停顿时跳过序列。 |
