...
registry_id: COD-012
name: data_packer_unpacker
version: 0.1.0
status: draft
basis: planning_intent
---

# data_packer_unpacker 需求合同
...
| 索引 ID | COD-012 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/coding_integrity / components/coding_integrity/data_packer_unpacker |
| 功能家族 | Data Packer/Unpacker |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：依据显式字段偏移与宽度将多个字段打包为位向量，或执行逆向解包。

边界焦点：字段重叠和越界在配置时拒绝；未使用位填零，跨字边界的字段分割与重组保持位序。
...
候选实现：field map、aligned/unaligned。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 依据显式字段偏移与宽度将多个字段打包为位向量，或执行逆向解包。 |
| REQ-002 | 关键边界 | 字段重叠和越界在配置时拒绝；未使用位填零，跨字边界的字段分割与重组保持位序。 |
