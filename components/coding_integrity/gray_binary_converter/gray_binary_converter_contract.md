...
registry_id: COD-006
name: gray_binary_converter
version: 0.1.0
status: draft
basis: planning_intent
---

# gray_binary_converter 需求合同
...
| 索引 ID | COD-006 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/coding_integrity / components/coding_integrity/gray_binary_converter |
| 功能家族 | Gray/Binary Converter |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：实现二进制与反射 Gray 码的双向组合转换，编码关系为 G=B xor (B右移一位)。

边界焦点：零位宽非法；流水可选时结果 valid 与输入延迟一致，本组件不包含跨域同步器。

拟复用场景：存储数据保护核心；链路编解码与完整性检查叶子核。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：数据/有效字节、校验或种子控制输入；编码数据、综合征/错误状态或更新状态输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：数据宽度≥1；校验矩阵/多项式和位序显式配置；串并行展开宽度、输出格式及是否流水共同定义配置。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：combinational、pipelined。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 实现二进制与反射 Gray 码的双向组合转换，编码关系为 G=B xor (B右移一位)。 |
| REQ-002 | 关键边界 | 零位宽非法；流水可选时结果 valid 与输入延迟一致，本组件不包含跨域同步器。 |
| REQ-003 | 时序/接受 | 状态按有效输入接受步进；流水保持 payload 与校验结果一致，不能多算空拍或未使能字节。 |
| REQ-004 | 复位/重启 | reset/load-seed 建立确定初始状态；有帧状态的核在帧起点重建，异常中止不得污染下一帧。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：全零/全一、逐位翻转、连续与停顿输入、位序、尾部有效字节、可纠正/不可纠正边界。 |
