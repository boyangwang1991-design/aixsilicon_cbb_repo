...
registry_id: COD-007
name: scrambler_descrambler
version: 0.1.0
status: draft
basis: planning_intent
---

# scrambler_descrambler 需求合同
...
| 索引 ID | COD-007 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/coding_integrity / components/coding_integrity/scrambler_descrambler |
| 功能家族 | Scrambler/Descrambler |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：按指定生成多项式和种子对输入比特序列执行扰码或解扰，串并行实现共享序列语义。

边界焦点：并行展开的位顺序必须明确；复位/重新装种子与数据接受冲突时重新装载优先，不隐式改变帧边界。
...
候选实现：self-synchronous、LFSR。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按指定生成多项式和种子对输入比特序列执行扰码或解扰，串并行实现共享序列语义。 |
| REQ-002 | 关键边界 | 并行展开的位顺序必须明确；复位/重新装种子与数据接受冲突时重新装载优先，不隐式改变帧边界。 |
