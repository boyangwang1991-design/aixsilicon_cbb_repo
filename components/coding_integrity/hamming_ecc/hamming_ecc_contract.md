...
| 索引 ID | COD-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/coding_integrity / components/coding_integrity/hamming_ecc |
| 功能家族 | Configurable Hamming ECC |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：按指定校验矩阵编码/解码缩短汉明码，并输出综合征和可纠正单错位置。

边界焦点：普通模式不宣称双错可可靠区分；扩展总奇偶位模式与非扩展模式分别定义，非法矩阵在生成时拒绝。
...
候选实现：shortened、extended。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按指定校验矩阵编码/解码缩短汉明码，并输出综合征和可纠正单错位置。 |
| REQ-002 | 关键边界 | 普通模式不宣称双错可可靠区分；扩展总奇偶位模式与非扩展模式分别定义，非法矩阵在生成时拒绝。 |
