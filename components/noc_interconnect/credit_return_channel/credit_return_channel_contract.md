...
registry_id: NOC-009
name: credit_return_channel
version: 0.1.0
status: draft
basis: planning_intent
---

# credit_return_channel 需求合同
...
| 索引 ID | NOC-009 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/noc_interconnect / components/noc_interconnect/credit_return_channel |
| 功能家族 | Credit Return Channel |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：把释放的缓冲 credit 按通道编号送回生产者，允许聚合多次归还为一个增量。

边界焦点：聚合累加器满时反压或及时发送，不得截断增量；复位后双方必须重新建立相同 credit 基线。
...
候选实现：aggregated/per-VC。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把释放的缓冲 credit 按通道编号送回生产者，允许聚合多次归还为一个增量。 |
| REQ-002 | 关键边界 | 聚合累加器满时反压或及时发送，不得截断增量；复位后双方必须重新建立相同 credit 基线。 |
