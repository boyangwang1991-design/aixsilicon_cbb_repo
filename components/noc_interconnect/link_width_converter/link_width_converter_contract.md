...
registry_id: NOC-012
name: link_width_converter
version: 0.1.0
status: draft
basis: planning_intent
---

# link_width_converter 需求合同
...
| 索引 ID | NOC-012 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/noc_interconnect / components/noc_interconnect/link_width_converter |
| 功能家族 | Link Width Converter |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：把一个局部 flit 分段成多个窄拍或将多个窄拍组装为宽 flit，保留边界标志。

边界焦点：部分尾 flit 携带有效位/字节数；组装中断按显式 flush 丢弃半包并报告，不输出拼接残值。

拟复用场景：IP 内点到点局部链路；协议适配层下方的数据交换结构。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：局部 flit/payload、路由选择或 credit 与流控输入；局部传输数据、接受和错误输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：端口数/数据宽度≥1；连接矩阵合法；缓冲容量与在途/credit 反馈延迟匹配；CDC 模式声明时钟关系。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：flit segmentation/assembly。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把一个局部 flit 分段成多个窄拍或将多个窄拍组装为宽 flit，保留边界标志。 |
| REQ-002 | 关键边界 | 部分尾 flit 携带有效位/字节数；组装中断按显式 flush 丢弃半包并报告，不输出拼接残值。 |
| REQ-003 | 时序/接受 | 每个已接受 flit 只能按配置送到合法目的；数据、目的与边界一起保持，不实现完整网络事务层。 |
| REQ-004 | 复位/重启 | reset 清流水/路由状态并与两端重新建立 credit 或链路有效基线，旧 flit 不得误消费。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：全端口竞争、合法/禁用连接、骤停、credit 延迟、尾 flit、链路复位与端到端数据守恒。 |
