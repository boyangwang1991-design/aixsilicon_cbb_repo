...
registry_id: NOC-011
name: link_cdc_adapter
version: 0.1.0
status: draft
basis: planning_intent
---

# link_cdc_adapter 需求合同
...
| 索引 ID | NOC-011 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/noc_interconnect / components/noc_interconnect/link_cdc_adapter |
| 功能家族 | Link CDC Adapter |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：在点到点流两端之间执行数据与流控跨域，采用明确的异步 FIFO 或同频弹性模式。

边界焦点：两域独立复位必须有重新同步/清空约定；有限缓存满时反压，不静默丢 flit。
...
候选实现：async FIFO/mesochronous。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在点到点流两端之间执行数据与流控跨域，采用明确的异步 FIFO 或同频弹性模式。 |
| REQ-002 | 关键边界 | 两域独立复位必须有重新同步/清空约定；有限缓存满时反压，不静默丢 flit。 |
