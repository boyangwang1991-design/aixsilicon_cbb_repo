...
registry_id: NOC-007
name: crossbar_fabric
version: 0.1.0
status: draft
basis: planning_intent
---

# crossbar_fabric 需求合同
...
| 索引 ID | NOC-007 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/noc_interconnect / components/noc_interconnect/crossbar_fabric |
| 功能家族 | Crossbar Fabric |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：按选择矩阵把局部输入 payload 路由到输出，冲突输出通过仲裁形成确定接受关系。

边界焦点：无广播配置时一个输入至多被一个输出接受；不实现 AXI/NoC 完整事务语义或系统路由策略。
...
候选实现：full/sparse/multistage。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按选择矩阵把局部输入 payload 路由到输出，冲突输出通过仲裁形成确定接受关系。 |
| REQ-002 | 关键边界 | 无广播配置时一个输入至多被一个输出接受；不实现 AXI/NoC 完整事务语义或系统路由策略。 |
