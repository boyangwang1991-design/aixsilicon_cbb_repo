...
registry_id: MIG-CBB-SAF-015
name: diversity_comparator
version: 0.1.0
status: draft
basis: planning_intent
---

# diversity_comparator 需求合同

本文为本轮规划需求草案，明确该核应交付的能力和验收条件。尚未完成 G1 参数/接口冻结；默认值、实现支持上限和具体端口名须在 G1 按以下需求落实到 YAML，并经校验后形成可执行规格。本文不构成已有 RTL 或已通过验证的声明。

| 字段 | 内容 |
|---|---|
| 索引 ID | MIG-CBB-SAF-015 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/safety_reliability / components/safety_reliability/diversity_comparator |
| 功能家族 | diversity_comparator |
| 优先级 | P2 |
| 当前登记状态 | planned |

## 1. 定位与使用场景

纯比较/CRC 核心逻辑，使用信号或简单握手接口；不承担系统故障管理、链路重放或完整链路协议。

需求焦点：对两个已对齐的不同实现结果执行位级比较，或按显式配置比较各自摘要。

边界焦点：摘要相等不证明原数据绝对相等；输出失配与比较有效标志，仅提供局部故障信息，不执行系统安全管理。

拟复用场景：异构执行结果局部核对；双数据通路冗余检查。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：已对齐的两路数据或摘要、比较有效与掩码；失配和有效诊断输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：比较宽度≥1；摘要宽度/算法/种子显式；流水级数≥0；比较窗口与输入对齐方式明确。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：待规格定义；沿用源条目的功能意图。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对两个已对齐的不同实现结果执行位级比较，或按显式配置比较各自摘要。 |
| REQ-002 | 关键边界 | 摘要相等不证明原数据绝对相等；输出失配与比较有效标志，仅提供局部故障信息，不执行系统安全管理。 |
| REQ-003 | 时序/接受 | 只对同一逻辑事务且比较有效的数据产生结论；摘要模式必须说明别名风险，不能替代原数据等价证明。 |
| REQ-004 | 复位/重启 | reset 清比较有效和已有诊断流水；重新使能需要有效输入基线，不把启动垃圾当成失配。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：一致/单错/多错、摘要碰撞边界说明、全屏蔽、使能切换、对齐偏差和故障指示延迟。 |
