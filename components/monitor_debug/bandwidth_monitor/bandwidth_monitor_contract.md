...
| 索引 ID | MON-005 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/monitor_debug / components/monitor_debug/bandwidth_monitor |
| 功能家族 | Bandwidth Monitor |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在配置窗口内累加实际接受的 beat 或有效字节数，输出带宽计数样本。

边界焦点：尾拍按有效字节掩码计数，valid 但未 ready 不计入；窗口切换不能丢掉边界那拍流量。
...
候选实现：bytes/beats/window。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在配置窗口内累加实际接受的 beat 或有效字节数，输出带宽计数样本。 |
| REQ-002 | 关键边界 | 尾拍按有效字节掩码计数，valid 但未 ready 不计入；窗口切换不能丢掉边界那拍流量。 |
