...
| 索引 ID | MON-006 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/monitor_debug / components/monitor_debug/occupancy_monitor |
| 功能家族 | Occupancy Monitor |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：对被监控对象的占用量记录当前值、峰值、累积和或分箱计数。

边界焦点：样本超出容量时报告错误；平均值的采样数和舍入方法公开，峰值 clear 与新峰值同拍新样本优先。
...
候选实现：current/max/average/histogram。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对被监控对象的占用量记录当前值、峰值、累积和或分箱计数。 |
| REQ-002 | 关键边界 | 样本超出容量时报告错误；平均值的采样数和舍入方法公开，峰值 clear 与新峰值同拍新样本优先。 |
