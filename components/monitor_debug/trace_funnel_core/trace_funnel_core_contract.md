...
registry_id: MON-011
name: trace_funnel_core
version: 0.1.0
status: draft
basis: planning_intent
---

# trace_funnel_core 需求合同
...
| 索引 ID | MON-011 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/monitor_debug / components/monitor_debug/trace_funnel_core |
| 功能家族 | Trace Funnel |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
无调试CSR的Trace汇聚核，关注仲裁与排序

需求焦点：将多源 trace 按优先级、轮转或时间戳规则汇聚为一条局部流。

边界焦点：源内顺序保持；跨源时间戳排序仅在共同时间基准且等待界限明确时承诺，不实现调试 CSR。
...
候选实现：priority/RR/timestamp merge。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 将多源 trace 按优先级、轮转或时间戳规则汇聚为一条局部流。 |
| REQ-002 | 关键边界 | 源内顺序保持；跨源时间戳排序仅在共同时间基准且等待界限明确时承诺，不实现调试 CSR。 |
