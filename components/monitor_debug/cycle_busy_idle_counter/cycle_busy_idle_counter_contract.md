...
registry_id: MON-003
name: cycle_busy_idle_counter
version: 0.1.0
status: draft
basis: planning_intent
---

# cycle_busy_idle_counter 需求合同
...
| 索引 ID | MON-003 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/monitor_debug / components/monitor_debug/cycle_busy_idle_counter |
| 功能家族 | Cycle/Busy/Idle Counter |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：在每个测量周期将样本计入 busy 或 idle，使两者之和等于有效测量周期数。

边界焦点：测量关闭时所有计数保持；窗口边界同拍 clear 与样本归属明确，计数溢出有标志。
...
候选实现：gated/free-running。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在每个测量周期将样本计入 busy 或 idle，使两者之和等于有效测量周期数。 |
| REQ-002 | 关键边界 | 测量关闭时所有计数保持；窗口边界同拍 clear 与样本归属明确，计数溢出有标志。 |
