...
| 索引 ID | MON-010 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/monitor_debug / components/monitor_debug/trace_fifo |
| 功能家族 | Trace FIFO |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：缓存 trace 记录，按配置选择 lossless 反压、丢新或覆盖旧记录的溢出策略。

边界焦点：丢弃必须以计数/标记可观察；记录不能只保留部分字段，复位清除旧记录有效性。
...
候选实现：lossless/drop/overwrite。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 缓存 trace 记录，按配置选择 lossless 反压、丢新或覆盖旧记录的溢出策略。 |
| REQ-002 | 关键边界 | 丢弃必须以计数/标记可观察；记录不能只保留部分字段，复位清除旧记录有效性。 |
