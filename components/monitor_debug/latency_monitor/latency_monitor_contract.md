...
| 索引 ID | MON-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/monitor_debug / components/monitor_debug/latency_monitor |
| 功能家族 | Latency Monitor |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：记录请求接受到对应完成的周期差，可输出单次延迟或分箱统计。

边界焦点：并发操作按 tag 匹配；未知完成、表满和时间戳回绕需检测，不能把不同请求开始/完成配对。
...
候选实现：timestamp/FIFO/histogram。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 记录请求接受到对应完成的周期差，可输出单次延迟或分箱统计。 |
| REQ-002 | 关键边界 | 并发操作按 tag 匹配；未知完成、表满和时间戳回绕需检测，不能把不同请求开始/完成配对。 |
