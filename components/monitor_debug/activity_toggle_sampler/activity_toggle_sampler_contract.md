...
registry_id: MON-008
name: activity_toggle_sampler
version: 0.1.0
status: draft
basis: planning_intent
---

# activity_toggle_sampler 需求合同
...
| 索引 ID | MON-008 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/monitor_debug / components/monitor_debug/activity_toggle_sampler |
| 功能家族 | Activity/Toggle Sampler |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：按配置采样间隔比较信号前后值，统计被观察到的翻转数或位图。

边界焦点：不声称可见采样间隔内所有毛刺；首次采样只建立基线，未知态另报不计入确定翻转。
...
候选实现：sampled/windowed。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按配置采样间隔比较信号前后值，统计被观察到的翻转数或位图。 |
| REQ-002 | 关键边界 | 不声称可见采样间隔内所有毛刺；首次采样只建立基线，未知态另报不计入确定翻转。 |
