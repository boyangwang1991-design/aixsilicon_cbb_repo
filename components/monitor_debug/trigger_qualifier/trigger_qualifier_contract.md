...
registry_id: MON-012
name: trigger_qualifier
version: 0.1.0
status: draft
basis: planning_intent
---

# trigger_qualifier 需求合同
...
| 索引 ID | MON-012 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/monitor_debug / components/monitor_debug/trigger_qualifier |
| 功能家族 | Trigger/Qualifier |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：对采样输入按值/掩码和有限序列条件产生触发事件，支持显式武装与清除。

边界焦点：多级触发状态仅在有效样本推进；超时或取消清掉当前匹配进度，单次模式触发后停止。
...
候选实现：match/mask/sequence。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对采样输入按值/掩码和有限序列条件产生触发事件，支持显式武装与清除。 |
| REQ-002 | 关键边界 | 多级触发状态仅在有效样本推进；超时或取消清掉当前匹配进度，单次模式触发后停止。 |
