...
| 索引 ID | MON-001 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/monitor_debug / components/monitor_debug/event_counter |
| 功能家族 | Event Counter |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：按输入有效事件增量累计次数，支持饱和或回绕及显式 clear/snapshot。

边界焦点：多事件增量不能被当作一；clear-on-read 只由已接受读事件触发，不因读地址出现而清除。
...
候选实现：saturating/wrapping/clear-on-read。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按输入有效事件增量累计次数，支持饱和或回绕及显式 clear/snapshot。 |
| REQ-002 | 关键边界 | 多事件增量不能被当作一；clear-on-read 只由已接受读事件触发，不因读地址出现而清除。 |
