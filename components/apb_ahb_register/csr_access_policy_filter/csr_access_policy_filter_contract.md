...
registry_id: BUS-015
name: csr_access_policy_filter
version: 0.1.0
status: draft
basis: planning_intent
---

# csr_access_policy_filter 需求合同
...
| 索引 ID | BUS-015 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/apb_ahb_register / components/apb_ahb_register/csr_access_policy_filter |
| 功能家族 | CSR Access Policy Filter |
| 优先级 | P1 |
| 当前登记状态 | planned |

## 1. 定位与使用场景

无总线事务处理的局部访问属性比较与写使能过滤；不承担拒绝响应、软件权限配置或系统策略。

需求焦点：根据输入访问属性与静态权限配置产生 allow 和有效写掩码，仅过滤局部寄存访问使能。

边界焦点：拒绝访问不产生写使能；权限配置不包含软件地址图，不承担总线拒绝响应或系统权限管理。
...
候选实现：RO/RW/W1C/privilege。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 根据输入访问属性与静态权限配置产生 allow 和有效写掩码，仅过滤局部寄存访问使能。 |
| REQ-002 | 关键边界 | 拒绝访问不产生写使能；权限配置不包含软件地址图，不承担总线拒绝响应或系统权限管理。 |
