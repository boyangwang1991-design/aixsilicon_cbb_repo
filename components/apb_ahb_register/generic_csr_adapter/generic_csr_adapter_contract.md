...
registry_id: BUS-001
name: generic_csr_adapter
version: 0.1.0
status: draft
basis: planning_intent
---

# generic_csr_adapter 需求合同
...
| 索引 ID | BUS-001 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/apb_ahb_register / components/apb_ahb_register/generic_csr_adapter |
| 功能家族 | Generic CSR Bus Adapter |
| 优先级 | P0 |
| 当前登记状态 | planned |

## 1. 定位与使用场景

简单 req/rsp 握手转换核；不实现完整 APB/AXI/AHB 协议或产品寄存器图。

需求焦点：将局部请求转成内部读写操作，并把一次操作结果作为一次响应返回，最多一个未完成请求。

边界焦点：响应未被接受前不接收新请求；非法读写组合给出局部错误，不生成 APB/AXI/AHB 完整事务。
...
候选实现：req/rsp、single outstanding。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 将局部请求转成内部读写操作，并把一次操作结果作为一次响应返回，最多一个未完成请求。 |
| REQ-002 | 关键边界 | 响应未被接受前不接收新请求；非法读写组合给出局部错误，不生成 APB/AXI/AHB 完整事务。 |
