...
registry_id: BUS-014
name: csr_shadow_commit_adapter
version: 0.1.0
status: draft
basis: planning_intent
---

# csr_shadow_commit_adapter 需求合同
...
| 索引 ID | BUS-014 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/apb_ahb_register / components/apb_ahb_register/csr_shadow_commit_adapter |
| 功能家族 | CSR Shadow/Commit Adapter |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：写入 shadow 副本，commit 时将一个配置组原子转为 active，读出可选择当前 active 或 shadow。

边界焦点：同拍 shadow 写与 commit 默认提交旧 shadow，再应用新写入；不得让消费者看到半组新配置。

拟复用场景：外围 IP 局部寄存器数据通路；配置镜像和访问使能处理。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：局部读写/提交/更新请求、数据与属性输入；局部结果、允许掩码或完成输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：数据宽度≥1；寄存组/副本数≥1；掩码粒度必须整除对应数据域；单未完成请求或组提交范围显式选择。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：atomic update/snapshot。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 写入 shadow 副本，commit 时将一个配置组原子转为 active，读出可选择当前 active 或 shadow。 |
| REQ-002 | 关键边界 | 同拍 shadow 写与 commit 默认提交旧 shadow，再应用新写入；不得让消费者看到半组新配置。 |
| REQ-003 | 时序/接受 | 只对局部事件负责；请求被接受后保留其数据与属性直到对应完成，不能产生完整总线响应。 |
| REQ-004 | 复位/重启 | reset 恢复配置初值并清未完成更新；清除、写入与提交的同拍优先级必须符合逐项需求。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：掩码全零/全一、同拍写提交、拒绝访问、响应停顿、多副本更新一致性。 |
