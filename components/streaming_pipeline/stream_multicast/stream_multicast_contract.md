...
| 索引 ID | STR-020 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_multicast |
| 功能家族 | Stream Multicast |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：依据输入携带的目的掩码将 beat 发送到指定多个输出，并记录每个目标的完成。

边界焦点：零目的掩码按显式丢弃并计数处理；已完成目的不能重复接受，掩码在输入接受时锁存。
...
候选实现：all/subset destinations。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 依据输入携带的目的掩码将 beat 发送到指定多个输出，并记录每个目标的完成。 |
| REQ-002 | 关键边界 | 零目的掩码按显式丢弃并计数处理；已完成目的不能重复接受，掩码在输入接受时锁存。 |
