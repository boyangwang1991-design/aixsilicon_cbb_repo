...
| 索引 ID | STR-010 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_fork |
| 功能家族 | Stream Fork |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：把每个输入 beat 复制到固定多个分支，并跟踪每个分支各自接受情况。

边界焦点：分支不同步 ready 时已接受者不得再次收到同一 beat；所有分支完成前保留原 payload。
...
候选实现：all-ready、independent buffer。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把每个输入 beat 复制到固定多个分支，并跟踪每个分支各自接受情况。 |
| REQ-002 | 关键边界 | 分支不同步 ready 时已接受者不得再次收到同一 beat；所有分支完成前保留原 payload。 |
