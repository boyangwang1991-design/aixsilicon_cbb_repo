...
registry_id: STR-024
name: stream_monitor_tap
version: 0.1.0
status: draft
basis: planning_intent
---

# stream_monitor_tap 需求合同
...
| 索引 ID | STR-024 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_monitor_tap |
| 功能家族 | Stream Monitor Tap |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：被动采样局部流握手及指定 payload 字段，输出监控事件或采样副本。

边界焦点：tap 不改变主流 ready/valid；观测端满时按显式丢样策略计数，不反压主数据流。
...
候选实现：passive/filtered/sampled。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 被动采样局部流握手及指定 payload 字段，输出监控事件或采样副本。 |
| REQ-002 | 关键边界 | tap 不改变主流 ready/valid；观测端满时按显式丢样策略计数，不反压主数据流。 |
