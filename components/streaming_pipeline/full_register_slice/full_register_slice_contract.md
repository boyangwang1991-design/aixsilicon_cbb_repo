...
registry_id: STR-006
name: full_register_slice
version: 0.1.0
status: draft
basis: planning_intent
---

# full_register_slice 需求合同
...
| 索引 ID | STR-006 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/full_register_slice |
| 功能家族 | Full Register Slice |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：同时切断前向和反向组合路径，以至少足够的弹性容量维持持续传输。

边界焦点：稳态上下游均就绪时每拍接受并输出一条；骤停时存下在途数据，不能丢最后一次合法握手。
...
候选实现：skid/full throughput。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 同时切断前向和反向组合路径，以至少足够的弹性容量维持持续传输。 |
| REQ-002 | 关键边界 | 稳态上下游均就绪时每拍接受并输出一条；骤停时存下在途数据，不能丢最后一次合法握手。 |
