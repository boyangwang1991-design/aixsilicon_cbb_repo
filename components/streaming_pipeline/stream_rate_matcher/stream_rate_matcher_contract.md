...
registry_id: STR-016
name: stream_rate_matcher
version: 0.1.0
status: draft
basis: planning_intent
---

# stream_rate_matcher 需求合同
...
| 索引 ID | STR-016 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_rate_matcher |
| 功能家族 | Stream Rate Matcher |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：按所选模式对流执行节流、复制或丢弃，明确输出与输入样本数量的关系。

边界焦点：复制/丢弃属于有损或变采样配置，必须输出事件计数；无损模式只能反压，不能隐式丢样本。
...
候选实现：throttle/replicate/drop。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按所选模式对流执行节流、复制或丢弃，明确输出与输入样本数量的关系。 |
| REQ-002 | 关键边界 | 复制/丢弃属于有损或变采样配置，必须输出事件计数；无损模式只能反压，不能隐式丢样本。 |
