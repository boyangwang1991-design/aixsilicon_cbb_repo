...
| 索引 ID | STR-008 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_mux |
| 功能家族 | Stream Mux |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：选择一路局部 ready/valid 流输出，只有实际选中端可获得本次传输的 ready。

边界焦点：输出停顿时锁定选择及 payload；多源 valid 不得合并数据，包模式可选保持选择到 EOP 接受。
...
候选实现：binary/one-hot/arbitrated。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 选择一路局部 ready/valid 流输出，只有实际选中端可获得本次传输的 ready。 |
| REQ-002 | 关键边界 | 输出停顿时锁定选择及 payload；多源 valid 不得合并数据，包模式可选保持选择到 EOP 接受。 |
