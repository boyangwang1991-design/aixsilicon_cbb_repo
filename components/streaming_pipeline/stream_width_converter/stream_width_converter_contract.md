...
registry_id: STR-014
name: stream_width_converter
version: 0.1.0
status: draft
basis: planning_intent
---

# stream_width_converter 需求合同
...
| 索引 ID | STR-014 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_width_converter |
| 功能家族 | Stream Width Converter |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：把输入流重组到另一数据宽度，保持有效字节顺序、包边界和错误侧带。

边界焦点：非整比宽度需要残余位缓存；末拍不足宽度使用 keep/有效计数表示，不引入伪有效填充。
...
候选实现：integer/non-integer ratio。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把输入流重组到另一数据宽度，保持有效字节顺序、包边界和错误侧带。 |
| REQ-002 | 关键边界 | 非整比宽度需要残余位缓存；末拍不足宽度使用 keep/有效计数表示，不引入伪有效填充。 |
