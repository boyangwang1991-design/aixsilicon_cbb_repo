...
registry_id: STR-023
name: stream_traffic_shaper
version: 0.1.0
status: draft
basis: planning_intent
---

# stream_traffic_shaper 需求合同
...
| 索引 ID | STR-023 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_traffic_shaper |
| 功能家族 | Stream Traffic Shaper |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：用 token/leaky bucket 的局部状态约束输出速率与最大突发，不修改 payload。

边界焦点：容量和补充速率显式配置，credit 饱和不能回绕；背压期间不消耗发送预算，不实现系统 QoS 管理。

拟复用场景：IP 内 ready/valid 数据管线；流分发/重组及时序切分。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：同域 valid/ready、payload 及所需边界/目的/有效字节侧带；变换后的同类局部流输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：payload 位宽≥1；分支数≥1；转换宽度/缓冲容量匹配；延迟级数≥0；包边界和有效字节编码显式定义。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：token/leaky bucket。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 用 token/leaky bucket 的局部状态约束输出速率与最大突发，不修改 payload。 |
| REQ-002 | 关键边界 | 容量和补充速率显式配置，credit 饱和不能回绕；背压期间不消耗发送预算，不实现系统 QoS 管理。 |
| REQ-003 | 时序/接受 | valid 且 ready 才发生接受；停顿时保持当前事务。复制/丢弃/采样核按其显式数量关系验收，其他核守恒。 |
| REQ-004 | 复位/重启 | reset 清缓冲有效位和半包状态；flush 仅在核明确支持时使用，不能未经说明从中间删除有效数据。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：持续满速、输入空拍、突发背压、复位中断、各分支独立停顿、尾字有效位和边界保持。 |
