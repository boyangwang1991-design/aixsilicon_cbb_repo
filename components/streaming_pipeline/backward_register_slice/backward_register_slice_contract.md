...
registry_id: STR-005
name: backward_register_slice
version: 0.1.0
status: draft
basis: planning_intent
---

# backward_register_slice 需求合同
...
| 索引 ID | STR-005 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/backward_register_slice |
| 功能家族 | Backward Register Slice |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：注册反向 ready 路径，并用必要的 skid 容量接住 ready 撤销前已接受的数据。

边界焦点：不能只打一拍 ready 而无数据保留能力；前向旁路是否存在由该模式明确声明。
...
候选实现：ready registered。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 注册反向 ready 路径，并用必要的 skid 容量接住 ready 撤销前已接受的数据。 |
| REQ-002 | 关键边界 | 不能只打一拍 ready 而无数据保留能力；前向旁路是否存在由该模式明确声明。 |
