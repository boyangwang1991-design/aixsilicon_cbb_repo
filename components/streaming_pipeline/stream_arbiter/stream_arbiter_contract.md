...
| 索引 ID | STR-019 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_arbiter |
| 功能家族 | Stream Arbiter |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在多个局部流请求间选择被服务源，授权推进以实际输出握手为依据。

边界焦点：可选包锁定需要看到被接受的 EOP 才释放；长期公平仅在下游持续有服务机会时承诺。
...
候选实现：transfer/packet locked。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在多个局部流请求间选择被服务源，授权推进以实际输出握手为依据。 |
| REQ-002 | 关键边界 | 可选包锁定需要看到被接受的 EOP 才释放；长期公平仅在下游持续有服务机会时承诺。 |
