...
| 索引 ID | STR-011 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_join |
| 功能家族 | Stream Join |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：等待所有必需输入具备同一组合事务后，将它们的 payload 拼接成一个输出。

边界焦点：只有输出可接受时才同时消费组成项；tagged 模式按相同 tag 配对，禁止拼接不同事务。
...
候选实现：lockstep、tagged join。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 等待所有必需输入具备同一组合事务后，将它们的 payload 拼接成一个输出。 |
| REQ-002 | 关键边界 | 只有输出可接受时才同时消费组成项；tagged 模式按相同 tag 配对，禁止拼接不同事务。 |
