...
| 索引 ID | STR-001 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/fixed_delay_line |
| 功能家族 | Fixed Delay Line |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：把输入样本延迟固定 L 个有效时钟周期后输出，L=0 为显式组合旁路配置。

边界焦点：启动期间输出 valid 为零直到对应样本到达；不能用未初始化存储内容冒充有效历史样本。
...
候选实现：FF、SRL、RAM-based。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把输入样本延迟固定 L 个有效时钟周期后输出，L=0 为显式组合旁路配置。 |
| REQ-002 | 关键边界 | 启动期间输出 valid 为零直到对应样本到达；不能用未初始化存储内容冒充有效历史样本。 |
