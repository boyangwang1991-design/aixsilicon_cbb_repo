...
registry_id: STR-018
name: stream_depacketizer
version: 0.1.0
status: draft
basis: planning_intent
---

# stream_depacketizer 需求合同
...
| 索引 ID | STR-018 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_depacketizer |
| 功能家族 | Stream Depacketizer |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：按声明的局部帧格式解析并剥离头尾，将 payload 与提取元数据对应输出。

边界焦点：长度不符或截断帧输出错误；元数据不能错配到下一帧，不承担完整网络协议栈处理。
...
候选实现：parse/strip/metadata extract。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按声明的局部帧格式解析并剥离头尾，将 payload 与提取元数据对应输出。 |
| REQ-002 | 关键边界 | 长度不符或截断帧输出错误；元数据不能错配到下一帧，不承担完整网络协议栈处理。 |
