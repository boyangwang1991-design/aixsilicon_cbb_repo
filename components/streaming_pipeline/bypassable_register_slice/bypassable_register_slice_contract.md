...
registry_id: STR-007
name: bypassable_register_slice
version: 0.1.0
status: draft
basis: planning_intent
---

# bypassable_register_slice 需求合同
...
| 索引 ID | STR-007 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/bypassable_register_slice |
| 功能家族 | Bypassable Register Slice |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在静态旁路或缓冲模式之间提供相同数据传输语义，并显式声明模式延迟差异。

边界焦点：动态切换仅在内部空且无握手冲突时提交；非空时禁止旁路导致跳过已缓存数据。
...
候选实现：static/dynamic bypass。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在静态旁路或缓冲模式之间提供相同数据传输语义，并显式声明模式延迟差异。 |
| REQ-002 | 关键边界 | 动态切换仅在内部空且无握手冲突时提交；非空时禁止旁路导致跳过已缓存数据。 |
