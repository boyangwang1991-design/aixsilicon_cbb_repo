...
| 索引 ID | STR-021 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_broadcaster |
| 功能家族 | Stream Broadcaster |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：把输入流广播到所有输出，可用分布式缓冲降低高扇出。

边界焦点：每个输出必须按原顺序看到一次数据；不同分支延迟可不同，不能因此提前释放尚有分支未消费的数据。
...
候选实现：registered/distributed。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把输入流广播到所有输出，可用分布式缓冲降低高扇出。 |
| REQ-002 | 关键边界 | 每个输出必须按原顺序看到一次数据；不同分支延迟可不同，不能因此提前释放尚有分支未消费的数据。 |
