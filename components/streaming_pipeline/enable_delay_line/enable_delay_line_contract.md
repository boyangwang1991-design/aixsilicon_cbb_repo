...
registry_id: STR-002
name: enable_delay_line
version: 0.1.0
status: draft
basis: planning_intent
---

# enable_delay_line 需求合同
...
| 索引 ID | STR-002 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/enable_delay_line |
| 功能家族 | Enable Delay Line |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：仅在公共 enable 有效时推进延迟线，输出对应 L 次使能之前的输入样本。

边界焦点：enable 关闭时所有数据和有效位保持；延迟单位为使能事件而非自然时钟周期。
...
候选实现：clock-enable/data-gated。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 仅在公共 enable 有效时推进延迟线，输出对应 L 次使能之前的输入样本。 |
| REQ-002 | 关键边界 | enable 关闭时所有数据和有效位保持；延迟单位为使能事件而非自然时钟周期。 |
