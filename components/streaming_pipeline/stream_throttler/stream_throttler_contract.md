...
| 索引 ID | STR-022 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_throttler |
| 功能家族 | Stream Throttler |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：限制 ready/valid 流在给定周期预算内的接受数量，暂停时通过反压保留数据。

边界焦点：令牌或周期预算只在实际传输后扣减；禁用节流时正常直通，零预算不得错误放行。
...
候选实现：fixed/token-bucket。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 限制 ready/valid 流在给定周期预算内的接受数量，暂停时通过反压保留数据。 |
| REQ-002 | 关键边界 | 令牌或周期预算只在实际传输后扣减；禁用节流时正常直通，零预算不得错误放行。 |
