...
registry_id: STR-003
name: data_control_aligner
version: 0.1.0
status: draft
basis: planning_intent
---

# data_control_aligner 需求合同
...
| 索引 ID | STR-003 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/data_control_aligner |
| 功能家族 | Data/Control Aligner |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：把数据和相关控制支路补齐到共同逻辑采样时刻，输出对齐后的整组信号。

边界焦点：每个支路的原延迟和补偿延迟显式配置；最大延迟以下的空槽以 valid 屏蔽，不混用不同事务。
...
候选实现：fixed/programmable latency。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把数据和相关控制支路补齐到共同逻辑采样时刻，输出对齐后的整组信号。 |
| REQ-002 | 关键边界 | 每个支路的原延迟和补偿延迟显式配置；最大延迟以下的空槽以 valid 屏蔽，不混用不同事务。 |
