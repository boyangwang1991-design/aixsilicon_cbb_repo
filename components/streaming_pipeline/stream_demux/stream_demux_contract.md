...
| 索引 ID | STR-009 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_demux |
| 功能家族 | Stream Demux |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：把每个输入 beat 依据目的索引送到一个输出，input ready 取决于该目的的可接受性。

边界焦点：目的索引与 payload 一起保持；非法目的不握手并报错，不能把 ready 从无关输出错误反馈。
...
候选实现：decoded/multicast。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把每个输入 beat 依据目的索引送到一个输出，input ready 取决于该目的的可接受性。 |
| REQ-002 | 关键边界 | 目的索引与 payload 一起保持；非法目的不握手并报错，不能把 ready 从无关输出错误反馈。 |
