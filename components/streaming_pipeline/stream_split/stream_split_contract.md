...
| 索引 ID | STR-013 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_split |
| 功能家族 | Stream Split |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：按字段、长度或包内位置把一个输入流拆分为多个局部输出子流。

边界焦点：每个有效字节的目标由映射唯一确定；跨输出背压必须保存未发送部分，不丢失边界。
...
候选实现：field/length/packet based。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按字段、长度或包内位置把一个输入流拆分为多个局部输出子流。 |
| REQ-002 | 关键边界 | 每个有效字节的目标由映射唯一确定；跨输出背压必须保存未发送部分，不丢失边界。 |
