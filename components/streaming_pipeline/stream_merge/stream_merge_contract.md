...
| 索引 ID | STR-012 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_merge |
| 功能家族 | Stream Merge |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：将多路输入合并成一条流，保留源内顺序并输出可选源标识。

边界焦点：包粒度模式从首拍接受到 EOP 接受保持源；停顿不能推进仲裁，跨源顺序只按所选策略保证。
...
候选实现：priority/RR/interleaved。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 将多路输入合并成一条流，保留源内顺序并输出可选源标识。 |
| REQ-002 | 关键边界 | 包粒度模式从首拍接受到 EOP 接受保持源；停顿不能推进仲裁，跨源顺序只按所选策略保证。 |
