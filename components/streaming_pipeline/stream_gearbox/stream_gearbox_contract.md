...
| 索引 ID | STR-015 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/stream_gearbox |
| 功能家族 | Stream Gearbox |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：以位为粒度在不同宽度拍之间重排连续位流，累积器保存跨拍余数。

边界焦点：位序从最低或最高有效位开始需固定；输入/输出停止时余位不丢，flush 只按显式规则处理残余。
...
候选实现：bit/byte lane gearbox。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 以位为粒度在不同宽度拍之间重排连续位流，累积器保存跨拍余数。 |
| REQ-002 | 关键边界 | 位序从最低或最高有效位开始需固定；输入/输出停止时余位不丢，flush 只按显式规则处理残余。 |
