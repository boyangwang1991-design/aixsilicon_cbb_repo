...
registry_id: STR-004
name: forward_register_slice
version: 0.1.0
status: draft
basis: planning_intent
---

# forward_register_slice 需求合同
...
| 索引 ID | STR-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/streaming_pipeline / components/streaming_pipeline/forward_register_slice |
| 功能家族 | Forward Register Slice |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：对前向 valid/payload 注册，并让 ready 按容量规则向上游传播。

边界焦点：下游停顿时保持输出；允许同拍消费并补入，声明 ready 仍可存在组合路径。
...
候选实现：payload registered。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对前向 valid/payload 注册，并让 ready 按容量规则向上游传播。 |
| REQ-002 | 关键边界 | 下游停顿时保持输出；允许同拍消费并补入，声明 ready 仍可存在组合路径。 |
