...
registry_id: SAF-029
name: interrupt_router_core
version: 0.1.0
status: draft
basis: planning_intent
---

# interrupt_router_core 需求合同
...
| 索引 ID | SAF-029 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/interrupt_router_core |
| 功能家族 | Interrupt Router |
| 优先级 | P1 |
| 当前登记状态 | planned |

## 1. 定位与使用场景

无芯片中断编号/CSR绑定的路由核，支持到CLIC/安全岛双送

需求焦点：按配置把局部中断源路由到一个或多个目标请求向量。

边界焦点：广播时每个目的端独立完成语义明确；目标编码是局部索引，不包含芯片中断编号或软件地址图。
...
候选实现：static/configurable/multicast。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按配置把局部中断源路由到一个或多个目标请求向量。 |
| REQ-002 | 关键边界 | 广播时每个目的端独立完成语义明确；目标编码是局部索引，不包含芯片中断编号或软件地址图。 |
