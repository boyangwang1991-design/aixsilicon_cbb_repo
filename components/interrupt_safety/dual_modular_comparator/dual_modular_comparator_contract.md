...
registry_id: SAF-003
name: dual_modular_comparator
version: 0.1.0
status: draft
basis: planning_intent
---

# dual_modular_comparator 需求合同
...
| 索引 ID | SAF-003 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/dual_modular_comparator |
| 功能家族 | Dual Modular Comparator |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：比较两路在同一逻辑时刻有效的结果，输出失配标志及可选失配位图。

边界焦点：未同时有效时不比较；输入对齐由调用者或独立对齐核提供，不执行锁步启动和系统故障处置。
...
候选实现：cycle/transaction compare。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 比较两路在同一逻辑时刻有效的结果，输出失配标志及可选失配位图。 |
| REQ-002 | 关键边界 | 未同时有效时不比较；输入对齐由调用者或独立对齐核提供，不执行锁步启动和系统故障处置。 |
