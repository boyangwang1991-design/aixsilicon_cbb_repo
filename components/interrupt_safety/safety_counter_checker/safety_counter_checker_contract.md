...
registry_id: SAF-025
name: safety_counter_checker
version: 0.1.0
status: draft
basis: planning_intent
---

# safety_counter_checker 需求合同
...
| 索引 ID | SAF-025 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/safety_counter_checker |
| 功能家族 | Safety Counter Checker |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：比较双计数器或编码计数状态，检测不一致、非法编码和不允许的跳变。

边界焦点：比较时刻及计数使能对齐；合法回绕不能误报，检测覆盖不等同故障率或安全等级认证。
...
候选实现：dual counter/encoded counter。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 比较双计数器或编码计数状态，检测不一致、非法编码和不允许的跳变。 |
| REQ-002 | 关键边界 | 比较时刻及计数使能对齐；合法回绕不能误报，检测覆盖不等同故障率或安全等级认证。 |
