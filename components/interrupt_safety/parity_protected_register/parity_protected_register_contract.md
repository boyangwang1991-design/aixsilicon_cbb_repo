...
registry_id: SAF-001
name: parity_protected_register
version: 0.1.0
status: draft
basis: planning_intent
---

# parity_protected_register 需求合同
...
| 索引 ID | SAF-001 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/parity_protected_register |
| 功能家族 | Parity-protected Register |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：为局部寄存值存储或计算奇偶校验，并在读出/周期检查时给出一致性结果。

边界焦点：掩码写后重新计算完整数据的 parity；数据位或校验位单错可检测，错误不自动覆盖原值。
...
候选实现：data+parity、auto-check。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 为局部寄存值存储或计算奇偶校验，并在读出/周期检查时给出一致性结果。 |
| REQ-002 | 关键边界 | 掩码写后重新计算完整数据的 parity；数据位或校验位单错可检测，错误不自动覆盖原值。 |
