...
registry_id: SAF-020
name: reset_monitor_core
version: 0.1.0
status: draft
basis: planning_intent
---

# reset_monitor_core 需求合同
...
| 索引 ID | SAF-020 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/reset_monitor_core |
| 功能家族 | Reset Monitor |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
无系统告警/CSR策略的复位检查核，关注RDC与安全状态

需求焦点：按输入复位事件与域就绪信息检查置位时长、释放次序及异常重复。

边界焦点：采用存活监控域观察；不直接产生系统复位或拥有告警 CSR，无法观测的无时钟窗口明确标示。
...
候选实现：cause/order/duration check。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按输入复位事件与域就绪信息检查置位时长、释放次序及异常重复。 |
| REQ-002 | 关键边界 | 采用存活监控域观察；不直接产生系统复位或拥有告警 CSR，无法观测的无时钟窗口明确标示。 |
