...
registry_id: SAF-016
name: e2e_protection_codec
version: 0.1.0
status: draft
basis: planning_intent
---

# e2e_protection_codec 需求合同
...
| 索引 ID | SAF-016 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/e2e_protection_codec |
| 功能家族 | End-to-end Protection Codec |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：对 payload、数据身份和序号组合执行 CRC 编解码，返回数据及校验状态。

边界焦点：保护字段顺序和覆盖范围固定在配置中；不宣称符合某外部 E2E 标准，除非提供对应 profile 与验证证据。
...
候选实现：data+sequence+CRC。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对 payload、数据身份和序号组合执行 CRC 编解码，返回数据及校验状态。 |
| REQ-002 | 关键边界 | 保护字段顺序和覆盖范围固定在配置中；不宣称符合某外部 E2E 标准，除非提供对应 profile 与验证证据。 |
