...
| 索引 ID | SAF-018 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/heartbeat_monitor |
| 功能家族 | Alive/Heartbeat Monitor |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在有效监控窗口内检查心跳事件是否过早、过晚或缺失，分别输出诊断。

边界焦点：启动有显式宽限期；停机/维护模式不能被当作缺失心跳，参考时钟假设须列入使用约束。
...
候选实现：periodic/windowed。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在有效监控窗口内检查心跳事件是否过早、过晚或缺失，分别输出诊断。 |
| REQ-002 | 关键边界 | 启动有显式宽限期；停机/维护模式不能被当作缺失心跳，参考时钟假设须列入使用约束。 |
