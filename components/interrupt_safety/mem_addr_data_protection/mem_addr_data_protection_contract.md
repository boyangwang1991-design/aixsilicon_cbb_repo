...
registry_id: SAF-023
name: mem_addr_data_protection
version: 0.1.0
status: draft
basis: planning_intent
---

# mem_addr_data_protection 需求合同
...
| 索引 ID | SAF-023 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/mem_addr_data_protection |
| 功能家族 | Memory Address/Data Protection |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：在局部存储访问上为地址、数据或 tag 附加 parity/ECC 侧带并检查一致性。

边界焦点：侧带与原访问一起排队和返回；地址保护不能被数据校验替代，错误只输出局部诊断。
...
候选实现：parity/tag/ECC sideband。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在局部存储访问上为地址、数据或 tag 附加 parity/ECC 侧带并检查一致性。 |
| REQ-002 | 关键边界 | 侧带与原访问一起排队和返回；地址保护不能被数据校验替代，错误只输出局部诊断。 |
