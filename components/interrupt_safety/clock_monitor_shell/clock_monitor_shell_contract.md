...
registry_id: SAF-019
name: clock_monitor_shell
version: 0.1.0
status: draft
basis: planning_intent
---

# clock_monitor_shell 需求合同
...
| 索引 ID | SAF-019 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/clock_monitor_shell |
| 功能家族 | Clock Monitor Digital Shell |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：用独立参考时钟统计被测时钟边沿，在窗口内判断过快、过慢或停止。

边界焦点：量化误差与同步误差计入门限；参考时钟失效时不能继续宣称被测时钟正常。
...
候选实现：missing/too-fast/too-slow。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 用独立参考时钟统计被测时钟边沿，在窗口内判断过快、过慢或停止。 |
| REQ-002 | 关键边界 | 量化误差与同步误差计入门限；参考时钟失效时不能继续宣称被测时钟正常。 |
