...
| 索引 ID | SAF-011 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/error_aggregator |
| 功能家族 | Error Aggregator |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：按位图、OR 或优先编码形式汇聚多路局部错误，保留所选模式的源身份信息。

边界焦点：多个源同拍错误不能在位图模式丢失；计数去重需明确按源、周期还是边沿，不隐含系统动作。
...
候选实现：OR/tree/vector/priority。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按位图、OR 或优先编码形式汇聚多路局部错误，保留所选模式的源身份信息。 |
| REQ-002 | 关键边界 | 多个源同拍错误不能在位图模式丢失；计数去重需明确按源、周期还是边沿，不隐含系统动作。 |
