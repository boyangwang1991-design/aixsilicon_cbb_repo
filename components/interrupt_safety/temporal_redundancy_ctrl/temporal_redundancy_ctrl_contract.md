...
registry_id: SAF-006
name: temporal_redundancy_ctrl
version: 0.1.0
status: draft
basis: planning_intent
---

# temporal_redundancy_ctrl 需求合同
...
| 索引 ID | SAF-006 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/temporal_redundancy_ctrl |
| 功能家族 | Temporal Redundancy Controller |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：为同一局部操作安排两次时间分离执行，并按操作身份收集两份结果供比较。

边界焦点：第一次结果需保留到第二次结束；有外部副作用的操作不得未经幂等约定重复执行。
...
候选实现：replay/double-execute。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 为同一局部操作安排两次时间分离执行，并按操作身份收集两份结果供比较。 |
| REQ-002 | 关键边界 | 第一次结果需保留到第二次结束；有外部副作用的操作不得未经幂等约定重复执行。 |
