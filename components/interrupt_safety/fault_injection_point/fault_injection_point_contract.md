...
registry_id: SAF-009
name: fault_injection_point
version: 0.1.0
status: draft
basis: planning_intent
---

# fault_injection_point 需求合同
...
| 索引 ID | SAF-009 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/fault_injection_point |
| 功能家族 | Fault Injection Point |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在显式测试使能下对指定信号执行翻转、强制值或脉冲注入，并提供注入活动标志。

边界焦点：默认禁用且功能直通；注入范围与持续周期受限，不允许生产配置中隐式激活。
...
接口角色：被比较/监测数据、事件/掩码和局部清除输入；诊断、pending、失配或局部路由输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。
候选实现：force/flip/stuck-at/pulse。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在显式测试使能下对指定信号执行翻转、强制值或脉冲注入，并提供注入活动标志。 |
| REQ-002 | 关键边界 | 默认禁用且功能直通；注入范围与持续周期受限，不允许生产配置中隐式激活。 |
