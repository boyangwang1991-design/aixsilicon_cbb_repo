...
| 索引 ID | SAF-007 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/tmr_voter |
| 功能家族 | TMR Voter |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：对三份输入逐位执行多数表决，输出表决结果及不一致标志。

边界焦点：位级多数不保证结果等于任一路完整字；若要求字级一致需单独模式，三路全异时必须报告无一致多数。
...
候选实现：bit/word/state voter。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对三份输入逐位执行多数表决，输出表决结果及不一致标志。 |
| REQ-002 | 关键边界 | 位级多数不保证结果等于任一路完整字；若要求字级一致需单独模式，三路全异时必须报告无一致多数。 |
