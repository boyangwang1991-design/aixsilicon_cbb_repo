...
registry_id: SAF-004
name: lockstep_alignment_buffer
version: 0.1.0
status: draft
basis: planning_intent
---

# lockstep_alignment_buffer 需求合同
...
| 索引 ID | SAF-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/lockstep_alignment_buffer |
| 功能家族 | Lockstep Alignment Buffer |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：对主影两路结果做固定周期延迟或弹性配对，输出可比较的同一事务对。

边界焦点：按 tag 配对时重复/未知 tag 报错；一路过慢导致容量耗尽须反压或告警，不能错配不同事务。
...
候选实现：fixed/elastic delay。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对主影两路结果做固定周期延迟或弹性配对，输出可比较的同一事务对。 |
| REQ-002 | 关键边界 | 按 tag 配对时重复/未知 tag 报错；一路过慢导致容量耗尽须反压或告警，不能错配不同事务。 |
