...
registry_id: SAF-026
name: safety_fsm_checker
version: 0.1.0
status: draft
basis: planning_intent
---

# safety_fsm_checker 需求合同
...
| 索引 ID | SAF-026 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/safety_fsm_checker |
| 功能家族 | Safety FSM Checker |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：监控 FSM 状态编码及允许的迁移关系，非法状态或非法转移产生局部故障指示。

边界焦点：初始状态和 reset 期间检查豁免明确；不修改被监控状态，综合时需保持必要的编码可观测性。
...
候选实现：illegal state/transition monitor。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 监控 FSM 状态编码及允许的迁移关系，非法状态或非法转移产生局部故障指示。 |
| REQ-002 | 关键边界 | 初始状态和 reset 期间检查豁免明确；不修改被监控状态，综合时需保持必要的编码可观测性。 |
