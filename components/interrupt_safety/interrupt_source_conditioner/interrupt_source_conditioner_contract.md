...
registry_id: SAF-027
name: interrupt_source_conditioner
version: 0.1.0
status: draft
basis: planning_intent
---

# interrupt_source_conditioner 需求合同
...
| 索引 ID | SAF-027 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/interrupt_source_conditioner |
| 功能家族 | Interrupt Source Conditioner |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：对输入中断源执行所选同步、边沿检测、电平保持和掩码，再输出本地请求。

边界焦点：短脉冲需要捕获或握手机制；清除与新事件同拍新事件保留，不定义系统中断号和优先级 CSR。
...
候选实现：sync/pulse2level/sticky/mask。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对输入中断源执行所选同步、边沿检测、电平保持和掩码，再输出本地请求。 |
| REQ-002 | 关键边界 | 短脉冲需要捕获或握手机制；清除与新事件同拍新事件保留，不定义系统中断号和优先级 CSR。 |
