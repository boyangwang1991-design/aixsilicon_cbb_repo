...
| 索引 ID | SAF-012 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/error_router |
| 功能家族 | Error Router |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：把局部错误按源到目的掩码路由给一个或多个消费者，并保留错误类型或身份。

边界焦点：目的端不就绪时按握手缓存或输出溢出，不静默丢掉安全事件；路由配置改变不能重发旧事件。
...
候选实现：destination mask/multicast。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把局部错误按源到目的掩码路由给一个或多个消费者，并保留错误类型或身份。 |
| REQ-002 | 关键边界 | 目的端不就绪时按握手缓存或输出溢出，不静默丢掉安全事件；路由配置改变不能重发旧事件。 |
