...
registry_id: SAF-028
name: interrupt_aggregator_core
version: 0.1.0
status: draft
basis: planning_intent
---

# interrupt_aggregator_core 需求合同
...
| 索引 ID | SAF-028 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/interrupt_aggregator_core |
| 功能家族 | Interrupt Aggregator |
| 优先级 | P0 |
| 当前登记状态 | planned |

## 1. 定位与使用场景

无芯片中断编号/CSR绑定的聚合核，关注大位宽扇入

需求焦点：把多个本地中断请求形成聚合电平或分组向量，支持按源使能。

边界焦点：被屏蔽源是否保留 pending 必须明确；不因撤销 mask 清掉尚未消费的事件，不承担 CPU 中断协议。

拟复用场景：IP 内局部故障检查；中断前端和安全数据路径保护。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：被比较/监测数据、事件/掩码和局部清除输入；诊断、pending、失配或局部路由输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：比较/源位数≥1；掩码宽度匹配；序列窗口不超过可无歧义比较范围；故障延迟/门限按核定义。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：vector/tree/hierarchical。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把多个本地中断请求形成聚合电平或分组向量，支持按源使能。 |
| REQ-002 | 关键边界 | 被屏蔽源是否保留 pending 必须明确；不因撤销 mask 清掉尚未消费的事件，不承担 CPU 中断协议。 |
| REQ-003 | 时序/接受 | 比较、掩码、valid 必须属于相同逻辑时刻；事件传播延迟明确，不用软件/系统动作代替本核检测责任。 |
| REQ-004 | 复位/重启 | reset 后输出不包含未定义故障；已采集故障的清除与新故障同拍采用逐项策略，复位屏蔽窗口公开。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：单比特/多比特失配、全屏蔽、同拍清除与新事件、序号回绕、缺失心跳以及检测延迟。 |
