...
registry_id: SAF-010
name: error_status_latch
version: 0.1.0
status: draft
basis: planning_intent
---

# error_status_latch 需求合同
...
| 索引 ID | SAF-010 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/error_status_latch |
| 功能家族 | Error Status Latch |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：记录错误位图、首个错误上下文及可选计数，直到显式清除。

边界焦点：新错误与清除同拍新错误优先；首错寄存仅在无已有记录时写入，后续错误不能覆盖首因。
...
候选实现：sticky/first-error/count。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 记录错误位图、首个错误上下文及可选计数，直到显式清除。 |
| REQ-002 | 关键边界 | 新错误与清除同拍新错误优先；首错寄存仅在无已有记录时写入，后续错误不能覆盖首因。 |
