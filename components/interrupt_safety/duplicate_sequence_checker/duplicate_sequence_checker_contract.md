...
registry_id: SAF-017
name: duplicate_sequence_checker
version: 0.1.0
status: draft
basis: planning_intent
---

# duplicate_sequence_checker 需求合同
...
| 索引 ID | SAF-017 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/interrupt_safety / components/interrupt_safety/duplicate_sequence_checker |
| 功能家族 | Duplicate/Sequence Checker |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：在有限滑动窗口中识别重复、过旧和可接受新序号，并更新已见位图。

边界焦点：窗口小于序号模空间的一半；只在条目真正被接受后标已见，避免反压重试被误报为重复。
...
候选实现：rolling window/bitmap。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在有限滑动窗口中识别重复、过旧和可接受新序号，并更新已见位图。 |
| REQ-002 | 关键边界 | 窗口小于序号模空间的一半；只在条目真正被接受后标已见，避免反压重试被误报为重复。 |
