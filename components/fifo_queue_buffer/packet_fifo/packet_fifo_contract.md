...
| 索引 ID | QUE-009 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/fifo_queue_buffer / components/fifo_queue_buffer/packet_fifo |
| 功能家族 | Packet FIFO |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：以包为单位缓存数据，仅在包完整 commit 后允许消费者读取，drop 可撤销未提交包。

边界焦点：回滚只能影响当前未提交包，不能撤销已可见数据；容量需同时计入已提交和未提交部分。
...
候选实现：packet commit/drop。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 以包为单位缓存数据，仅在包完整 commit 后允许消费者读取，drop 可撤销未提交包。 |
| REQ-002 | 关键边界 | 回滚只能影响当前未提交包，不能撤销已可见数据；容量需同时计入已提交和未提交部分。 |
