// ============================================================
// skid_buffer —— Valid-Ready Skid Buffer（QUE-007, A3）
// 极简单文件：参数化位宽 + OUT 寄存级 + SKID 槽（bubble-free full-throughput）
...
// - 保序：输入→输出 FIFO 顺序不变，无丢无重。
// 契约：behavior.yaml（INV-001..004 / ASM-001..003 / non_goals）
// 参数：DATA_W ∈ [1,1024]（PC-001/002，elaboration 期 generate $error 拦截）
// ============================================================
...
module skid_buffer #(
    ...
    // ------------------------------------------------------------------
    // 参数检查（generate 块内 $error，elaboration 期拦截；PC-001/002）
    // ------------------------------------------------------------------
    generate
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // ---- OUT 级 next：槽优先于输入直达（FIFO 保序，2026-08 修正）----
