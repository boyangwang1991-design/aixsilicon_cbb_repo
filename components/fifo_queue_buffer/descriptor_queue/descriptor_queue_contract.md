...
registry_id: QUE-018
name: descriptor_queue
version: 0.1.0
status: draft
basis: planning_intent
---

# descriptor_queue 需求合同
...
| 索引 ID | QUE-018 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/fifo_queue_buffer / components/fifo_queue_buffer/descriptor_queue |
| 功能家族 | Descriptor Queue |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：对固定格式描述符进行入队、按序读取和回收，可用环或索引链组织。

边界焦点：本核只缓存/链接描述符，不解释 DMA/软件操作；链表断链、循环或越界索引应被检查。
...
候选实现：linked/ring/indexed。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对固定格式描述符进行入队、按序读取和回收，可用环或索引链组织。 |
| REQ-002 | 关键边界 | 本核只缓存/链接描述符，不解释 DMA/软件操作；链表断链、循环或越界索引应被检查。 |
