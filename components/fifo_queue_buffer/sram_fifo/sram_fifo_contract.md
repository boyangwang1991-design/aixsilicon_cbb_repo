...
| 索引 ID | QUE-005 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/fifo_queue_buffer / components/fifo_queue_buffer/sram_fifo |
| 功能家族 | SRAM FIFO |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：用指定读延迟的 SRAM 宏存储队列数据，通过预取缓冲提供局部队列接口。

边界焦点：SRAM 已发出的读与下游背压需正确缓冲；full 计算包含在途/预取条目，不能超卖容量。
...
候选实现：single/dual-port、prefetch。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 用指定读延迟的 SRAM 宏存储队列数据，通过预取缓冲提供局部队列接口。 |
| REQ-002 | 关键边界 | SRAM 已发出的读与下游背压需正确缓冲；full 计算包含在途/预取条目，不能超卖容量。 |
