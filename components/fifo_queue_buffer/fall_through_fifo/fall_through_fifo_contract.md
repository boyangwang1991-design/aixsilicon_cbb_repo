...
| 索引 ID | QUE-003 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/fifo_queue_buffer / components/fifo_queue_buffer/fall_through_fifo |
| 功能家族 | Fall-through FIFO |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：非空时直接呈现队首，在空态到首个数据到达时尽可能减少读出等待。

边界焦点：组合直通与预取注册为不同延迟配置；背压下队首稳定，空态输入直通不能造成重复存储和读取。
...
候选实现：combinational head、registered。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 非空时直接呈现队首，在空态到首个数据到达时尽可能减少读出等待。 |
| REQ-002 | 关键边界 | 组合直通与预取注册为不同延迟配置；背压下队首稳定，空态输入直通不能造成重复存储和读取。 |
