...
registry_id: QUE-014
name: multi_enqueue_fifo
version: 0.1.0
status: draft
basis: planning_intent
---

# multi_enqueue_fifo 需求合同
...
| 索引 ID | QUE-014 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/fifo_queue_buffer / components/fifo_queue_buffer/multi_enqueue_fifo |
| 功能家族 | Multi-enqueue FIFO |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：每拍接受若干输入条目并按输入端口优先顺序紧凑放入同一 FIFO。

边界焦点：空位不足时只接受确定前缀或显式接受掩码；指针增加量等于实际接受条目数。
...
候选实现：2/N push、compaction。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 每拍接受若干输入条目并按输入端口优先顺序紧凑放入同一 FIFO。 |
| REQ-002 | 关键边界 | 空位不足时只接受确定前缀或显式接受掩码；指针增加量等于实际接受条目数。 |
