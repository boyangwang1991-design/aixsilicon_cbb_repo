...
| 索引 ID | QUE-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/fifo_queue_buffer / components/fifo_queue_buffer/shift_reg_fifo |
| 功能家族 | Shift-register FIFO |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：以移位寄存器或抽头选择组织浅队列，对接受的入队/出队维持 FIFO 顺序。

边界焦点：同拍入队出队不能覆盖队首之后的数据；深度一要明确支持或在参数检查拒绝。
...
候选实现：shift-all、tap pointer。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 以移位寄存器或抽头选择组织浅队列，对接受的入队/出队维持 FIFO 顺序。 |
| REQ-002 | 关键边界 | 同拍入队出队不能覆盖队首之后的数据；深度一要明确支持或在参数检查拒绝。 |
