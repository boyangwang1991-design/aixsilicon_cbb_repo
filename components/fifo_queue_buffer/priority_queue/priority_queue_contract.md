...
| 索引 ID | QUE-017 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/fifo_queue_buffer / components/fifo_queue_buffer/priority_queue |
| 功能家族 | Priority Queue |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：按条目优先值选择下一个输出，条目同时携带 payload 和入队顺序标记。

边界焦点：同优先级采用稳定先入先出；容量满反压，优先值更新如支持须保证排序结构一致。
...
候选实现：heap、bucket、sorted array。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按条目优先值选择下一个输出，条目同时携带 payload 和入队顺序标记。 |
| REQ-002 | 关键边界 | 同优先级采用稳定先入先出；容量满反压，优先值更新如支持须保证排序结构一致。 |
