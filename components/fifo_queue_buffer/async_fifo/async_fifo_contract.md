...
| 索引 ID | QUE-002 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/fifo_queue_buffer / components/fifo_queue_buffer/async_fifo |
| 功能家族 | Asynchronous FIFO |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：用双时钟存储与跨域指针同步传递数据，各域独立产生安全的 full/empty。

边界焦点：Gray 指针方案的深度要求为二次幂且至少二；同步延迟允许保守满空，不允许未写读或覆盖未读数据。
...
候选实现：Gray pointer、bundled reset。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 用双时钟存储与跨域指针同步传递数据，各域独立产生安全的 full/empty。 |
| REQ-002 | 关键边界 | Gray 指针方案的深度要求为二次幂且至少二；同步延迟允许保守满空，不允许未写读或覆盖未读数据。 |
