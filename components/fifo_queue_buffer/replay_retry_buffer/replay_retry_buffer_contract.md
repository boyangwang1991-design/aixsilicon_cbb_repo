...
registry_id: QUE-019
name: replay_retry_buffer
version: 0.1.0
status: draft
basis: planning_intent
---

# replay_retry_buffer 需求合同
...
| 索引 ID | QUE-019 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/fifo_queue_buffer / components/fifo_queue_buffer/replay_retry_buffer |
| 功能家族 | Replay/Retry Buffer |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：保存可重放数据并在 checkpoint/ack 前保留，收到重放请求后从指定有效位置重新输出。

边界焦点：未确认数据不得覆盖；重放期间新的写入必须容量受控，本核不判断完整协议重试策略。

拟复用场景：外围/DMA 的速率解耦；加速器流缓存与局部排队。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：入队数据、有效/请求及出队就绪/请求输入；队首、接受、满空/占用及局部状态输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：数据宽度≥1；容量为正且满足实现最小深度；异步 Gray 指针深度为至少二的二次幂；端口数和转换比显式配置。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：checkpoint、selective replay。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 保存可重放数据并在 checkpoint/ack 前保留，收到重放请求后从指定有效位置重新输出。 |
| REQ-002 | 关键边界 | 未确认数据不得覆盖；重放期间新的写入必须容量受控，本核不判断完整协议重试策略。 |
| REQ-003 | 时序/接受 | 入队/出队以实际接受为准；延迟、空直通、注册 ready 和每拍接受数分别声明，不能只用容量推断吞吐。 |
| REQ-004 | 复位/重启 | reset 清有效计数与读写控制；存储数据不必清零但必须被 valid 屏蔽，半包/在途重放按逐项规则中止。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：空/满、同拍入出、长背压、回绕、深度边界、尾包/尾字、并发端口和零丢失/零重复。 |
