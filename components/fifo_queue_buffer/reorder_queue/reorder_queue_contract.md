...
| 索引 ID | QUE-016 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/fifo_queue_buffer / components/fifo_queue_buffer/reorder_queue |
| 功能家族 | Reorder Queue |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：接收带序号的条目并缓存乱序完成，只有当前期望序号就绪才按序释放。

边界焦点：序号回绕与窗口容量匹配；重复、超窗和未知完成给出错误，不能覆盖尚未退休项。
...
候选实现：tag/index、CAM/window。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 接收带序号的条目并缓存乱序完成，只有当前期望序号就绪才按序释放。 |
| REQ-002 | 关键边界 | 序号回绕与窗口容量匹配；重复、超窗和未知完成给出错误，不能覆盖尚未退休项。 |
