...
| 索引 ID | QUE-011 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/fifo_queue_buffer / components/fifo_queue_buffer/credit_fifo |
| 功能家族 | Credit FIFO |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：将队列存储容量与生产者可用 credit 关联，每次入队消费容量、出队释放容量。

边界焦点：credit 返回必须与真实释放一致；复位时双方重建基线，不能因重复归还造成覆盖。
...
候选实现：credit-aware enqueue/dequeue。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 将队列存储容量与生产者可用 credit 关联，每次入队消费容量、出队释放容量。 |
| REQ-002 | 关键边界 | credit 返回必须与真实释放一致；复位时双方重建基线，不能因重复归还造成覆盖。 |
