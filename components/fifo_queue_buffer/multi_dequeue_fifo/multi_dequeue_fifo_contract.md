...
registry_id: QUE-015
name: multi_dequeue_fifo
version: 0.1.0
status: draft
basis: planning_intent
---

# multi_dequeue_fifo 需求合同
...
| 索引 ID | QUE-015 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/fifo_queue_buffer / components/fifo_queue_buffer/multi_dequeue_fifo |
| 功能家族 | Multi-dequeue FIFO |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：每拍提供连续多个队首条目，并按已接受输出数推进读指针。

边界焦点：默认仅允许连续前缀消费；若支持独立输出 ready 则必须有保序缓冲，不能留下无法回收的洞。
...
候选实现：2/N pop、lookahead。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 每拍提供连续多个队首条目，并按已接受输出数推进读指针。 |
| REQ-002 | 关键边界 | 默认仅允许连续前缀消费；若支持独立输出 ready 则必须有保序缓冲，不能留下无法回收的洞。 |
