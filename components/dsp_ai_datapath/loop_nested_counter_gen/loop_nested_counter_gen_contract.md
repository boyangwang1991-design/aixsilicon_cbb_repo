...
registry_id: DSP-015
name: loop_nested_counter_gen
version: 0.1.0
status: draft
basis: planning_intent
---

# loop_nested_counter_gen 需求合同
...
| 索引 ID | DSP-015 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dsp_ai_datapath / components/dsp_ai_datapath/loop_nested_counter_gen |
| 功能家族 | Loop/Nested-counter Generator |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：维护有限层嵌套循环索引，内层达到边界后回绕并推进外层，最外层完成时产生一次 done。

边界焦点：计数仅在 enable 时推进；每层 trip count 至少为一，重载与推进同拍时重载优先。
...
候选实现：programmable/static。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 维护有限层嵌套循环索引，内层达到边界后回绕并推进外层，最外层完成时产生一次 done。 |
| REQ-002 | 关键边界 | 计数仅在 enable 时推进；每层 trip count 至少为一，重载与推进同拍时重载优先。 |
