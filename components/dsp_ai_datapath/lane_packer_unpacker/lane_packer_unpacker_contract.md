...
registry_id: DSP-001
name: lane_packer_unpacker
version: 0.1.0
status: draft
basis: planning_intent
---

# lane_packer_unpacker 需求合同
...
| 索引 ID | DSP-001 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dsp_ai_datapath / components/dsp_ai_datapath/lane_packer_unpacker |
| 功能家族 | Lane Packer/Unpacker |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：依据 lane 有效掩码紧凑打包或按原位置展开数据，并保留 lane 身份与元素顺序。

边界焦点：无效 lane 填零且 valid 清除；部分尾组输出有效元素数，不将填充元素当作真实样本。
...
候选实现：fixed/masked lanes。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 依据 lane 有效掩码紧凑打包或按原位置展开数据，并保留 lane 身份与元素顺序。 |
| REQ-002 | 关键边界 | 无效 lane 填零且 valid 清除；部分尾组输出有效元素数，不将填充元素当作真实样本。 |
