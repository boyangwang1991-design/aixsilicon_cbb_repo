...
registry_id: DSP-008
name: scatter_gather_index_gen
version: 0.1.0
status: draft
basis: planning_intent
---

# scatter_gather_index_gen 需求合同
...
| 索引 ID | DSP-008 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dsp_ai_datapath / components/dsp_ai_datapath/scatter_gather_index_gen |
| 功能家族 | Scatter/Gather Index Generator |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：按输入索引列表或仿射模式生成元素地址和原元素身份，支持有效掩码过滤。

边界焦点：索引越界须报告且不发访问；不解析软件 DMA 描述符，不重排已生成地址与数据的绑定关系。
...
候选实现：list/affine/masked。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按输入索引列表或仿射模式生成元素地址和原元素身份，支持有效掩码过滤。 |
| REQ-002 | 关键边界 | 索引越界须报告且不发访问；不解析软件 DMA 描述符，不重排已生成地址与数据的绑定关系。 |
