...
registry_id: DSP-011
name: activation_approx
version: 0.1.0
status: draft
basis: planning_intent
---

# activation_approx 需求合同
...
| 索引 ID | DSP-011 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dsp_ai_datapath / components/dsp_ai_datapath/activation_approx |
| 功能家族 | Activation Approximation |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：对输入执行 ReLU、PReLU 或所选分段近似，输出函数值及适用域异常标志。

边界焦点：分段边界采用确定的半开区间；每种近似配置给出误差界，ReLU 在零处输出精确零。
...
候选实现：ReLU/PReLU/LUT-piecewise。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对输入执行 ReLU、PReLU 或所选分段近似，输出函数值及适用域异常标志。 |
| REQ-002 | 关键边界 | 分段边界采用确定的半开区间；每种近似配置给出误差界，ReLU 在零处输出精确零。 |
