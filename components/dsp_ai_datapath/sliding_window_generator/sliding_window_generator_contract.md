...
registry_id: DSP-004
name: sliding_window_generator
version: 0.1.0
status: draft
basis: planning_intent
---

# sliding_window_generator 需求合同
...
| 索引 ID | DSP-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dsp_ai_datapath / components/dsp_ai_datapath/sliding_window_generator |
| 功能家族 | Sliding Window Generator |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：将按行顺序输入的像素流转换为配置尺寸的二维滑动窗口，保持行列坐标对应。

边界焦点：行首、行尾及图像边界按显式 padding 策略处理；背压必须同时冻结窗口数据、行缓冲索引和坐标。
...
候选实现：register/SRAM line buffer。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 将按行顺序输入的像素流转换为配置尺寸的二维滑动窗口，保持行列坐标对应。 |
| REQ-002 | 关键边界 | 行首、行尾及图像边界按显式 padding 策略处理；背压必须同时冻结窗口数据、行缓冲索引和坐标。 |
