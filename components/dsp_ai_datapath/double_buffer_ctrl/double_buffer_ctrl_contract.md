...
| 索引 ID | DSP-014 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dsp_ai_datapath / components/dsp_ai_datapath/double_buffer_ctrl |
| 功能家族 | Double-buffer Controller |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：在生产者与消费者间分配 ping-pong 或 N 个缓冲区的所有权，提交后才允许读取。

边界焦点：未消费完成的缓冲不能重新分配给写者；满/空状态驱动反压，切换不改变正在消费的缓冲号。
...
候选实现：ping-pong/N-buffer。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在生产者与消费者间分配 ping-pong 或 N 个缓冲区的所有权，提交后才允许读取。 |
| REQ-002 | 关键边界 | 未消费完成的缓冲不能重新分配给写者；满/空状态驱动反压，切换不改变正在消费的缓冲号。 |
