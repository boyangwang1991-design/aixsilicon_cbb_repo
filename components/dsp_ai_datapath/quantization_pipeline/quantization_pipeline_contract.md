...
| 索引 ID | DSP-010 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dsp_ai_datapath / components/dsp_ai_datapath/quantization_pipeline |
| 功能家族 | Quantization Pipeline |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：把输入、scale、zero-point 和通道标识按流水执行缩放、舍入、偏移和夹取。

边界焦点：各级必须传递相同事务的系数；背压时整组状态保持，通道切换不能使用上一通道的系数。
...
候选实现：scale/zero-point/clamp。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把输入、scale、zero-point 和通道标识按流水执行缩放、舍入、偏移和夹取。 |
| REQ-002 | 关键边界 | 各级必须传递相同事务的系数；背压时整组状态保持，通道切换不能使用上一通道的系数。 |
