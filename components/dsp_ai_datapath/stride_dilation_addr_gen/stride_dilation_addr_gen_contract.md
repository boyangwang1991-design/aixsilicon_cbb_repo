...
registry_id: DSP-007
name: stride_dilation_addr_gen
version: 0.1.0
status: draft
basis: planning_intent
---

# stride_dilation_addr_gen 需求合同
...
| 索引 ID | DSP-007 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dsp_ai_datapath / components/dsp_ai_datapath/stride_dilation_addr_gen |
| 功能家族 | Stride/Dilation Address Generator |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：按步幅和 dilation 计算采样坐标，坐标关系为输出索引乘 stride 加核索引乘 dilation。

边界焦点：超出输入边界时输出无效/padding 标志而不发真实访问；负偏移必须有显式有符号表示。
...
候选实现：nested/parameterized。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按步幅和 dilation 计算采样坐标，坐标关系为输出索引乘 stride 加核索引乘 dilation。 |
| REQ-002 | 关键边界 | 超出输入边界时输出无效/padding 标志而不发真实访问；负偏移必须有显式有符号表示。 |
