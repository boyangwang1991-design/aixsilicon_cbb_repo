...
registry_id: DSP-006
name: tile_address_generator
version: 0.1.0
status: draft
basis: planning_intent
---

# tile_address_generator 需求合同
...
| 索引 ID | DSP-006 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dsp_ai_datapath / components/dsp_ai_datapath/tile_address_generator |
| 功能家族 | Tile Address Generator |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：根据 tile 维度、基地址和每维步长生成仿射地址序列及 tile 完成标志。

边界焦点：地址运算使用足够位宽并报告溢出；只有地址被接受后推进索引，零维度配置拒绝。
...
候选实现：nested counter/affine。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 根据 tile 维度、基地址和每维步长生成仿射地址序列及 tile 完成标志。 |
| REQ-002 | 关键边界 | 地址运算使用足够位宽并报告溢出；只有地址被接受后推进索引，零维度配置拒绝。 |
