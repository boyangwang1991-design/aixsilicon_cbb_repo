...
| 索引 ID | DSP-002 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/dsp_ai_datapath / components/dsp_ai_datapath/vector_reduction |
| 功能家族 | Vector Reduction |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：对有效向量元素执行 sum、min、max、and 或 or 归约，操作在配置中固定或显式传入。

边界焦点：空集合使用操作的单位元并输出 no_data；sum 保留增长位，min/max 忽略被屏蔽元素。
...
候选实现：sum/min/max/and/or。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对有效向量元素执行 sum、min、max、and 或 or 归约，操作在配置中固定或显式传入。 |
| REQ-002 | 关键边界 | 空集合使用操作的单位元并输出 no_data；sum 保留增长位，min/max 忽略被屏蔽元素。 |
