...
registry_id: MEM-008
name: sram_width_composer
version: 0.1.0
status: draft
basis: planning_intent
---

# sram_width_composer 需求合同
...
| 索引 ID | MEM-008 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/sram_width_composer |
| 功能家族 | SRAM Width Composer |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：把多个窄宏并联组成更宽逻辑存储，按位段拆分写数据并拼接读结果。

边界焦点：写掩码映射与位段一一对应；逻辑位宽非宏整数倍时尾部位固定无效，禁止越界连线。
...
候选实现：concat、byte-lane banking。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把多个窄宏并联组成更宽逻辑存储，按位段拆分写数据并拼接读结果。 |
| REQ-002 | 关键边界 | 写掩码映射与位段一一对应；逻辑位宽非宏整数倍时尾部位固定无效，禁止越界连线。 |
