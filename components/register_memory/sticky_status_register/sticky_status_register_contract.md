...
registry_id: MEM-003
name: sticky_status_register
version: 0.1.0
status: draft
basis: planning_intent
---

# sticky_status_register 需求合同
...
| 索引 ID | MEM-003 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/sticky_status_register |
| 功能家族 | Sticky/W1C/W1S Register |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：事件置位后保持状态，直到显式清除；可按配置选择粘滞位图或其他局部状态语义。

边界焦点：事件与清除同拍默认事件优先，读取本身无副作用；clear-on-read 必须由已接受的读事件显式触发。
...
候选实现：status semantics variants。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 事件置位后保持状态，直到显式清除；可按配置选择粘滞位图或其他局部状态语义。 |
| REQ-002 | 关键边界 | 事件与清除同拍默认事件优先，读取本身无副作用；clear-on-read 必须由已接受的读事件显式触发。 |
