...
| 索引 ID | MEM-007 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/rf_multi_write |
| 功能家族 | Multi-write Register File |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：支持多个写端口对寄存文件更新，并给出每端写入是否被接受。

边界焦点：同址冲突按固定优先级默认只接受一路，或显式合并不重叠掩码；失败端不可被计为已写入。
...
候选实现：arbitration、banked。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 支持多个写端口对寄存文件更新，并给出每端写入是否被接受。 |
| REQ-002 | 关键边界 | 同址冲突按固定优先级默认只接受一路，或显式合并不重叠掩码；失败端不可被计为已写入。 |
