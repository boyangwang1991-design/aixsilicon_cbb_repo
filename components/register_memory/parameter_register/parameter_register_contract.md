...
registry_id: MEM-001
name: parameter_register
version: 0.1.0
status: draft
basis: planning_intent
---

# parameter_register 需求合同
...
| 索引 ID | MEM-001 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/parameter_register |
| 功能家族 | Parameter Register |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：保存配置宽度的局部寄存值，支持使能和逐位或逐字节写掩码。

边界焦点：未使能或被屏蔽位保持原值；复位值须适合位宽，写掩码粒度不整除数据宽度时拒绝配置。
...
候选实现：plain、enable、masked write。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 保存配置宽度的局部寄存值，支持使能和逐位或逐字节写掩码。 |
| REQ-002 | 关键边界 | 未使能或被屏蔽位保持原值；复位值须适合位宽，写掩码粒度不整除数据宽度时拒绝配置。 |
