...
registry_id: MEM-025
name: content_tag_array
version: 0.1.0
status: draft
basis: planning_intent
---

# content_tag_array 需求合同
...
| 索引 ID | MEM-025 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/content_tag_array |
| 功能家族 | Content Tag Array |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：为有限条目维护 tag 与 valid，按索引或集合读出并比较输入查询 tag。

边界焦点：无效条目不命中；替换与查询同拍语义明确，本核不实现完整 Cache/TLB 一致性或替换控制器。
...
候选实现：tag+valid+compare。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 为有限条目维护 tag 与 valid，按索引或集合读出并比较输入查询 tag。 |
| REQ-002 | 关键边界 | 无效条目不命中；替换与查询同拍语义明确，本核不实现完整 Cache/TLB 一致性或替换控制器。 |
