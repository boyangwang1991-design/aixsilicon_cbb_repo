...
| 索引 ID | MEM-024 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/cam |
| 功能家族 | CAM |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：对有效条目并行匹配搜索键，输出匹配位图及可选优先索引。

边界焦点：多重命中要显式报告，不能隐式认为唯一；写/失效与查询同拍按旧状态查询后更新。
...
候选实现：register-based、banked。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对有效条目并行匹配搜索键，输出匹配位图及可选优先索引。 |
| REQ-002 | 关键边界 | 多重命中要显式报告，不能隐式认为唯一；写/失效与查询同拍按旧状态查询后更新。 |
