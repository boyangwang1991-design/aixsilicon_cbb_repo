...
registry_id: MEM-010
name: sram_bank_mapper
version: 0.1.0
status: draft
basis: planning_intent
---

# sram_bank_mapper 需求合同
...
| 索引 ID | MEM-010 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/sram_bank_mapper |
| 功能家族 | SRAM Bank Mapper |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：把逻辑地址映射为 Bank 号和 Bank 内地址，支持明确的交织、范围或哈希模式。

边界焦点：映射应可证明不把两个不同有效地址错误别名到同一存储位置；非二次幂 Bank 数须处理取模。
...
候选实现：interleave、hash、range。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把逻辑地址映射为 Bank 号和 Bank 内地址，支持明确的交织、范围或哈希模式。 |
| REQ-002 | 关键边界 | 映射应可证明不把两个不同有效地址错误别名到同一存储位置；非二次幂 Bank 数须处理取模。 |
