...
| 索引 ID | MEM-005 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/rf_1r1w |
| 功能家族 | 1R1W Register File |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：每周期支持至多一读一写的寄存器文件，读响应延迟在配置中固定。

边界焦点：同地址读写需选 read-first、write-first 或 no-change；旁路数据必须应用写掩码后的合并值。
...
候选实现：flop、macro、replicated。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 每周期支持至多一读一写的寄存器文件，读响应延迟在配置中固定。 |
| REQ-002 | 关键边界 | 同地址读写需选 read-first、write-first 或 no-change；旁路数据必须应用写掩码后的合并值。 |
