...
| 索引 ID | MEM-022 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/circular_buffer |
| 功能家族 | Circular Buffer |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在固定容量内用读写指针循环存储数据，维护满空与有效区间。

边界焦点：回绕时保留用于区分满空的占用信息；覆盖模式只有显式启用才可丢最旧数据并报告丢弃。
...
候选实现：pointer/wrap、power-of-two。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在固定容量内用读写指针循环存储数据，维护满空与有效区间。 |
| REQ-002 | 关键边界 | 回绕时保留用于区分满空的占用信息；覆盖模式只有显式启用才可丢最旧数据并报告丢弃。 |
