...
registry_id: MEM-002
name: shadowed_register
version: 0.1.0
status: draft
basis: planning_intent
---

# shadowed_register 需求合同
...
| 索引 ID | MEM-002 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/shadowed_register |
| 功能家族 | Shadowed Register |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：维护写入影子与生效值，按显式提交或双写匹配机制更新生效值。

边界焦点：未完成的更新不能影响 active 输出；两次写不一致给出局部错误并保持旧生效值。
...
候选实现：dual-copy、commit。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 维护写入影子与生效值，按显式提交或双写匹配机制更新生效值。 |
| REQ-002 | 关键边界 | 未完成的更新不能影响 active 输出；两次写不一致给出局部错误并保持旧生效值。 |
