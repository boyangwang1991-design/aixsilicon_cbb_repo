...
registry_id: MEM-019
name: multi_bank_access_scheduler
version: 0.1.0
status: draft
basis: planning_intent
---

# multi_bank_access_scheduler 需求合同
...
| 索引 ID | MEM-019 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/multi_bank_access_scheduler |
| 功能家族 | Multi-bank Access Scheduler |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：在多个局部请求间根据 Bank 可用性选择可并行服务集合，并返回接受向量。

边界焦点：同一物理端口最多接收能力允许的访问数；重试请求维持身份，不执行系统 QoS 配置或总线响应。

拟复用场景：IP 内局部状态与存储组织；多 Bank 数据缓冲和宏适配。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：局部地址、读写使能、数据与掩码输入；读数据/有效、接受及局部错误输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：数据宽度/深度≥1；地址编码覆盖深度且越界拒绝；mask 粒度整除数据域；端口数和读延迟符合物理实现。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：fixed/RR/conflict-aware。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在多个局部请求间根据 Bank 可用性选择可并行服务集合，并返回接受向量。 |
| REQ-002 | 关键边界 | 同一物理端口最多接收能力允许的访问数；重试请求维持身份，不执行系统 QoS 配置或总线响应。 |
| REQ-003 | 时序/接受 | 明确同步/异步读及读写同址语义；有握手时只对接受访问更新状态，宏多拍读保留请求身份。 |
| REQ-004 | 复位/重启 | reset 清控制状态不自动清宏存储；需要初始化内容时必须走明确初始化流程并等待完成。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：首末地址、越界地址、部分写、RAW/WAW、多端口碰撞、连续读写、暂停以及 Bank 边界。 |
