...
registry_id: MEM-020
name: ping_pong_buffer
version: 0.1.0
status: draft
basis: planning_intent
---

# ping_pong_buffer 需求合同
...
| 索引 ID | MEM-020 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/ping_pong_buffer |
| 功能家族 | Ping-pong Buffer |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：用两个或多个 Bank 缓存连续数据块，以生产完成和消费完成事件切换角色。

边界焦点：写者不能覆盖未消费块；块长度与有效字数一起提交，最后一个元素接受后才确认消费完成。
...
候选实现：dual-bank、N-bank rotation。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 用两个或多个 Bank 缓存连续数据块，以生产完成和消费完成事件切换角色。 |
| REQ-002 | 关键边界 | 写者不能覆盖未消费块；块长度与有效字数一起提交，最后一个元素接受后才确认消费完成。 |
