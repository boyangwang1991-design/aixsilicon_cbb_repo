...
registry_id: MEM-011
name: sram_port_adapter
version: 0.1.0
status: draft
basis: planning_intent
---

# sram_port_adapter 需求合同
...
| 索引 ID | MEM-011 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/sram_port_adapter |
| 功能家族 | SRAM Port Adapter |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：用仲裁和局部握手把逻辑读写请求适配到指定的 SP、1R1W 或 TDP 宏端口能力。

边界焦点：物理端口不足时必须反压，不承诺超过宏能力的吞吐；读写碰撞遵守所选宏语义。
...
候选实现：1R1W↔SP/TDP语义适配。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 用仲裁和局部握手把逻辑读写请求适配到指定的 SP、1R1W 或 TDP 宏端口能力。 |
| REQ-002 | 关键边界 | 物理端口不足时必须反压，不承诺超过宏能力的吞吐；读写碰撞遵守所选宏语义。 |
