...
registry_id: MEM-017
name: memory_scrubber_engine
version: 0.1.0
status: draft
basis: planning_intent
---

# memory_scrubber_engine 需求合同
...
| 索引 ID | MEM-017 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/memory_scrubber_engine |
| 功能家族 | Memory Scrubber |
| 优先级 | P2 |
| 当前登记状态 | planned |

## 1. 定位与使用场景

无内存控制器/CSR绑定的巡检引擎，关注带宽占用、功耗

需求焦点：按地址范围发出后台读巡检，对可纠正错误请求回写纠正数据并汇报计数。

边界焦点：功能写与巡检同址时不得以旧巡检数据覆盖新写；支持暂停及带宽限制，不拥有总线/中断管理策略。
...
候选实现：periodic、on-demand、priority。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按地址范围发出后台读巡检，对可纠正错误请求回写纠正数据并汇报计数。 |
| REQ-002 | 关键边界 | 功能写与巡检同址时不得以旧巡检数据覆盖新写；支持暂停及带宽限制，不拥有总线/中断管理策略。 |
