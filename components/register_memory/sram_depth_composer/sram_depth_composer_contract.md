...
registry_id: MEM-009
name: sram_depth_composer
version: 0.1.0
status: draft
basis: planning_intent
---

# sram_depth_composer 需求合同
...
| 索引 ID | MEM-009 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/sram_depth_composer |
| 功能家族 | SRAM Depth Composer |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：把多个存储宏按地址高位选择级联为更深逻辑存储，并延迟 Bank 选择到读返回时刻。

边界焦点：未选宏不产生写使能；不同宏读延迟不能未经对齐直接拼接，尾部越界地址须拒绝。
...
候选实现：cascaded decode、bank select。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把多个存储宏按地址高位选择级联为更深逻辑存储，并延迟 Bank 选择到读返回时刻。 |
| REQ-002 | 关键边界 | 未选宏不产生写使能；不同宏读延迟不能未经对齐直接拼接，尾部越界地址须拒绝。 |
