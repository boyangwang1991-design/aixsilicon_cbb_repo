...
registry_id: MEM-023
name: lookup_table_rom
version: 0.1.0
status: draft
basis: planning_intent
---

# lookup_table_rom 需求合同
...
| 索引 ID | MEM-023 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/lookup_table_rom |
| 功能家族 | Lookup Table/ROM |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：按输入索引返回预定义表项，支持组合或固定延迟读的实现配置。

边界焦点：内容文件长度和位宽必须匹配配置；越界索引返回明确无效标志，不把未知初值当作正确查表。
...
候选实现：case、distributed、macro。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按输入索引返回预定义表项，支持组合或固定延迟读的实现配置。 |
| REQ-002 | 关键边界 | 内容文件长度和位宽必须匹配配置；越界索引返回明确无效标志，不把未知初值当作正确查表。 |
