...
| 索引 ID | MEM-021 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/line_buffer |
| 功能家族 | Line Buffer |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：缓存前若干行或固定距离的流数据，为当前输入提供对应历史位置样本。

边界焦点：行宽改变需清空或重新装载状态；停顿冻结行内位置，未填满时使用无效标志而非伪造历史数据。
...
候选实现：shift/SRAM/circular。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 缓存前若干行或固定距离的流数据，为当前输入提供对应历史位置样本。 |
| REQ-002 | 关键边界 | 行宽改变需清空或重新装载状态；停顿冻结行内位置，未填满时使用无效标志而非伪造历史数据。 |
