...
registry_id: MEM-015
name: memory_sleep_ctrl
version: 0.1.0
status: draft
basis: planning_intent
---

# memory_sleep_ctrl 需求合同
...
| 索引 ID | MEM-015 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/register_memory / components/register_memory/memory_sleep_ctrl |
| 功能家族 | Memory Sleep/Retention Controller |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：根据空闲和请求握手控制存储宏 sleep，并在规定唤醒延迟后重新允许访问。

边界焦点：进入 sleep 前排空访问；唤醒期间请求反压，内容保持能力必须按宏模型声明。
...
候选实现：idle-based、software-driven。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 根据空闲和请求握手控制存储宏 sleep，并在规定唤醒延迟后重新允许访问。 |
| REQ-002 | 关键边界 | 进入 sleep 前排空访问；唤醒期间请求反压，内容保持能力必须按宏模型声明。 |
