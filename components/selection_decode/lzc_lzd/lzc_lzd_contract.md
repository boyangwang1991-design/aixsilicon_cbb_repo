...
| 索引 ID | SEL-011 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/lzc_lzd |
| 功能家族 | Leading Zero/One Count |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：从最高有效位一端计算连续零或一的数量，结果范围为零到输入位宽。

边界焦点：全零或全一对应匹配模式时返回整个位宽；结果位宽须能表示输入位宽本身。
...
候选实现：tree、segmented、pipelined。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 从最高有效位一端计算连续零或一的数量，结果范围为零到输入位宽。 |
| REQ-002 | 关键边界 | 全零或全一对应匹配模式时返回整个位宽；结果位宽须能表示输入位宽本身。 |
