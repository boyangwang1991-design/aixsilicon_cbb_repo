...
| 索引 ID | SEL-003 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/priority_mux |
| 功能家族 | Priority Mux |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：从有效选择集合中按指定索引方向选最高优先级数据，并输出命中与索引。

边界焦点：多选择是合法输入而非 one-hot 错误；无选择时 valid 为零，不把索引零当作真实命中。
...
候选实现：Linear、tree、grouped。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 从有效选择集合中按指定索引方向选最高优先级数据，并输出命中与索引。 |
| REQ-002 | 关键边界 | 多选择是合法输入而非 one-hot 错误；无选择时 valid 为零，不把索引零当作真实命中。 |
