...
| 索引 ID | SEL-002 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/onehot_mux |
| 功能家族 | One-hot Mux |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：在零个或一个选择位有效的前提下输出所选数据，零选择输出零。

边界焦点：多热输入必须检测并报告违反前提，不能把 OR 混合结果当作正常有效选择。
...
候选实现：OR-tree、AND-OR、segmented。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在零个或一个选择位有效的前提下输出所选数据，零选择输出零。 |
| REQ-002 | 关键边界 | 多热输入必须检测并报告违反前提，不能把 OR 混合结果当作正常有效选择。 |
