...
registry_id: SEL-004
name: sparse_masked_mux
version: 0.1.0
status: draft
basis: planning_intent
---

# sparse_masked_mux 需求合同
...
| 索引 ID | SEL-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/sparse_masked_mux |
| 功能家族 | Sparse/Masked Mux |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：仅在静态允许连接且动态掩码使能的输入集合内选择数据。

边界焦点：稀疏映射中的禁用连接始终不可选；全屏蔽时输出无效，掩码变化不能绕过连接约束。
...
候选实现：sparse map、mask select。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 仅在静态允许连接且动态掩码使能的输入集合内选择数据。 |
| REQ-002 | 关键边界 | 稀疏映射中的禁用连接始终不可选；全屏蔽时输出无效，掩码变化不能绕过连接约束。 |
