...
| 索引 ID | SEL-017 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/address_decoder |
| 功能家族 | Address Decoder |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：按 base/size 或 base/mask 表产生地址命中位图、唯一命中索引和未命中/多命中状态。

边界焦点：区间重叠必须可检测；本核只输出命中信息，不生成总线错误响应或执行访问权限策略。
...
候选实现：range、mask、base+size。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按 base/size 或 base/mask 表产生地址命中位图、唯一命中索引和未命中/多命中状态。 |
| REQ-002 | 关键边界 | 区间重叠必须可检测；本核只输出命中信息，不生成总线错误响应或执行访问权限策略。 |
