...
registry_id: SEL-020
name: bit_permutation_network
version: 0.1.0
status: draft
basis: planning_intent
---

# bit_permutation_network 需求合同
...
| 索引 ID | SEL-020 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/bit_permutation_network |
| 功能家族 | Bit Permutation Network |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：根据静态或可配置置换映射重排位位置，输出各位来自唯一指定输入位。

边界焦点：纯置换模式映射须为双射；若允许复制/丢位应作为独立模式声明，配置越界或重复不得静默接受。
...
候选实现：fixed、programmable、Benes。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 根据静态或可配置置换映射重排位位置，输出各位来自唯一指定输入位。 |
| REQ-002 | 关键边界 | 纯置换模式映射须为双射；若允许复制/丢位应作为独立模式声明，配置越界或重复不得静默接受。 |
