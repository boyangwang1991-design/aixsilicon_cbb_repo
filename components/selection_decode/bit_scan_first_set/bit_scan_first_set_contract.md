...
registry_id: SEL-013
name: bit_scan_first_set
version: 0.1.0
status: draft
basis: planning_intent
---

# bit_scan_first_set 需求合同
...
| 索引 ID | SEL-013 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/bit_scan_first_set |
| 功能家族 | Bit Scan/First-set |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：按配置方向扫描位向量，返回第一个置位索引、one-hot 掩码和存在标志。

边界焦点：无置位时存在标志为零；输入含多置位仍只选第一个，不能混用计数和索引语义。
...
候选实现：LSB/MSB、priority tree。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按配置方向扫描位向量，返回第一个置位索引、one-hot 掩码和存在标志。 |
| REQ-002 | 关键边界 | 无置位时存在标志为零；输入含多置位仍只选第一个，不能混用计数和索引语义。 |
