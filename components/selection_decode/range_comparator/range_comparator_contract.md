...
| 索引 ID | SEL-016 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/range_comparator |
| 功能家族 | Range Comparator |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：判断输入是否落在配置区间内，默认区间为 lower≤value<upper。

边界焦点：空区间不命中，lower>upper 非法；端点和符号域必须一致，禁止无说明地混用闭区间。
...
候选实现：single/multi-range、tree。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 判断输入是否落在配置区间内，默认区间为 lower≤value<upper。 |
| REQ-002 | 关键边界 | 空区间不命中，lower>upper 非法；端点和符号域必须一致，禁止无说明地混用闭区间。 |
