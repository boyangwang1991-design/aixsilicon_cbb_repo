...
| 索引 ID | SEL-015 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/onehot_checker |
| 功能家族 | One-hot Checker |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：对输入分类为零热、单热或多热，可选择把零热作为合法或非法。

边界焦点：多热判定不能因高位截断漏检；纯组合版无时钟语义，注册版分类结果与采样对齐。
...
候选实现：zero/one/multi-hot detect。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对输入分类为零热、单热或多热，可选择把零热作为合法或非法。 |
| REQ-002 | 关键边界 | 多热判定不能因高位截断漏检；纯组合版无时钟语义，注册版分类结果与采样对齐。 |
