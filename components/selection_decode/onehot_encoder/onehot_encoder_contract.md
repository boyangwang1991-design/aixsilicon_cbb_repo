...
| 索引 ID | SEL-007 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/onehot_encoder |
| 功能家族 | One-hot Encoder |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：按 strict、first-hot 或 last-hot 模式把输入命中集合编码为独热选择结果。

边界焦点：strict 模式多热报错；first/last 模式明确选择索引方向，零输入始终返回零选择。
...
候选实现：strict、first-hot、last-hot。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按 strict、first-hot 或 last-hot 模式把输入命中集合编码为独热选择结果。 |
| REQ-002 | 关键边界 | strict 模式多热报错；first/last 模式明确选择索引方向，零输入始终返回零选择。 |
