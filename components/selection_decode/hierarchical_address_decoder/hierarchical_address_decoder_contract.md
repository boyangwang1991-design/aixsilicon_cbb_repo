...
registry_id: SEL-018
name: hierarchical_address_decoder
version: 0.1.0
status: draft
basis: planning_intent
---

# hierarchical_address_decoder 需求合同
...
| 索引 ID | SEL-018 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/hierarchical_address_decoder |
| 功能家族 | Hierarchical Address Decoder |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：按上层分区再按局部区间完成地址译码，结果与同等平面地址表一致。

边界焦点：局部偏移计算不得跨分区溢出；上层未命中时不允许下层误命中，流水需保持请求地址身份。

拟复用场景：控制路径选择与译码；地址匹配及位向量扫描。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：数据向量、索引/选择/掩码输入；选择数据、命中索引、计数或有效/非法标志输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：输入位宽/路数≥1；索引宽度至少覆盖所有合法位置；计数宽度必须可表示全宽结果；范围端点/掩码明确。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：cluster/local decode。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按上层分区再按局部区间完成地址译码，结果与同等平面地址表一致。 |
| REQ-002 | 关键边界 | 局部偏移计算不得跨分区溢出；上层未命中时不允许下层误命中，流水需保持请求地址身份。 |
| REQ-003 | 时序/接受 | 组合核不引入隐式状态；若采用流水，选择、数据及 valid 同拍采样并按相同级数到达输出。 |
| REQ-004 | 复位/重启 | 纯组合实现无复位要求；寄存实现 reset 清输出 valid，不以未初始化值代表命中。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：零输入、全一、单热、多热、非法索引、非二次幂规模、范围端点及所有实现的位级等价。 |
