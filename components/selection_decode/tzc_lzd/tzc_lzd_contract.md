...
| 索引 ID | SEL-012 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/tzc_lzd |
| 功能家族 | Trailing Zero/One Count |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：从最低有效位一端计算连续零或一的数量，输出计数及可选非匹配位位置。

边界焦点：全匹配输入返回整个位宽，不因输出索引位宽不足回绕为零；反转实现必须保持端序。
...
候选实现：reverse、tree。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 从最低有效位一端计算连续零或一的数量，输出计数及可选非匹配位位置。 |
| REQ-002 | 关键边界 | 全匹配输入返回整个位宽，不因输出索引位宽不足回绕为零；反转实现必须保持端序。 |
