...
| 索引 ID | SEL-009 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/priority_encoder |
| 功能家族 | Priority Encoder |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：从位向量中找最高或最低优先的置位，返回二进制索引及有效标志。

边界焦点：全零输入返回 valid=0；位宽非二次幂时索引必须保持合法，优先方向通过参数固定。
...
候选实现：leading/trailing、tree。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 从位向量中找最高或最低优先的置位，返回二进制索引及有效标志。 |
| REQ-002 | 关键边界 | 全零输入返回 valid=0；位宽非二次幂时索引必须保持合法，优先方向通过参数固定。 |
