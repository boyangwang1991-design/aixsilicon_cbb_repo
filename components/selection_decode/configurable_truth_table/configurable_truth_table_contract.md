...
registry_id: SEL-019
name: configurable_truth_table
version: 0.1.0
status: draft
basis: planning_intent
---

# configurable_truth_table 需求合同
...
| 索引 ID | SEL-019 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/configurable_truth_table |
| 功能家族 | Configurable Truth Table |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：依据输入索引返回配置真值表的输出向量，可用 case/LUT/ROM 实现。

边界焦点：运行时更新如支持需采用写保护或原子切表；读写冲突明确，未配置表项不作为有效功能输出。
...
候选实现：LUT/case/ROM mapped。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 依据输入索引返回配置真值表的输出向量，可用 case/LUT/ROM 实现。 |
| REQ-002 | 关键边界 | 运行时更新如支持需采用写保护或原子切表；读写冲突明确，未配置表项不作为有效功能输出。 |
