...
if command -v vcs >/dev/null 2>&1; then
  # 正向：多实现 × 多参数点编译/elab 矩阵（可复现；+incdir+rtl 找 gen/*.sv）
  : > "$EV/param_matrix.txt"
  for impl in 0 1 2 3 4; do
    for n in 8 16 32 64; do
      if ( cd "$WORK" && vcs -full64 -timescale=1ns/1ps -sverilog \
          -pvalue+popcount.DATA_W=$n \
          -pvalue+popcount.PC_IMPL=$impl \
          +incdir+"$P/rtl" "$RTL" -o /tmp/pc_g3_${impl}_${n} > "$EV/tmp.log" 2>&1 ); then
        echo "PASS impl=$impl W=$n" | tee -a "$EV/param_matrix.txt"
      else
        echo "FAIL impl=$impl W=$n" | tee -a "$EV/param_matrix.txt"; cat "$EV/tmp.log"; exit 1
      fi
    done
  done
  ...
  ( cd "$WORK" && vcs -full64 -timescale=1ns/1ps -sverilog \
      +incdir+"$P/rtl" "$RTL" "$NEG_TB" -top negative_elab_tb \
      -o /tmp/pc_neg > "$EV/negative_elab.txt" 2>&1 )
  neg_rc=$?
  set -e
  if [ "$neg_rc" -eq 0 ]; then
...
if command -v spyglass >/dev/null 2>&1; then
  ...
  cat > "$SGW/pc_lint.prj" <<EOF
set_option enableSV yes
set_option enableSV09 yes
set_option top popcount
set_option incdir $P/rtl
set_option mthresh 200000
read_file -type hdl $RTL
EOF
  ( cd "$SGW" && spyglass -project pc_lint.prj -goal lint/lint_rtl -batch \
