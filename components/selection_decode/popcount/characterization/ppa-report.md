...
**核心发现——显式 FA 网表路线验证成功**：同约束下比列计数递推形态
面积 **−78%**（271 vs 1258）、时序由违例转为 MET。证实"让综合器看到真 FA 结构"
远优于"算术表达式推导压缩"。但对比 tree 仍付出 ~2.2× 面积——**tree 的默认推荐
地位经帕累托数据确认不变**；Wallace/Dadda 保留为 PPA 探索视图（experimental）。
