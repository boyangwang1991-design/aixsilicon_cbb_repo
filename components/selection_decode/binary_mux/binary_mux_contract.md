...
| 索引 ID | SEL-001 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/binary_mux |
| 功能家族 | 2:1/N:1 Binary Mux |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：按二进制选择索引输出指定输入的数据，索引有效时输出与该输入逐位一致。

边界焦点：非二次幂路数的多余编码输出零并拉低选择有效标志；禁止索引截断映射到其他输入。
...
候选实现：Linear、balanced tree、pipelined。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按二进制选择索引输出指定输入的数据，索引有效时输出与该输入逐位一致。 |
| REQ-002 | 关键边界 | 非二次幂路数的多余编码输出零并拉低选择有效标志；禁止索引截断映射到其他输入。 |
