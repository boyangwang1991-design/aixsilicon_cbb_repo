...
registry_id: SEL-005
name: cross_point_switch
version: 0.1.0
status: draft
basis: planning_intent
---

# cross_point_switch 需求合同
...
| 索引 ID | SEL-005 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/cross_point_switch |
| 功能家族 | Cross-point Switch |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：按交叉点选择矩阵连接输入与输出，支持配置的全连接或稀疏连接拓扑。

边界焦点：无仲裁配置时冲突选择由调用者禁止并由检查发现；有仲裁配置只接受合法匹配集合，不承担系统协议路由。
...
候选实现：full、sparse、staged。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按交叉点选择矩阵连接输入与输出，支持配置的全连接或稀疏连接拓扑。 |
| REQ-002 | 关键边界 | 无仲裁配置时冲突选择由调用者禁止并由检查发现；有仲裁配置只接受合法匹配集合，不承担系统协议路由。 |
