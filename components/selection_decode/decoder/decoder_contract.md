...
| 索引 ID | SEL-008 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/decoder |
| 功能家族 | Decoder |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：把合法二进制索引展开为独热输出，可用使能控制全部输出为零。

边界焦点：索引超出配置路数时全部输出零并报无效；使能关闭时无输出置位。
...
候选实现：binary-to-onehot、segmented。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把合法二进制索引展开为独热输出，可用使能控制全部输出为零。 |
| REQ-002 | 关键边界 | 索引超出配置路数时全部输出零并报无效；使能关闭时无输出置位。 |
