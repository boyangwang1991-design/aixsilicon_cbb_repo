...
| 索引 ID | SEL-006 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/binary_encoder |
| 功能家族 | Binary Encoder |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：把独热输入位置转换为二进制索引，并给出 valid 和多热错误指示。

边界焦点：无置位输出无效；多热默认报错，不以未声明的优先逻辑隐式选择一个。
...
候选实现：combinational、tree。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把独热输入位置转换为二进制索引，并给出 valid 和多热错误指示。 |
| REQ-002 | 关键边界 | 无置位输出无效；多热默认报错，不以未声明的优先逻辑隐式选择一个。 |
