...
registry_id: SEL-010
name: thermometer_codec
version: 0.1.0
status: draft
basis: planning_intent
---

# thermometer_codec 需求合同
...
| 索引 ID | SEL-010 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/selection_decode / components/selection_decode/thermometer_codec |
| 功能家族 | Thermometer Encoder/Decoder |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：在整数计数与单调温度计码之间转换，编码时恰好产生指定数量的连续置位。

边界焦点：解码输入出现空洞时给出非法标志；计数大于码宽拒绝，不做静默模回绕。
...
候选实现：binary/thermometer conversion。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在整数计数与单调温度计码之间转换，编码时恰好产生指定数量的连续置位。 |
| REQ-002 | 关键边界 | 解码输入出现空洞时给出非法标志；计数大于码宽拒绝，不做静默模回绕。 |
