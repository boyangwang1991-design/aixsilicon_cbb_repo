...
registry_id: CRP-002
name: hierarchical_clock_gating
version: 0.1.0
status: draft
basis: planning_intent
---

# hierarchical_clock_gating 需求合同
...
| 索引 ID | CRP-002 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/hierarchical_clock_gating |
| 功能家族 | Hierarchical Clock Gating Controller |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：汇聚叶级活动请求形成上级时钟请求，并在各层传递对应测试覆盖控制。

边界焦点：上级关闭前所有叶级必须确认空闲；任一有效叶请求均能保持上级时钟开启，不能产生无法唤醒的闭环。
...
候选实现：local/global enables。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 汇聚叶级活动请求形成上级时钟请求，并在各层传递对应测试覆盖控制。 |
| REQ-002 | 关键边界 | 上级关闭前所有叶级必须确认空闲；任一有效叶请求均能保持上级时钟开启，不能产生无法唤醒的闭环。 |
