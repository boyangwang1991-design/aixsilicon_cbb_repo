...
registry_id: CRP-016
name: power_domain_handshake
version: 0.1.0
status: draft
basis: planning_intent
---

# power_domain_handshake 需求合同
...
| 索引 ID | CRP-016 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/power_domain_handshake |
| 功能家族 | Power-domain Handshake |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：在局部电源控制请求下协调 quiesce、隔离、保留和电源确认握手。

边界焦点：未完成隔离/保留不得确认可断电；输出仅为数字请求，不驱动电源网络或代替 SoC 电源策略。
...
候选实现：request/ack/isolate/save。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在局部电源控制请求下协调 quiesce、隔离、保留和电源确认握手。 |
| REQ-002 | 关键边界 | 未完成隔离/保留不得确认可断电；输出仅为数字请求，不驱动电源网络或代替 SoC 电源策略。 |
