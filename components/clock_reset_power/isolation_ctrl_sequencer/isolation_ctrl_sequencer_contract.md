...
registry_id: CRP-017
name: isolation_ctrl_sequencer
version: 0.1.0
status: draft
basis: planning_intent
---

# isolation_ctrl_sequencer 需求合同
...
| 索引 ID | CRP-017 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/isolation_ctrl_sequencer |
| 功能家族 | Isolation Control Sequencer |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：根据域开关请求按安全顺序产生隔离控制，断电前先夹持，上电稳定后再解除夹持。

边界焦点：超时保持夹持并输出错误；异常重入不能提前解除隔离，控制必须由存活域提供。
...
候选实现：clamp/unclamp ordering。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 根据域开关请求按安全顺序产生隔离控制，断电前先夹持，上电稳定后再解除夹持。 |
| REQ-002 | 关键边界 | 超时保持夹持并输出错误；异常重入不能提前解除隔离，控制必须由存活域提供。 |
