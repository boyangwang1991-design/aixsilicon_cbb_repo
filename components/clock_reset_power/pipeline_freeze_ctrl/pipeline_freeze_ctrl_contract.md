...
registry_id: CRP-013
name: pipeline_freeze_ctrl
version: 0.1.0
status: draft
basis: planning_intent
---

# pipeline_freeze_ctrl 需求合同
...
| 索引 ID | CRP-013 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/pipeline_freeze_ctrl |
| 功能家族 | Pipeline Freeze Controller |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：对一组相关流水状态生成一致的冻结和释放使能，使数据、valid 和侧带共同停留。

边界焦点：冻结期间不得推进计数器或事务标签；解冻后从原位置继续，不能重新发出已完成事务。
...
候选实现：clock/data enable。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对一组相关流水状态生成一致的冻结和释放使能，使数据、valid 和侧带共同停留。 |
| REQ-002 | 关键边界 | 冻结期间不得推进计数器或事务标签；解冻后从原位置继续，不能重新发出已完成事务。 |
