...
registry_id: CRP-005
name: clock_switch_ctrl
version: 0.1.0
status: draft
basis: planning_intent
---

# clock_switch_ctrl 需求合同
...
| 索引 ID | CRP-005 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/clock_switch_ctrl |
| 功能家族 | Clock Switch Controller |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：按停止旧源、确认安全、选择新源、恢复输出的握手顺序控制外部时钟复用单元。

边界焦点：目标时钟不可用时保持安全停机并报告超时；本核不凭组合选择信号承诺任意时钟源无毛刺切换。
...
候选实现：glitch-free mux protocol。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 按停止旧源、确认安全、选择新源、恢复输出的握手顺序控制外部时钟复用单元。 |
| REQ-002 | 关键边界 | 目标时钟不可用时保持安全停机并报告超时；本核不凭组合选择信号承诺任意时钟源无毛刺切换。 |
