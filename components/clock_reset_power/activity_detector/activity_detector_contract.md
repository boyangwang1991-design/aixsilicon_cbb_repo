...
| 索引 ID | CRP-015 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/activity_detector |
| 功能家族 | Activity Detector |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：对配置的信号翻转或事件进行采样，在统计窗口内输出活动位图或活动次数。

边界焦点：采样间隔内不可见的翻转不计为已观测；窗口边界上的样本明确归属，禁止重复计数。
...
候选实现：toggle/event/window。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对配置的信号翻转或事件进行采样，在统计窗口内输出活动位图或活动次数。 |
| REQ-002 | 关键边界 | 采样间隔内不可见的翻转不计为已观测；窗口边界上的样本明确归属，禁止重复计数。 |
