...
| 索引 ID | CRP-014 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/idle_detector |
| 功能家族 | Idle Detector |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在全部活动条件为假且未完成计数为零时累计空闲周期，达到门限后输出 idle。

边界焦点：任一新活动或未完成操作立即取消 idle；计数饱和，不用计数回绕触发周期性误判。
...
候选实现：counter/window/protocol-aware。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在全部活动条件为假且未完成计数为零时累计空闲周期，达到门限后输出 idle。 |
| REQ-002 | 关键边界 | 任一新活动或未完成操作立即取消 idle；计数饱和，不用计数回绕触发周期性误判。 |
