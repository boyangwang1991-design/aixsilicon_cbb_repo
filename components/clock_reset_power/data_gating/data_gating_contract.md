...
| 索引 ID | CRP-012 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/data_gating |
| 功能家族 | Data Gating |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：用有效性或变化检测抑制无效数据更新，同时保留消费方所需的 valid 和控制信息。

边界焦点：有效数据不能因与上一拍相等而被丢弃；变化检测只影响寄存写使能，不能改变事务数量。
...
候选实现：valid-based/change-based。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 用有效性或变化检测抑制无效数据更新，同时保留消费方所需的 valid 和控制信息。 |
| REQ-002 | 关键边界 | 有效数据不能因与上一拍相等而被丢弃；变化检测只影响寄存写使能，不能改变事务数量。 |
