...
registry_id: CRP-006
name: clock_request_ack
version: 0.1.0
status: draft
basis: planning_intent
---

# clock_request_ack 需求合同
...
| 索引 ID | CRP-006 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/clock_request_ack |
| 功能家族 | Clock Request/Acknowledge |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：保持时钟请求直到源端确认时钟可用，并在使用者空闲后执行关钟握手。

边界焦点：关钟请求后新请求到达应取消关钟或重新启动；ack 不得在时钟实际不可用时被解释为可工作。
...
候选实现：gated source handshake。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 保持时钟请求直到源端确认时钟可用，并在使用者空闲后执行关钟握手。 |
| REQ-002 | 关键边界 | 关钟请求后新请求到达应取消关钟或重新启动；ack 不得在时钟实际不可用时被解释为可工作。 |
