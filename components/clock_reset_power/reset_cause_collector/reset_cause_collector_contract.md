...
registry_id: CRP-009
name: reset_cause_collector
version: 0.1.0
status: draft
basis: planning_intent
---

# reset_cause_collector 需求合同
...
| 索引 ID | CRP-009 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/reset_cause_collector |
| 功能家族 | Reset Cause Collector |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：对多个复位原因输入形成粘滞位图并提供局部清除和首因记录接口。

边界焦点：同拍新原因与清除冲突时新事件优先；不实现软件 CSR，不因系统复位清掉要求跨复位保留的记录域。
...
候选实现：sticky/priority encode。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对多个复位原因输入形成粘滞位图并提供局部清除和首因记录接口。 |
| REQ-002 | 关键边界 | 同拍新原因与清除冲突时新事件优先；不实现软件 CSR，不因系统复位清掉要求跨复位保留的记录域。 |
