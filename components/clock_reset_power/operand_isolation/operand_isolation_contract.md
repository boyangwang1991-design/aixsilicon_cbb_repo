...
registry_id: CRP-011
name: operand_isolation
version: 0.1.0
status: draft
basis: planning_intent
---

# operand_isolation 需求合同
...
| 索引 ID | CRP-011 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/operand_isolation |
| 功能家族 | Operand Isolation |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在操作无效时把算术输入固定为零或保持值，以降低下游无效翻转。

边界焦点：操作有效时必须等价直通，隔离选择不得阻断有效操作；保持模式需要显式时钟与初始化语义。
...
候选实现：input hold/zero/mux isolation。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在操作无效时把算术输入固定为零或保持值，以降低下游无效翻转。 |
| REQ-002 | 关键边界 | 操作有效时必须等价直通，隔离选择不得阻断有效操作；保持模式需要显式时钟与初始化语义。 |
