...
| 索引 ID | CRP-007 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/reset_synchronizer |
| 功能家族 | Reset Synchronizer |
| 优先级 | P0 |
| 当前登记状态 | planned |
...
需求焦点：实现复位异步置位、目的时钟同步释放，释放延迟为配置的同步链级数。

边界焦点：源复位重新拉低时立即重置同步链；无目的时钟时复位保持有效，不因源复位撤销而异步释放。
...
候选实现：parameterized stages。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 实现复位异步置位、目的时钟同步释放，释放延迟为配置的同步链级数。 |
| REQ-002 | 关键边界 | 源复位重新拉低时立即重置同步链；无目的时钟时复位保持有效，不因源复位撤销而异步释放。 |
