...
registry_id: CRP-020
name: high_fanout_replicator
version: 0.1.0
status: draft
basis: planning_intent
---

# high_fanout_replicator 需求合同
...
| 索引 ID | CRP-020 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/high_fanout_replicator |
| 功能家族 | High-fanout Replicator |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：把高扇出控制复制为多个功能等价分支，并为可选寄存分支声明固定延迟。

边界焦点：数据路径必须与复制后的控制延迟匹配；禁止将不同分支偏斜误当作同周期广播。
...
候选实现：register/mux/decode replication。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把高扇出控制复制为多个功能等价分支，并为可选寄存分支声明固定延迟。 |
| REQ-002 | 关键边界 | 数据路径必须与复制后的控制延迟匹配；禁止将不同分支偏斜误当作同周期广播。 |
