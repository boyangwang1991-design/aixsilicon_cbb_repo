...
registry_id: CRP-003
name: auto_clock_gating_detector
version: 0.1.0
status: draft
basis: planning_intent
---

# auto_clock_gating_detector 需求合同
...
| 索引 ID | CRP-003 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/clock_reset_power / components/clock_reset_power/auto_clock_gating_detector |
| 功能家族 | Auto Clock Gating Detector |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：根据无未完成操作且持续空闲达到门限的条件发出关钟请求，并在新活动时请求唤醒。

边界焦点：不能仅凭输入 valid 为零判断内部空闲；唤醒检测必须位于仍有时钟或异步可达的控制域。

拟复用场景：IP 局部关钟/空闲控制；复位分区及低功耗握手。这些是需求场景，不表示已有消费者依赖或完成集成。

## 2. 接口与配置需求

接口角色：活动/启停/复位请求、时钟或宏确认输入；局部控制使能、复位、握手状态输出。只要求实现本核实际需要的角色，不为目录分类添加空接口。

配置合法域：计时门限为正整数（零若表示禁用须显式编码）；同步级数≥2；分支数≥1；极性及存活控制域必须声明。数学合法域不代表无限规模均可实现；G1 必须在有限支持集合中给出默认、最小、最大和非法组合，最大支持配置需通过展开与验证。

候选实现：idle/activity based。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 根据无未完成操作且持续空闲达到门限的条件发出关钟请求，并在新活动时请求唤醒。 |
| REQ-002 | 关键边界 | 不能仅凭输入 valid 为零判断内部空闲；唤醒检测必须位于仍有时钟或异步可达的控制域。 |
| REQ-003 | 时序/接受 | 控制确认必须反映真实前提完成；时钟路径采用受支持单元与明确安全窗口，不以组合毛刺作为时钟。 |
| REQ-004 | 复位/重启 | 复位/掉电默认进入本核定义的安全状态；唤醒检测与恢复控制不能依赖已经关闭的唯一时钟。 |
...
| 验收项 | 覆盖需求 | 通过条件 |
|---|---|---|
| AC-01 功能 | REQ-001、REQ-002 | 用独立参考关系逐项检查核心功能与边界，覆盖：启停中重入、门限边界、停钟、确认超时、复位重入、同拍空闲与新工作以及低功耗顺序检查。 |
