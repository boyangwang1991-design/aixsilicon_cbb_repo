...
registry_id: TEC-021
name: fpga_memory_wrapper
version: 0.1.0
status: draft
basis: planning_intent
---

# fpga_memory_wrapper 需求合同
...
| 索引 ID | TEC-021 |
| 资产版本 | 0.1.0 |
| 分类/路径 | adapters / adapters/fpga_memory_wrapper |
| 功能家族 | FPGA Memory Wrapper |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在 BRAM/URAM/LUTRAM 目标上提供固定的逻辑存储端口契约，并支持目标能力检查。

边界焦点：读写同址、输出寄存和初始化行为必须与所选资源一致；不能把 FPGA 初始化保证直接沿用到 ASIC。
...
候选实现：BRAM/URAM/LUTRAM。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在 BRAM/URAM/LUTRAM 目标上提供固定的逻辑存储端口契约，并支持目标能力检查。 |
| REQ-002 | 关键边界 | 读写同址、输出寄存和初始化行为必须与所选资源一致；不能把 FPGA 初始化保证直接沿用到 ASIC。 |
