...
registry_id: TEC-017
name: rom_macro_wrapper
version: 0.1.0
status: draft
basis: planning_intent
---

# rom_macro_wrapper 需求合同
...
| 索引 ID | TEC-017 |
| 资产版本 | 0.1.0 |
| 分类/路径 | adapters / adapters/rom_macro_wrapper |
| 功能家族 | ROM Macro Wrapper |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：统一 ROM 宏的地址与固定延迟数据输出，绑定明确的内容版本或掩模配置。

边界焦点：内容尺寸与端口位宽必须匹配；不能让仿真文件内容与目标掩模 ROM 数据不一致而无检查。
...
候选实现：Mask ROM、compiler ROM。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 统一 ROM 宏的地址与固定延迟数据输出，绑定明确的内容版本或掩模配置。 |
| REQ-002 | 关键边界 | 内容尺寸与端口位宽必须匹配；不能让仿真文件内容与目标掩模 ROM 数据不一致而无检查。 |
