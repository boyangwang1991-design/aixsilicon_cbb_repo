...
registry_id: TEC-010
name: isolation_cell_wrapper
version: 0.1.0
status: draft
basis: planning_intent
---

# isolation_cell_wrapper 需求合同
...
| 索引 ID | TEC-010 |
| 资产版本 | 0.1.0 |
| 分类/路径 | adapters / adapters/isolation_cell_wrapper |
| 功能家族 | Isolation Cell Wrapper |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：在隔离使能下把跨域信号夹到声明的零、一或保持值，非隔离状态按单元语义传递。

边界焦点：夹持极性和单元供电域明确；保持型隔离与固定夹持不能混用，UPF 绑定不能重复插入。
...
候选实现：Clamp-0/1、Latch isolation。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 在隔离使能下把跨域信号夹到声明的零、一或保持值，非隔离状态按单元语义传递。 |
| REQ-002 | 关键边界 | 夹持极性和单元供电域明确；保持型隔离与固定夹持不能混用，UPF 绑定不能重复插入。 |
