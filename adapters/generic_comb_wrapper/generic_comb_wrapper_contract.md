...
registry_id: TEC-001
name: generic_comb_wrapper
version: 0.1.0
status: draft
basis: planning_intent
---

# generic_comb_wrapper 需求合同
...
| 索引 ID | TEC-001 |
| 资产版本 | 0.1.0 |
| 分类/路径 | adapters / adapters/generic_comb_wrapper |
| 功能家族 | 通用组合标准单元 Wrapper |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：为逻辑门和组合单元提供可移植布尔功能及受控工艺映射。

边界焦点：每个映射与通用 RTL 真值表等价；禁止在通用接口中承诺具体驱动强度或延迟数字。
...
候选实现：AND/OR/XOR/MUX/AOI/OAI 映射。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 为逻辑门和组合单元提供可移植布尔功能及受控工艺映射。 |
| REQ-002 | 关键边界 | 每个映射与通用 RTL 真值表等价；禁止在通用接口中承诺具体驱动强度或延迟数字。 |
