...
| 索引 ID | TEC-004 |
| 资产版本 | 0.1.0 |
| 分类/路径 | adapters / adapters/latch_wrapper |
| 功能家族 | Latch Wrapper |
| 优先级 | P3 |
| 当前登记状态 | planned |
...
需求焦点：封装透明锁存器，使能窗口内透明、窗口关闭后保持最近输入值。

边界焦点：必须给出 latch 极性和复位能力；不把锁存器宣称为边沿触发器，时序借用需约束支持。
...
候选实现：普通、门控、Scan。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 封装透明锁存器，使能窗口内透明、窗口关闭后保持最近输入值。 |
| REQ-002 | 关键边界 | 必须给出 latch 极性和复位能力；不把锁存器宣称为边沿触发器，时序借用需约束支持。 |
