...
registry_id: TEC-015
name: sram_macro_wrapper
version: 0.1.0
status: draft
basis: planning_intent
---

# sram_macro_wrapper 需求合同
...
| 索引 ID | TEC-015 |
| 资产版本 | 0.1.0 |
| 分类/路径 | adapters / adapters/sram_macro_wrapper |
| 功能家族 | SRAM Macro Wrapper |
| 优先级 | P0 |
| 当前登记状态 | planned |

## 1. 定位与使用场景

统一读延迟、mask、sleep、BIST

需求焦点：统一 SRAM 宏的地址、数据、写掩码、使能及可选 sleep/BIST 信号，提供目标宏映射。

边界焦点：端口类型和读延迟按具体宏声明；未支持的 RAW、复位清存储或初始化能力不可通过包装虚构。
...
候选实现：1P、SP、1R1W、TDP。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 统一 SRAM 宏的地址、数据、写掩码、使能及可选 sleep/BIST 信号，提供目标宏映射。 |
| REQ-002 | 关键边界 | 端口类型和读延迟按具体宏声明；未支持的 RAW、复位清存储或初始化能力不可通过包装虚构。 |
