...
registry_id: TEC-022
name: fpga_dsp_wrapper
version: 0.1.0
status: draft
basis: planning_intent
---

# fpga_dsp_wrapper 需求合同
...
| 索引 ID | TEC-022 |
| 资产版本 | 0.1.0 |
| 分类/路径 | adapters / adapters/fpga_dsp_wrapper |
| 功能家族 | FPGA DSP Wrapper |
| 优先级 | P2 |
| 当前登记状态 | planned |
...
需求焦点：把指定算术核映射到 FPGA DSP 资源，统一乘加、预加器、级联及流水控制接口。

边界焦点：符号扩展、截断位置和各级使能与逻辑模型一致；超出资源位宽时明确分解或拒绝，不静默截断。
...
候选实现：DSP slice、MAC、pre-adder。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 把指定算术核映射到 FPGA DSP 资源，统一乘加、预加器、级联及流水控制接口。 |
| REQ-002 | 关键边界 | 符号扩展、截断位置和各级使能与逻辑模型一致；超出资源位宽时明确分解或拒绝，不静默截断。 |
