...
| 索引 ID | TEC-002 |
| 资产版本 | 0.1.0 |
| 分类/路径 | adapters / adapters/dff_wrapper |
| 功能家族 | DFF Wrapper |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：封装数据触发器及显式使能、复位/置位、扫描变体，保持目标单元的采样语义。

边界焦点：异步与同步复位不能互换；set/reset 同时有效的非法性及极性按目标单元契约定义。
...
候选实现：普通、Enable、Set/Reset、Scan。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 封装数据触发器及显式使能、复位/置位、扫描变体，保持目标单元的采样语义。 |
| REQ-002 | 关键边界 | 异步与同步复位不能互换；set/reset 同时有效的非法性及极性按目标单元契约定义。 |
