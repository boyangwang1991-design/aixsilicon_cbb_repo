...
registry_id: NOC-010
name: link_register_slice
version: 0.1.0
status: draft
basis: planning_intent
---

# link_register_slice 需求合同
...
| 索引 ID | NOC-010 |
| 资产版本 | 0.1.0 |
| 分类/路径 | components/noc_interconnect / components/noc_interconnect/link_register_slice |
| 功能家族 | Link Register Slice |
| 优先级 | P1 |
| 当前登记状态 | planned |
...
需求焦点：对点到点局部链路数据和反向流控提供配置的寄存切分，并保持接受次数守恒。

边界焦点：链路停顿期间保持数据及侧带；只处理局部传输，不执行链路训练、重放或完整协议事务。
...
候选实现：forward/reverse/full。这些是选型空间，不要求首版同时实现全部结构。对比时固定相同可观察功能配置；改变延迟、舍入或丢弃策略须作为显式配置差异。

## 3. 可验证需求

| ID | 类别 | 要求 |
|---|---|---|
| REQ-001 | 核心功能 | 对点到点局部链路数据和反向流控提供配置的寄存切分，并保持接受次数守恒。 |
| REQ-002 | 关键边界 | 链路停顿期间保持数据及侧带；只处理局部传输，不执行链路训练、重放或完整协议事务。 |
